...
## React frontend: Agent-UI design system

The production React SPA (`frontend/`, Agent-UI migration) adopts the **Agent-UI** interaction model for application chrome: a flat, zinc-neutral shell with bordered panels, a left nav rail, and shadcn-style primitives. This is intentional product chrome, not a replacement of the conversation rules in this document.

**What “Agent-UI” means here**

- **Shell chrome** — login, three-panel layout (sessions | chat | workspace), composer footer, settings nav, and session list rows follow Agent-UI–like surfaces: `bg-card`, thin `border-border`, `rounded-xl` panels, Inter / system sans, lucide icons. No glass blur, grain overlay, or decorative glow on routine controls.
- **Transcript** — user bubbles, assistant prose, tool/thinking disclosure, and semantic state colors still follow the hierarchy and metadata rules below. Feature CSS (`frontend/src/features/**`) consumes legacy Hermes variables bridged from the new token layer so chat behavior stays aligned with `static/style.css` contracts during cutover.
- **Motion** — Framer Motion remains for shell stagger and message reveal; honor `prefers-reduced-motion` (see `docs/UIUX-GUIDE.md`).

**Token stack (do not bypass)**

| Layer | File | Role |
|-------|------|------|
| Tailwind v4 + shadcn semantics | `frontend/src/index.css` | `@theme` color aliases, `:root` / `.dark` RGB tokens (`--background`, `--card`, `--border`, …), Agent-UI session-row active styles |
| Bridge to feature CSS | `frontend/src/styles/theme-bridge.css` | Maps shadcn tokens → `--bg`, `--surface`, `--text-muted`, composer/message vars; sets `--glass-blur: 0` so legacy `glass-*` names resolve to flat panels |
| Hermes palette + scale | `frontend/src/styles/tokens.css` | DESIGN.md source colors, type scale, spacing; semantic success/warning/error for transcript |
| Global typography helpers | `frontend/src/styles/global.css` | `.lum-prose`, `.lum-user-message`, `.lum-meta`, etc. |
**Theme switching**

- `ThemeProvider` (`frontend/src/hooks/useTheme.tsx`) persists `light` | `dark` | `system` in `localStorage` (`hermes-webui-theme`) and toggles the `.dark` class on `<html>`.

**UI primitives**

- `cn()` — `frontend/src/lib/utils.ts` (`clsx` + `tailwind-merge`).
- `Button`, `Input` — `frontend/src/components/ui/` (`class-variance-authority` variants).
- `ShellPanel` — flat bordered panel; `GlassPanel` is a deprecated alias.
- Prefer Tailwind utility classes on shell routes; use bridged CSS variables inside chat/workspace feature modules until those modules are fully utility-based.

**Dependencies (shell styling only)**

`tailwindcss`, `@tailwindcss/vite`, `clsx`, `tailwind-merge`, `class-variance-authority`, `lucide-react`.

When adding chrome, extend the semantic tokens in `index.css` and the bridge in `theme-bridge.css` instead of hardcoding zinc hex values in components. When adding transcript UI, respect the conversation hierarchy in this file and `docs/UIUX-GUIDE.md`.
