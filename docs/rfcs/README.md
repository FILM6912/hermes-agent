...
- [`user-admin-system.md`](user-admin-system.md) — Multi-user WebUI accounts
  with admin vs regular-user roles, 1:1 user–profile binding, session
  extension (not JWT), authorization matrix, and FastAPI `Depends()` /
  middleware integration; migration from single-password auth.
