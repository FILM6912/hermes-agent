...
**Backend:** Python + FastAPI serve Hermes `/api/v1/*` (unchanged). **UI:** a
**React + Vite** SPA in `frontend-v2/` (built to `static/dist-v2/`) whose shell is based
on [Agent-UI](https://github.com/FILM6912/Agent-UI) (FILM6912/Agent-UI). Legacy
vanilla assets under `static/` remain only during cutover — new UI work belongs in
`frontend-v2/src/`.

**Archive — react-luminous:** the earlier in-repo **Agent-UI**
(branch `feat/react-luminous-frontend`, glass/shell experiments) is archived
reference only. Do not extend that visual system; follow Agent-UI + shadcn chrome
below.
...
## Agent-UI design system (React UI)

The React SPA (`frontend/`, built to `static/dist-v2/`) is based on
[Agent-UI](https://github.com/FILM6912/Agent-UI) for application chrome, wired to
the **Hermes WebUI API** (`/api/v1/*`). It keeps the calm developer-console
information hierarchy from [`DESIGN.md`](../DESIGN.md). See the **React frontend:
Agent-UI design system** section in `DESIGN.md` for the full token stack and file
map. Prior **react-luminous** / Agent-UI styling is archived — not for
new features.

**Shell chrome (Agent-UI / shadcn)**

- **Surfaces:** zinc neutrals, flat `bg-card` panels with thin borders — no glass
  blur, grain overlay, or ambient glow on routine controls.
- **Layout:** left nav rail + three-panel shell (sessions | chat | workspace);
  `ShellPanel` / `ShellLayout` in `frontend-v2/src/components/shell/`.
- **Primitives:** Tailwind v4 utilities, `cn()`, `Button` / `Input` with CVA
  variants, `lucide-react` icons.
- **Theme:** `light` | `dark` | `system` via `ThemeProvider` and the `.dark`
  class on `<html>`; tokens in `frontend-v2/src/index.css`, bridge in
  `theme-bridge.css`.

**Transcript and features**

Chat, tool cards, composer, and workspace modules still use bridged Hermes CSS
variables (`--bg`, `--user-bubble-bg`, semantic colors from `tokens.css`) so
behavior matches legacy contracts and e2e selectors. Do not make every tool call
look like a separate chat message; follow the conversation hierarchy below.

**Motion**

Framer Motion for shell load (staggered sidebar → chat → workspace), route
transitions, and message reveal. Honor `prefers-reduced-motion`: disable stagger
and decorative animation when the user requests reduced motion.

Conversation-first rules below still apply. Flat shell chrome is not a substitute
for readable transcript hierarchy.
...
- quiet surfaces (glass is allowed when it stays legible),
- clear spacing,
...
Theme and skin work should use the design token system. During React cutover:

- **Shell / shadcn semantics** — `frontend-v2/src/index.css` (`:root` / `.dark`) and
  `theme-bridge.css` (maps to `--bg`, `--surface`, and other feature variables).
- **Hermes palette + transcript** — `frontend-v2/src/styles/tokens.css` (DESIGN.md
  colors and semantic states).
- **Legacy vanilla** — `static/style.css` until cutover completes.

`THEMES.md` documents palette intent; port skins through the bridge rather than
hardcoding colors in components.

Legacy implementation (vanilla, pre-cutover) has two appearance axes, sourced from
`static/boot.js`:
...
- add new vanilla JS in `static/*.js` after React cutover (use `frontend/`),
...
|------|-------------------|
| Install deps | `cd frontend-v2 && npm ci` |
| Dev (hot reload, API proxy) | `npm run dev` — proxies `/api` to backend (see `vite.config.ts`) |
| Production bundle | `npm run build` → `static/dist-v2/` |
| Typecheck only | `npm run typecheck` |

Run the FastAPI backend separately (`bootstrap.py` or uvicorn). For UI changes,
verify desktop, narrow, and mobile breakpoints in both Vite dev and a built
bundle served by uvicorn. See `AGENTS.md` and `ARCHITECTURE.md` section 2.5.
