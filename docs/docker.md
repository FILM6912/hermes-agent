...
### Local embedding GPU (optional)

All compose files that include **hermes-webui** request GPU passthrough by default
(`gpus: all`, `NVIDIA_VISIBLE_DEVICES=all`) for local Hugging Face
embedding/reranker/ASR. The host needs the [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html)
installed alongside the driver. On hosts without NVIDIA, `docker compose up` may
log a warning; the WebUI still starts and embedding falls back to CPU when CUDA
is not visible inside the container.

```bash
# Default — GPU when available
docker compose up -d --build

# Explicit CPU-only (no GPU device requests)
docker compose -f docker-compose.yml -f docker-compose.cpu.yml up -d --build

# Verify CUDA inside the running container
docker exec hermes-webui /app/venv/bin/python -c "import torch; print(torch.cuda.is_available())"
```

`docker-compose.gpu.yml` is deprecated (a no-op kept for older scripts). See
`.env.docker.example` for model download paths under `./models`.
...
### 3b. "Chat upload fails with PermissionError on workspace/.uploads"

**Symptom**: Container logs show `Permission denied: .../workspace/<account>/.uploads/<file>` when uploading from chat.

**Cause**: Multi-user workspaces live under the bind-mounted `workspace/` tree (for example `workspace/pathom_u/.uploads/`). Subdirectories created as `root` on the host or during `docker exec` stay unwritable for the `hermeswebui` runtime user even when the top-level mount owner looks correct.

**Fix**:
- Restart the WebUI container after upgrading — root init now recursively chowns the shared workspace mount on startup.
- For an existing deployment without rebuilding yet, fix ownership once from the host or as root in the container:

```bash
# Host bind mount (adjust paths to your compose layout)
sudo chown -R "${UID:-1000}:${GID:-1000}" ./workspace

# Or inside the running container
docker exec -u root hermes-webui chown -R hermeswebui:hermeswebui /home/hermeswebui/.hermes/workspace
```
