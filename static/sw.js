 ...
 *
 * Supports the legacy vanilla bundle (versioned static/*.js) and Vite SPAs in
 * static/dist (Luminous React) and static/dist-v2 (Agent-UI). Hashed assets
 * under assets/ are matched by path prefix at fetch time because filenames
 * change every build. manifest.json and sw.js are always served from static/
 * at the site root (/manifest.json, /sw.js), not from the dist output dirs.
 */
...
const SHELL_ASSETS = [
  ...
  './manifest.json',
  // Vite SPA shell (static/dist or static/dist-v2; served at / when SPA is on)
  './index.html',
];

/** Vite dist hashed bundles — network-first, not install pre-cache (names change per build). */
function isViteShellRelPath(relPath) {
  if (!relPath) return false;
  if (relPath === 'index.html') return true;
  if (relPath.startsWith('assets/')) {
    return /\.(js|css|woff2?|map)$/i.test(relPath);
  }
  return false;
}

function isShellAsset(relPath, shellPath) {
  return SHELL_ASSETS.includes(shellPath) || isViteShellRelPath(relPath);
}
...
self.addEventListener('fetch', (event) => {
  ...
  // requests can look like /hermes/api/v1/sessions rather than /api/v1/sessions.
  if (
  ...
  if (!isShellAsset(relPath, shellPath)) return;
