...
function _connectTerminalOutput(){
  ...
  const url=new URL(apiV1Path('api/terminal/output'),document.baseURI||location.href);
...
window.addEventListener('beforeunload',()=>{
  ...
  if(TERMINAL_UI.sessionId){
    const url=new URL(apiV1Path('api/terminal/close'),document.baseURI||location.href).href;
