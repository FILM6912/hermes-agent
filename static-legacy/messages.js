...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
      ...
      try{
        if(streamId){
          ...
          if(st.active){
            ...
            _wireSSE(new EventSource(new URL(apiV1Path(`api/chat/stream?stream_id=${encodeURIComponent(streamId)}`),document.baseURI||location.href).href,{withCredentials:true}));
            return;
          }
        }
      }catch(_){
        ...
        setTimeout(async()=>{
          try{
            ...
            if(st.replay_available){
              ...
              _wireSSE(new EventSource(new URL(apiV1Path(`api/chat/stream?stream_id=${encodeURIComponent(streamId)}${_runJournalReplayParams()}`),document.baseURI||location.href).href,{withCredentials:true}));
              return;
            }
          }catch(_){
            if(_deferStreamErrorIfOffline()) return;
  ...
  (async()=>{
    ...
    _wireSSE(new EventSource(new URL(apiV1Path(`api/chat/stream?stream_id=${encodeURIComponent(streamId)}${replayParams}`),document.baseURI||location.href).href,{withCredentials:true}));
  })();
...
function startApprovalPolling(sid) {
  ...
  try {
    const es = new EventSource(new URL(apiV1Path('api/approval/stream?session_id=' + encodeURIComponent(sid)), document.baseURI || location.href).href);
...
function startClarifyPolling(sid) {
  ...
  try {
    _clarifyEventSource = new EventSource(new URL(apiV1Path('api/clarify/stream?session_id=' + encodeURIComponent(sid)), document.baseURI || location.href).href);
  } catch(e) {
...
function attachBtwStream(parentSid, streamId, question){
  ...
  const src=new EventSource(new URL(apiV1Path('api/chat/stream?stream_id='+encodeURIComponent(streamId)), document.baseURI||location.href).href);
  let answer='';
  let btwRow=null;
