...
function _fileViewBrowserUrl(path){
  if(!S.session||!path) return '';
  const q=new URLSearchParams({session_id:S.session.session_id,path});
  return new URL(apiV1Path('/api/file/view?'+q), location.origin).href;
}
...
function showPreview(mode){
  ...
  _previewDirty = false;
  updateEditBtn();
  // Show "Open in browser" for iframe-backed document previews
  const openBtn=$('btnOpenInBrowser');
  if(openBtn) openBtn.style.display = (mode==='html'||mode==='pdf')?'inline-flex':'none';
}

function openInBrowser(ev){
  if(ev) ev.preventDefault();
  if(!_previewCurrentPath||!S.session) return;
  void _openPreviewInBrowserTab(_previewCurrentPath);
}

function _htmlForBrowserTab(content){
  const base='<base target="_blank">';
  if(/<head(?:\s[^>]*)?>/i.test(content)){
    return content.replace(/(<head\b[^>]*>)/i,'$1'+base);
  }
  if(/<!doctype[^>]*>/i.test(content)){
    return content.replace(/(<!doctype[^>]*>)/i,'$1<head>'+base+'</head>');
  }
  return '<head>'+base+'</head>'+content;
}

function _openBlobInBrowserTab(blob){
  const objUrl=URL.createObjectURL(blob);
  let opened=null;
  try{ opened=window.open(objUrl,'_blank','noopener,noreferrer'); }catch(_e){}
  if(!opened){
...
function _openServerViewInBrowserTab(path){
  const url=_fileViewBrowserUrl(path);
  if(!url) return;
  const link=document.createElement('a');
  link.href=url;
  link.target='_blank';
...
async function _openPreviewInBrowserTab(path){
  if(!path||!S.session) return;
  const ext=fileExt(path);
  const sid=S.session.session_id;
  try{
    if(HTML_EXTS.has(ext)){
      const data=await api(`/api/file?session_id=${encodeURIComponent(sid)}&path=${encodeURIComponent(path)}`);
      const html=_htmlForBrowserTab(data.content||'');
      _openBlobInBrowserTab(new Blob([html],{type:'text/html;charset=utf-8'}));
      return;
    }
    if(PDF_EXTS.has(ext)){
      const rawUrl=new URL(apiV1Path(`/api/file/raw?session_id=${encodeURIComponent(sid)}&path=${encodeURIComponent(path)}&inline=1`),location.origin).href;
      ...
      _openBlobInBrowserTab(blob);
      return;
    }
    _openServerViewInBrowserTab(path);
  }catch(_e){
    setStatus(t('file_open_failed'));
    _openServerViewInBrowserTab(path);
  }
...
async function openFile(path){
  ...
  if(IMAGE_EXTS.has(ext)){
    ...
    showPreview('image');
    const url=apiV1Path(`api/file/raw?session_id=${encodeURIComponent(S.session.session_id)}&path=${encodeURIComponent(path)}`);
    $('previewImg').alt=path;
    $('previewImg').src=url;
    ...
    showPreview(mode);
    const url=apiV1Path(`api/file/raw?session_id=${encodeURIComponent(S.session.session_id)}&path=${encodeURIComponent(path)}&inline=1`);
...
function downloadFile(path){
  ...
  const url=apiV1Path(`api/file/raw?session_id=${encodeURIComponent(S.session.session_id)}&path=${encodeURIComponent(path)}&download=1`);
