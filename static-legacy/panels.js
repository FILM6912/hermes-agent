...
let _profileDetailTab = 'overview'; // 'overview' | 'mcp'
let _profileMcpMode = 'list'; // 'list' | 'create' | 'edit'
let _profileMcpInputMode = 'form'; // 'form' | 'json'
let _profileMcpDraft = null;
let _profileMcpServersCache = {};
let _profileMcpDiscoverSeq = 0;
...
async function switchPanel(name, opts = {}) {
  ...
  if (nextPanel === 'profiles') {
    if (typeof _fetchAuthRole === 'function') {
      const auth = await _fetchAuthRole();
      if (!_canManageProfiles(auth)) {
  ...
  if (nextPanel === 'insights') {
    if (typeof _fetchAuthRole === 'function') {
      const auth = await _fetchAuthRole();
      if (!_canAccessInsightsLogs(auth)) return switchPanel('chat', opts);
    }
    await loadInsights();
  }
  if (nextPanel === 'logs') {
...
async function loadCronProfiles(){
  ...
  try {
    if (typeof _fetchAuthRole === 'function') {
      const auth = await _fetchAuthRole();
      if (!_canManageProfiles(auth)) {
        const bound = (auth && auth.profile_name) || S.activeProfile || 'default';
        _cronProfilesCache = bound ? [{ name: bound }] : [];
...
function _kanbanColumnLabel(name){
  if (name === 'todo') return 'TO-DO';
  const label = t('kanban_status_' + name) || name;
  return String(label).toUpperCase();
}
function _kanbanPriorityLevels(){
  return [
    {value: -1, key: 'kanban_priority_low'},
    {value: 0, key: 'kanban_priority_normal'},
    {value: 1, key: 'kanban_priority_high'},
    {value: 2, key: 'kanban_priority_urgent'},
  ];
}

function _kanbanPriorityLabel(priority){
  const n = Number(priority);
  if (Number.isNaN(n)) return '';
  const hit = _kanbanPriorityLevels().find(level => level.value === n);
  if (hit) return t(hit.key) || hit.key;
  return `${t('kanban_priority') || 'Priority'} ${n}`;
}

function _kanbanPopulatePrioritySelect(selected){
  const el = document.getElementById('kanbanTaskModalPriority');
  if (!el) return;
  const levels = _kanbanPriorityLevels();
  const value = selected == null || selected === '' ? 0 : Number(selected);
  const safeValue = Number.isNaN(value) ? 0 : value;
  let html = levels.map(level => `<option value="${level.value}">${esc(t(level.key) || level.key)}</option>`).join('');
  if (!levels.some(level => level.value === safeValue)) {
    html += `<option value="${safeValue}">${esc(_kanbanPriorityLabel(safeValue))}</option>`;
  }
  el.innerHTML = html;
...
function _kanbanTaskMeta(task){
  ...
  if (task.priority !== undefined && task.priority !== null && Number(task.priority) !== 0) {
    bits.push(_kanbanPriorityLabel(task.priority));
  }
...
function _kanbanAssigneeInitials(task){
  const raw = String((task && (task.tenant || task.assignee)) || '').replace(/^@/, '').trim();
  if (!raw) return '?';
...
function _kanbanRenderColumn(col){
  ...
  return `<section class="kanban-column" data-status="${esc(col.name)}" data-kanban-status="${esc(col.name)}" ondragover="allowKanbanDrop(event)" ondragenter="event.currentTarget.classList.add('drop-target')" ondragleave="clearKanbanDrop(event)" ondrop="dropKanbanTask(event, '${esc(col.name)}')">
      <div class="kanban-column-head">
        <span class="kanban-column-title">${esc(_kanbanColumnLabel(col.name))}</span>
        <span class="kanban-count">${tasks.length}</span>
      </div>
      <div class="kanban-column-body">
        ${tasks.length ? tasks.map(task => _kanbanCard(task, col.name)).join('') : `<div class="kanban-column-empty">—</div>`}
      </div>
    </section>`;
}
...
function _kanbanCard(task, status){
  ...
  const assignee = String(task.assignee || 'default').replace(/^@/, '');
  const initials = _kanbanAssigneeInitials(task);
  return `<article class="kanban-card kanban-card-compact ${esc(stale)}" data-kanban-task-id="${esc(task.id)}" draggable="true" ondragstart="dragKanbanTask(event, '${id}')" ondragend="finishKanbanDrag(event)" onclick="return openKanbanCard(event, '${id}')" tabindex="0" role="button" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();loadKanbanTask('${id}')}">
    <div class="kanban-card-title">${esc(_kanbanTaskTitle(task))}</div>
    <div class="kanban-card-foot">
      <span class="kanban-card-avatar" aria-hidden="true">${esc(initials)}</span>
      <span class="kanban-card-assignee">@${esc(assignee)}</span>
      ${age ? `<span class="kanban-card-age">${esc(age)}</span>` : ''}
    </div>
  </article>`;
}
...
function _kanbanStartEventStream(){
  ...
  let url = apiV1Path('/api/kanban/events/stream' + _kanbanBoardQuery({since: since}));
  let es;
  try {
...
async function _kanbanLoadProfileNames(){
  ...
  try {
    if (typeof _fetchAuthRole === 'function') {
      const auth = await _fetchAuthRole();
      if (!_canManageProfiles(auth)) {
        const bound = (auth && auth.profile_name) || S.activeProfile || '';
        _kanbanProfileNamesCache = bound ? [bound] : [];
        _kanbanProfileNamesCacheAt = Date.now();
        return _kanbanProfileNamesCache;
      }
    }
    const data = await api('/api/profiles');
...
function _kanbanResetTaskModalFields(values){
  ...
  _kanbanPopulatePrioritySelect(v.priority != null ? v.priority : 0);
...
async function loadKanbanBoards(){
  // Fetches the boards list and updates the tab bar. Best-effort — failures
  // fall back to a single Default tab rather than blocking the panel.
  const switcher = document.getElementById('kanbanBoardSwitcher');
  const tabsEl = document.getElementById('kanbanBoardTabs');
  if (!switcher && !tabsEl) return;
  let data;
  try {
    data = await api('/api/kanban/boards');
  } catch(e) {
    if (switcher) switcher.hidden = true;
    _renderKanbanBoardTabs([], 'default');
    return;
  }
  ...
  if (switcher) switcher.hidden = true;
  _renderKanbanBoardTabs(boards, active);
...
function _kanbanBoardTaskTotal(board){
  if (board == null) return 0;
  if (board.total != null) return Number(board.total) || 0;
  if (board.counts && typeof board.counts === 'object') {
    return Object.values(board.counts).reduce((sum, count) => sum + Number(count || 0), 0);
  }
  return 0;
}

function _renderKanbanBoardTabs(boards, current){
  const tabsEl = document.getElementById('kanbanBoardTabs');
  if (!tabsEl) return;
  const list = (boards && boards.length) ? boards : [{slug: 'default', name: 'Default', total: 0}];
  tabsEl.innerHTML = list.map(b => {
    const slug = b.slug || 'default';
    const isActive = slug === current;
    const total = _kanbanBoardTaskTotal(b);
    const name = b.name || slug;
    const label = total > 0 ? `${name} ${total}` : name;
    return `<button type="button" class="kanban-board-tab${isActive ? ' is-active' : ''}" role="tab" aria-selected="${isActive ? 'true' : 'false'}" data-board-slug="${esc(slug)}" onclick="switchKanbanBoard('${esc(slug)}')">
      ${isActive ? '<span class="kanban-board-tab-dot" aria-hidden="true"></span>' : ''}
      <span class="kanban-board-tab-label">${esc(label)}</span>
    </button>`;
  }).join('');
}
...
async function loadLogs(animate) {
  ...
  try {
    ...
    const scope = typeof _insightsLogsScopeQuery === 'function' ? _insightsLogsScopeQuery() : '';
    const data = await api('/api/logs?file=' + encodeURIComponent(file) + '&tail=' + encodeURIComponent(tail) + scope);
    _renderLogs(data);
  } catch(e) {
    _lastLogsLines = [];
...
async function loadInsights(animate) {
  ...
  try {
    const [data, wikiStatus] = await Promise.all([
      api(`/api/insights?days=${period}${scope}`),
...
async function openMemorySection(section, el) {
  ...
  if (section === 'soul') {
    const auth = await _fetchAuthRole();
    if (!_canAccessAgentSoul(auth)) return;
  }
...
let _nestedWorkspacesEnabled = false;
...
function _workspaceEntryMatchesPath(entry, path){
  if(!entry||!path) return false;
  if(entry.path===path) return true;
  if(entry.disk_path&&entry.disk_path===path) return true;
  return false;
}
...
function getWorkspaceFriendlyName(path){
  ...
  if(_workspaceList && _workspaceList.length){
    const match=_workspaceList.find(w=>_workspaceEntryMatchesPath(w,path));
...
function _syncWorkspaceCreateButtonVisibility(){
  const btn = $('btnAddWorkspace');
  if (!btn) return;
  // Nested sub-workspaces: user and admin can create under /workspace (not profile-mgmt).
  btn.style.display = _nestedWorkspacesEnabled ? '' : 'none';
}
...
function syncWorkspaceDisplays(){
  ...
  if(sidebarPath) sidebarPath.textContent=_workspaceDisplayPath(ws);
...
async function loadWorkspaceList(){
  try{
    ...
    _nestedWorkspacesEnabled = !!data.nested_workspaces;
    _syncWorkspaceCreateButtonVisibility();
...
function renderWorkspaceDropdownInto(dd, workspaces, currentWs){
  ...
  function renderList(){
    ...
    for(const w of sorted){
      ...
      const active=_workspaceEntryMatchesPath(w,currentWs);
      opt.className='ws-opt'+(active?' active':'');
      opt.dataset.name=w.name||'';
      opt.dataset.path=w.path||'';
      const indent=depth?`padding-left:${8+depth*14}px`:'';
      opt.innerHTML=`<span class="ws-opt-name" style="${indent}">${esc(w.name)}</span><span class="ws-opt-path">${esc(w.path)}</span>`;
...
function renderWorkspacesPanel(workspaces){
  ...
  for(let i=0;i<workspaces.length;i++){
    ...
    row.draggable = !singleWorkspace;
    const isActive = _workspaceEntryMatchesPath(w, activePath);
    ...
    row.innerHTML=`
      ${singleWorkspace ? '' : `<span class="ws-drag-handle" title="${esc(t('workspace_drag_hint'))}">${li('grip-vertical',12)}</span>`}
    ...
    if (!singleWorkspace) row.addEventListener('dragstart', (e) => {
    ...
    if (!singleWorkspace) row.addEventListener('dragend', () => {
      row.classList.remove('dragging');
      panel.querySelectorAll('.ws-row.drag-over').forEach(r => r.classList.remove('drag-over'));
    });
    if (!singleWorkspace) row.addEventListener('dragover', (e) => {
      e.preventDefault();
    ...
    if (!singleWorkspace) row.addEventListener('dragleave', () => {
      row.classList.remove('drag-over');
    });
    if (!singleWorkspace) row.addEventListener('drop', async (e) => {
      e.preventDefault();
      row.classList.remove('drag-over');
  ...
  hint.textContent=_nestedWorkspacesEnabled
    ? (t('workspace_nested_hint') || t('workspace_paths_validated_hint'))
    : (t('workspace_one_per_profile') || t('workspace_paths_validated_hint'));
...
function _setWorkspaceHeaderButtons(mode, ws){
  ...
  if (mode === 'read') {
    const activePath = S.session ? S.session.workspace : '';
    const isActive = ws && _workspaceEntryMatchesPath(ws, activePath);
    const isDefault = !!(ws && ws.is_default);
    const isRoot = !!(ws && ws.path === '/workspace');
    if (isActive) hide(actBtn); else show(actBtn);
    show(editBtn);
    if (_nestedWorkspacesEnabled && ws && !isRoot) show(delBtn); else hide(delBtn);
...
function openWorkspaceCreate(){
  if(_nestedWorkspacesEnabled){
    _workspacePreFormDetail = null;
    _workspaceMode = 'create';
    _renderWorkspaceForm({ name: '', path: '/workspace', isEdit: false, nested: true });
    return;
  }
  showToast(t('workspace_add_blocked') || 'Each profile can only have one workspace.', 'info');
}
...
function _renderWorkspaceForm({ name, path, isEdit, nested }){
  const title = $('workspaceDetailTitle');
  const body = $('workspaceDetailBody');
  const empty = $('workspaceDetailEmpty');
  if (!title || !body) return;
  ...
  const pathHint = isEdit
    ...
    : `<div class="detail-form-hint">${esc(nested ? (t('workspace_nested_create_hint') || t('workspace_paths_validated_hint')) : t('workspace_paths_validated_hint'))}</div>`;
  const parentRow = nested && !isEdit ? (() => {
    const options = (_workspaceList || []).map(w => {
      const p = w.path || '/workspace';
      const label = `${w.name || p} (${p})`;
      return `<option value="${esc(p)}">${esc(label)}</option>`;
    }).join('');
    return `<div class="detail-form-row">
          <label for="workspaceFormParent">${esc(t('workspace_parent_label') || 'Parent')}</label>
          <select id="workspaceFormParent">${options}</select>
        </div>`;
  })() : '';
  const pathRow = nested && !isEdit ? `<div class="detail-form-row">
          <label for="workspaceFormPath">${esc(t('workspace_path_label') || 'Path')}</label>
          <input type="text" id="workspaceFormPath" value="/workspace" disabled>
          <div class="detail-form-hint">${esc(t('workspace_nested_path_preview') || 'Sub-workspaces are created under /workspace')}</div>
        </div>` : `<div class="detail-form-row">
          <label for="workspaceFormPath">${esc(t('workspace_path_label') || 'Path')}</label>
    ...
    <div class="main-view-content">
      <form class="detail-form" onsubmit="event.preventDefault(); saveWorkspaceForm();">
        <div class="detail-form-row">
          ...
          <input type="text" id="workspaceFormName" value="${esc(name || '')}" placeholder="${esc(nested ? (t('workspace_nested_name_placeholder') || 'project1') : (t('workspace_name_placeholder') || 'Optional friendly name'))}" autocomplete="off" ${nested && !isEdit ? 'required' : ''}>
        </div>
        ${parentRow}
        ${pathRow}
        ${nested && !isEdit ? `<div class="detail-form-hint">${pathHint}</div>` : ''}
  ...
  if (!isEdit && !nested) _wireWorkspaceFormPathSuggestions();
  const focus = isEdit ? $('workspaceFormName') : $('workspaceFormName');
...
async function saveWorkspaceForm(){
  ...
  const parentEl = $('workspaceFormParent');
  const parent = parentEl ? (parentEl.value || '').trim() : '';
  errEl.style.display = 'none';
  if (_nestedWorkspacesEnabled && _workspaceMode === 'create') {
    if (!name) { errEl.textContent = t('workspace_name_required') || 'Name is required'; errEl.style.display = ''; return; }
  } else if (!path) {
    errEl.textContent = t('workspace_path_required') || 'Path is required';
    errEl.style.display = '';
    return;
  }
  try {
    ...
    const addBody = _nestedWorkspacesEnabled
      ? { path: name, name, parent, create: true }
      : { path };
    const data = await api('/api/workspaces/add', { method:'POST', body: JSON.stringify(addBody) });
    _workspaceList = data.workspaces || [];
    _nestedWorkspacesEnabled = _nestedWorkspacesEnabled || !!data.nested_workspaces;
    _workspacePreFormDetail = null;
    const createdPath = data.path || path;
    // Apply rename if a friendly name was supplied (legacy absolute-path flow)
    if (name && !_nestedWorkspacesEnabled) {
      try { await api('/api/workspaces/rename', { method:'POST', body: JSON.stringify({ path: createdPath, name }) }); } catch(_) {}
      const refreshed = await api('/api/workspaces');
      _workspaceList = refreshed.workspaces || _workspaceList;
    }
    renderWorkspacesPanel(_workspaceList);
    showToast(t('workspace_added'));
    const added = _workspaceList.find(w => w.path === createdPath) || _workspaceList[_workspaceList.length - 1];
...
function _workspacePanelOpen(){
  return typeof _workspacePanelMode !== 'undefined' && _workspacePanelMode !== 'closed';
}

function _currentWorkspacePathForUi(){
  ...
  if (typeof S._profileDefaultWorkspace === 'string' && S._profileDefaultWorkspace) {
    return S._profileDefaultWorkspace;
  }
  return '';
}

async function _refreshWorkspaceAfterProfileSwitch(gen, opts={}){
  if (gen != null && gen !== _profileSwitchGeneration) return;

  // Drop stale tree state immediately so the panel does not keep showing the
  // previous profile's files while the new listing request is in flight.
  S.currentDir = '.';
  S._dirCache = {};
  S.entries = [];
  ...
  if (S.session && S.session.workspace && typeof loadDir === 'function') {
    ...
    const awaitTree = Object.prototype.hasOwnProperty.call(opts, 'awaitTree')
      ? opts.awaitTree
      : _workspacePanelOpen();
    if (awaitTree) await dirLoad;
  }

  if (_workspaceList && typeof renderWorkspaceDropdownInto === 'function') {
    const currentWs = _currentWorkspacePathForUi();
    const composerDd = $('composerWsDropdown');
    const sidebarDd = $('wsDropdown');
    if (composerDd && composerDd.classList.contains('open')) {
      renderWorkspaceDropdownInto(composerDd, _workspaceList, currentWs);
    }
    if (sidebarDd && sidebarDd.classList.contains('open')) {
      renderWorkspaceDropdownInto(sidebarDd, _workspaceList, currentWs);
    }
  }
}
...
function _refreshProfileSwitchBackground(gen){
  ...
  Promise.resolve(loadWorkspaceList()).then(()=>{
    ...
    void _refreshWorkspaceAfterProfileSwitch(gen, { awaitTree: false });
  }).catch(()=>{});
...
async function loadProfilesPanel() {
  ...
  try {
    ...
    const canSoul = _canAccessAgentSoul(auth);
    const syncAllBtn = $('btnSyncAllProfiles');
    if (syncAllBtn) syncAllBtn.style.display = canSoul ? '' : 'none';
    const data = await api('/api/profiles');
    _profilesCache = data;
    panel.innerHTML = '';
    const activeName = (S.activeProfile && data.profiles && data.profiles.some(p => p.name === S.activeProfile))
      ? S.activeProfile
      : (data.active || 'default');
    const explainer = document.createElement('button');
    explainer.type = 'button';
    explainer.className = 'profile-concept-help-card';
    explainer.textContent = 'Use profiles for how the agent works; use workspaces for what files it works on.';
    explainer.onclick = () => _renderProfileConceptHelp(activeName);
    ...
    for (const p of data.profiles) {
      ...
      const meta = [];
      if (p.workspace_name || p.default_workspace) meta.push(_profileWorkspaceSummary(p));
      ...
      const syncAction = (p.is_default || !canSoul) ? '' : `<div class="profile-card-actions">
          <button type="button" class="panel-head-btn profile-card-sync-btn has-tooltip has-tooltip--left" data-profile="${esc(p.name)}" data-i18n-title="profile_sync_from_default_title" title="${esc(t('profile_sync_from_default_title') || 'Update from default')}" aria-label="${esc(t('profile_sync_from_default_title') || 'Update from default')}" onclick="event.stopPropagation(); syncProfileFromDefaultBtn(this)">${_profileSyncIconSvg()}</button>
        </div>`;
      card.innerHTML = `
...
function _profileWorkspaceSummary(p){
  if(!p||!p.default_workspace) return '';
  const label = p.workspace_name || p.name || t('profile_workspace_label') || 'Workspace';
  const normalized = String(p.default_workspace).replace(/^~(?=\/)/,'');
  const parts = normalized.split('/').filter(Boolean);
  const shortPath = parts.length > 2 ? parts.slice(-2).join('/') : (parts[parts.length - 1] || normalized);
  return `${label} · ${shortPath}`;
}

function _profileSyncIconSvg(){
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>';
}

function _profileSyncItemChips(canSoul){
  const items = [
    ['profile_sync_items_config', 'Config'],
    ['profile_sync_items_mcp', 'MCP servers'],
    ['profile_sync_items_skills', 'Skills'],
  ];
  if (canSoul !== false) {
    items.push(['profile_sync_items_soul', 'Agent soul']);
  }
  return items.map(([key, fallback]) =>
    `<span class="profile-sync-item">${_profileSyncIconSvg()}${esc(t(key) || fallback)}</span>`
  ).join('');
}

function _renderProfileSyncCard(canSoul){
  if (canSoul === false) return '';
  return `<div class="detail-card profile-sync-card">
    <div class="detail-card-title">${esc(t('profile_sync_from_default_title') || 'Update from default')}</div>
    <div class="detail-row"><div class="detail-row-value" style="color:var(--muted);font-size:12px;line-height:1.55">${esc(t('profile_sync_from_default_hint') || '')}</div></div>
    <div class="profile-sync-items" aria-label="${esc(t('profile_sync_items_label') || 'Synced items')}">${_profileSyncItemChips(canSoul)}</div>
    <div class="profile-sync-actions">
      <button type="button" class="settings-action-btn primary" onclick="syncCurrentProfileFromDefault()">${_profileSyncIconSvg()} ${esc(t('profile_sync_from_default_title') || 'Update from default')}</button>
    </div>
    <div id="profileSyncResult" class="profile-sync-result" style="display:none"></div>
  </div>`;
}

function _formatProfileSyncSummary(result){
  const added = result && result.added ? result.added : {};
  const bits = [];
  const configCount = Array.isArray(added.config) ? added.config.length : 0;
  const mcpCount = Array.isArray(added.mcp_servers) ? added.mcp_servers.length : 0;
  const skillCount = Array.isArray(added.skills) ? added.skills.length : 0;
  const fileCount = Array.isArray(added.files) ? added.files.length : 0;
  if (configCount) bits.push(t('profile_sync_added_config', configCount));
  if (mcpCount) bits.push(t('profile_sync_added_mcp', mcpCount));
  if (skillCount) bits.push(t('profile_sync_added_skills', skillCount));
  if (fileCount) bits.push(t('profile_sync_added_soul', fileCount));
  if (!bits.length) return t('profile_sync_result_none');
  return t('profile_sync_result_added', bits.join(', '));
}

function _showProfileSyncResult(name, result){
  const summary = _formatProfileSyncSummary(result);
  const resultEl = $('profileSyncResult');
  if (resultEl) {
    const hasAdded = summary !== t('profile_sync_result_none');
    resultEl.className = 'profile-sync-result' + (hasAdded ? ' has-added' : '');
    resultEl.textContent = summary;
    resultEl.style.display = '';
  }
  showToast(hasAdded ? t('profile_sync_from_default_done', name) + ' — ' + summary : summary);
}
...
function _renderProfileDetail(p, activeName){
  ...
  body.innerHTML = `
    <div class="main-view-content">
      <div class="profile-detail-tabs" role="tablist" aria-label="Profile sections">
        <button type="button" class="profile-detail-tab${_profileDetailTab==='overview'?' active':''}" role="tab" aria-selected="${_profileDetailTab==='overview'}" onclick="switchProfileDetailTab('overview')">${esc(t('profile_tab_overview')||'Overview')}</button>
        <button type="button" class="profile-detail-tab${_profileDetailTab==='mcp'?' active':''}" role="tab" aria-selected="${_profileDetailTab==='mcp'}" onclick="switchProfileDetailTab('mcp')">MCP</button>
      </div>
      <div id="profileDetailTabPanel"></div>
    </div>`;
  _renderProfileDetailTabPanel(p, activeName);
  body.style.display = '';
  if (empty) empty.style.display = 'none';
  _profileMode = 'read';
...
function switchProfileDetailTab(tab){
  if(!_currentProfileDetail) return;
  _profileDetailTab = tab;
  if(tab === 'mcp') _profileMcpMode = 'list';
  _profileMcpInputMode = 'form';
  _profileMcpDraft = null;
  _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
}

function _renderProfileDetailTabPanel(p, activeName){
  const panel = $('profileDetailTabPanel');
  if(!panel) return;
  if(_profileDetailTab === 'mcp'){
    _renderProfileMcpPanel(p, panel);
    return;
  }
  const isActive = p.name === activeName;
  ...
  if (p.workspace_name || p.default_workspace) {
    rows.push(`<div class="detail-row"><div class="detail-row-label">${esc(t('profile_workspace_label') || 'Workspace')}</div><div class="detail-row-value">${esc(p.workspace_name || p.name || t('profile_workspace_label') || 'Workspace')}</div></div>`);
  }
  if (p.default_workspace) rows.push(`<div class="detail-row"><div class="detail-row-label">${esc(t('profile_workspace_path_label') || 'Path')}</div><div class="detail-row-value"><code>${esc(p.default_workspace)}</code></div></div>`);
  ...
  const canSoul = _canAccessAgentSoul(_authRoleCache);
  const syncCard = isDefault ? '' : _renderProfileSyncCard(canSoul);
  panel.innerHTML = `<div class="detail-card"><div class="detail-card-title">Profile</div>${rows.join('')}</div>${syncCard}`;
}

function _mcpProfileQuery(profileName){
  return profileName ? `?profile=${encodeURIComponent(profileName)}` : '';
}

function _profileMcpJsonExample(){
  return `{
  "opensandbox": {
...
function _serverToMcpConfig(server){
  const cfg = {};
...
function _profileMcpJsonDraftText(draft){
  if(draft && typeof draft.jsonText === 'string' && draft.jsonText.trim()) return draft.jsonText;
  const name = draft && draft.name ? draft.name : '';
  const cfg = _serverToMcpConfig(draft || {});
  if(name) return JSON.stringify({[name]: cfg}, null, 2);
  return _profileMcpJsonExample();
}

function _parseMcpJsonInput(raw){
  let text = String(raw || '').trim();
  if(!text) throw new Error(t('mcp_json_required') || 'JSON is required.');
  if(!text.startsWith('{')){
    text = '{' + text.replace(/,\s*$/, '') + '}';
  }
  let parsed;
  try{
    ...
    throw new Error((t('mcp_json_invalid') || 'Invalid JSON.') + ' ' + (err && err.message ? err.message : ''));
  }
  if(!parsed || typeof parsed !== 'object' || Array.isArray(parsed)){
    throw new Error(t('mcp_json_object_required') || 'JSON must be an object.');
  }
  const servers = {};
  for(const [name, cfg] of Object.entries(parsed)){
    ...
    if(!cfg || typeof cfg !== 'object' || Array.isArray(cfg)){
      throw new Error((t('mcp_json_entry_invalid') || 'Invalid config for server') + ' "' + key + '"');
    }
    if(!cfg.url && !cfg.command){
      throw new Error((t('mcp_json_entry_missing_transport') || 'Each server needs url or command') + ': "' + key + '"');
    }
    servers[key] = cfg;
  }
  if(!Object.keys(servers).length){
    throw new Error(t('mcp_json_no_servers') || 'No MCP server entries found in JSON.');
  }
  return servers;
}

function switchProfileMcpInputMode(mode){
  _profileMcpInputMode = mode === 'json' ? 'json' : 'form';
  if((_profileMcpMode === 'create' || _profileMcpMode === 'edit') && _currentProfileDetail){
    if(_profileMcpInputMode === 'json'){
      _profileMcpDraft = _profileMcpDraft || {};
      _profileMcpDraft.jsonText = _profileMcpJsonDraftText(_profileMcpDraft);
    }else{
      _syncProfileMcpDraftFromJson();
    }
    _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
  }
}

function _syncProfileMcpDraftFromJson(){
  const jsonEl = $('profileMcpJson');
  if(!jsonEl) return;
  try{
    const servers = _parseMcpJsonInput(jsonEl.value);
    const names = Object.keys(servers);
    if(names.length === 1){
      const name = names[0];
      const cfg = servers[name];
      _profileMcpDraft = Object.assign({}, _profileMcpDraft || {}, {
        name,
        transport: cfg.url ? 'http' : 'stdio',
        command: cfg.command || '',
        args: Array.isArray(cfg.args) ? cfg.args.join(', ') : '',
        url: cfg.url || '',
        timeout: cfg.timeout != null ? cfg.timeout : 120,
        enabled: cfg.enabled !== false,
        jsonText: jsonEl.value,
      });
    }
  }catch(_e){}
}

function _renderProfileMcpPanel(p, panel){
  const profileName = p.name;
  if(_profileMcpMode === 'create' || _profileMcpMode === 'edit'){
    const draft = _profileMcpDraft || {};
    const transport = draft.transport || 'stdio';
    const isEdit = _profileMcpMode === 'edit';
    const jsonMode = _profileMcpInputMode === 'json';
    panel.innerHTML = `
      <div class="detail-card">
        <div class="detail-card-title">${esc(isEdit ? (t('mcp_edit_server')||'Edit MCP Server') : (t('mcp_add_server')||'+ Add Server'))}</div>
        <div class="mcp-input-mode-tabs" role="tablist" aria-label="MCP input mode">
          <button type="button" class="mcp-input-mode-tab${!jsonMode?' active':''}" role="tab" aria-selected="${!jsonMode}" onclick="switchProfileMcpInputMode('form')">${esc(t('mcp_input_form')||'Form')}</button>
          <button type="button" class="mcp-input-mode-tab${jsonMode?' active':''}" role="tab" aria-selected="${jsonMode}" onclick="switchProfileMcpInputMode('json')">JSON</button>
        </div>
        ${jsonMode ? `
        <div class="mcp-form-grid">
          <label>${esc(t('mcp_json_label')||'MCP server JSON')}
            <span class="mcp-json-hint">${esc(t('mcp_json_hint')||'Paste one or more servers keyed by name, as in config.yaml mcp_servers.')}</span>
            <textarea id="profileMcpJson" class="mcp-json-input" spellcheck="false" autocomplete="off">${esc(_profileMcpJsonDraftText(draft))}</textarea>
          </label>
        </div>` : `
        <div class="mcp-form-grid">
          <label>${esc(t('mcp_field_name')||'Server Name')}
            <input id="profileMcpName" type="text" value="${esc(draft.name||'')}" ${isEdit?'readonly':''} autocomplete="off">
          </label>
          <label>${esc(t('mcp_transport_label')||'Transport Type')}
            <select id="profileMcpTransport" onchange="profileMcpTransportChanged()">
              <option value="stdio"${transport==='stdio'?' selected':''}>stdio</option>
              <option value="http"${transport==='http'?' selected':''}>HTTP</option>
            </select>
          </label>
          <label id="profileMcpCommandWrap"${transport==='http'?' hidden':''}>${esc(t('mcp_field_command')||'Command')}
            <input id="profileMcpCommand" type="text" value="${esc(draft.command||'')}" autocomplete="off">
          </label>
          <label id="profileMcpArgsWrap"${transport==='http'?' hidden':''}>${esc(t('mcp_field_args')||'Arguments (comma-separated)')}
            <input id="profileMcpArgs" type="text" value="${esc(draft.args||'')}" autocomplete="off">
          </label>
          <label id="profileMcpUrlWrap"${transport!=='http'?' hidden':''}>${esc(t('mcp_field_url')||'URL')}
            <input id="profileMcpUrl" type="text" value="${esc(draft.url||'')}" autocomplete="off">
          </label>
          <label>${esc(t('mcp_field_timeout')||'Timeout (seconds)')}
            <input id="profileMcpTimeout" type="number" min="1" value="${esc(String(draft.timeout||120))}">
          </label>
          <label class="mcp-enabled-row"><input id="profileMcpEnabled" type="checkbox"${draft.enabled!==false?' checked':''}> ${esc(t('mcp_enabled_yes')||'Enabled')}</label>
        </div>`}
        <div class="mcp-form-actions">
          <button type="button" class="btn" onclick="cancelProfileMcpForm()">${esc(t('mcp_cancel')||'Cancel')}</button>
          <button type="button" class="btn primary" onclick="saveProfileMcpForm('${esc(profileName)}')">${esc(t('mcp_save')||'Save')}</button>
        </div>
      </div>`;
    return;
  }
  panel.innerHTML = `
    <div class="detail-card">
      <div class="profile-mcp-head">
        <div>
          <div class="detail-card-title">${esc(t('mcp_servers_title')||'MCP Servers')}</div>
          <div class="profile-mcp-desc">${esc(t('profile_mcp_desc')||'MCP servers stored in this profile config.yaml.')}</div>
        </div>
        <button type="button" class="btn primary" onclick="openProfileMcpCreate('${esc(profileName)}')">${esc(t('mcp_add_server')||'+ Add Server')}</button>
      </div>
      <div id="profileMcpServerList"></div>
      <div id="profileMcpDiscoverHint" class="mcp-restart-hint">${esc(t('profile_mcp_reload_hint')||'MCP servers connect automatically when you open this tab or save changes.')}</div>
    </div>`;
  loadProfileMcpServers(profileName);
}

function profileMcpTransportChanged(){
  const transport = ($('profileMcpTransport')||{}).value || 'stdio';
  const showStdio = transport !== 'http';
  const cmd = $('profileMcpCommandWrap');
  const args = $('profileMcpArgsWrap');
  const url = $('profileMcpUrlWrap');
  if(cmd) cmd.hidden = !showStdio;
  if(args) args.hidden = !showStdio;
  if(url) url.hidden = showStdio;
}

function openProfileMcpCreate(profileName){
  _profileMcpMode = 'create';
  _profileMcpInputMode = 'form';
  _profileMcpDraft = {transport:'stdio', enabled:true, timeout:120};
  if(_currentProfileDetail && _currentProfileDetail.name === profileName){
    _profileDetailTab = 'mcp';
    _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
  }
}

function openProfileMcpEdit(profileName, server){
  _profileMcpMode = 'edit';
  _profileMcpInputMode = 'form';
  _profileMcpDraft = {
    name: server.name,
    ...
    jsonText: JSON.stringify({[server.name]: _serverToMcpConfig(server)}, null, 2),
  };
  if(_currentProfileDetail && _currentProfileDetail.name === profileName){
    _profileDetailTab = 'mcp';
    _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
...
function cancelProfileMcpForm(){
  _profileMcpMode = 'list';
  _profileMcpInputMode = 'form';
  _profileMcpDraft = null;
  if(_currentProfileDetail) _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
}

function _saveProfileMcpJson(profileName){
  const jsonEl = $('profileMcpJson');
  let servers;
  try{
    servers = _parseMcpJsonInput(jsonEl && jsonEl.value);
  }catch(err){
    showToast(String(err && err.message ? err.message : (t('mcp_json_invalid')||'Invalid JSON.')), 'error');
    return;
  }
  const names = Object.keys(servers);
  if(_profileMcpMode === 'edit' && _profileMcpDraft && _profileMcpDraft.name){
    if(names.length !== 1 || names[0] !== _profileMcpDraft.name){
      showToast(t('mcp_json_edit_single')||'Edit JSON must contain only the current server name.', 'error');
      return;
    }
  }
  api('/api/mcp/servers/import'+_mcpProfileQuery(profileName), {
    method:'POST',
    body:JSON.stringify({profile:profileName, servers}),
  }).then(r=>{
    if(r&&r.ok){
      showToast(t('mcp_saved')||'MCP server saved.');
      _profileMcpMode = 'list';
      _profileMcpInputMode = 'form';
      _profileMcpDraft = null;
      if(_currentProfileDetail) _renderProfileDetail(_currentProfileDetail, (_profilesCache&&_profilesCache.active)||'default');
    }else{
      showToast((r&&r.error)||(t('mcp_save_failed')||'Failed to save MCP server.'), 'error');
    }
  }).catch(()=>showToast(t('mcp_save_failed')||'Failed to save MCP server.','error'));
}

function saveProfileMcpForm(profileName){
  if(_profileMcpInputMode === 'json'){
    _saveProfileMcpJson(profileName);
    return;
  }
  const nameEl = $('profileMcpName');
  const transport = ($('profileMcpTransport')||{}).value || 'stdio';
  const name = (nameEl&&nameEl.value||'').trim();
  if(!name){ showToast(t('mcp_name_required')||'Server name is required.','error'); return; }
  const payload = {
    profile: profileName,
    timeout: Number(($('profileMcpTimeout')||{}).value || 120),
    enabled: !!($('profileMcpEnabled')&&$('profileMcpEnabled').checked),
  };
  if(transport === 'http'){
    const url = (($('profileMcpUrl')||{}).value||'').trim();
    if(!url){ showToast(t('mcp_url_required')||'URL is required for HTTP transport.','error'); return; }
    payload.url = url;
  }else{
    const command = (($('profileMcpCommand')||{}).value||'').trim();
    if(!command){ showToast(t('mcp_command_required')||'Command is required for stdio transport.','error'); return; }
    payload.command = command;
    const argsRaw = (($('profileMcpArgs')||{}).value||'').trim();
    if(argsRaw) payload.args = argsRaw.split(',').map(s=>s.trim()).filter(Boolean);
  }
  api('/api/mcp/servers/'+encodeURIComponent(name)+_mcpProfileQuery(profileName), {
    method:'PUT',
    body:JSON.stringify(payload),
  }).then(r=>{
    if(r&&r.ok){
      ...
      showToast(t('mcp_save_failed')||'Failed to save MCP server.','error');
    }
  }).catch(()=>showToast(t('mcp_save_failed')||'Failed to save MCP server.','error'));
}

async function deleteProfileMcpServer(profileName, serverName){
  const ok = await showConfirmDialog({
    title: t('mcp_delete_confirm_title')||'Delete MCP Server',
    message: typeof t('mcp_delete_confirm_message')==='function' ? t('mcp_delete_confirm_message', serverName) : `Delete MCP server "${serverName}"?`,
    confirmLabel: t('delete_title')||'Delete',
    danger: true,
    focusCancel: true,
  });
  if(!ok) return;
  api('/api/mcp/servers/'+encodeURIComponent(serverName)+_mcpProfileQuery(profileName), {
    method:'DELETE',
    body:JSON.stringify({profile:profileName}),
  }).then(r=>{
    if(r&&r.ok){
      showToast(t('mcp_deleted')||'MCP server deleted.');
      loadProfileMcpServers(profileName);
    }else{
      showToast(t('mcp_delete_failed')||'Failed to delete MCP server.','error');
    }
  }).catch(()=>showToast(t('mcp_delete_failed')||'Failed to delete MCP server.','error'));
}

function toggleProfileMcpServer(profileName, serverName, enabled){
  api('/api/mcp/servers/'+encodeURIComponent(serverName)+_mcpProfileQuery(profileName), {
    method:'PATCH',
    body:JSON.stringify({enabled:enabled, profile:profileName}),
  }).then(r=>{
    if(r&&r.ok) showToast(t(enabled?'mcp_enabled_toast':'mcp_disabled_toast', serverName));
    else showToast(t('mcp_toggle_failed')||'Failed to update MCP server.','error');
    loadProfileMcpServers(profileName);
  }).catch(()=>{showToast(t('mcp_toggle_failed')||'Failed to update MCP server.','error'); loadProfileMcpServers(profileName);});
}

function _renderProfileMcpServerList(profileName, r){
  const list = $('profileMcpServerList');
  if(!list) return;
  if(!r||!Array.isArray(r.servers)){
    list.innerHTML = `<div class="mcp-error-state" style="color:#ef4444;font-size:12px;padding:6px 0">${esc(t('mcp_load_failed')||'Failed to load MCP servers.')}</div>`;
    return;
  }
  if(!r.servers.length){
    list.innerHTML = `<div class="mcp-empty-state" style="color:var(--muted);font-size:12px;padding:6px 0">${esc(t('mcp_no_servers')||'No MCP servers configured.')}</div>`;
    return;
  }
  list.innerHTML = r.servers.map(s=>_renderEditableMcpServerRow(s, profileName, !!r.toggle_supported)).join('');
  _profileMcpServersCache[profileName] = r.servers;
}

function _setProfileMcpDiscoverHint(discovering){
  const hint = $('profileMcpDiscoverHint');
  if(!hint) return;
  hint.textContent = discovering
    ? (t('mcp_updating') || 'Updating connection status…')
    : (t('profile_mcp_reload_hint') || 'MCP servers connect automatically when you open this tab or save changes.');
}

function loadProfileMcpServers(profileName){
  const list = $('profileMcpServerList');
  if(!list) return;
  const query = _mcpProfileQuery(profileName);
  const seq = ++_profileMcpDiscoverSeq;
  list.innerHTML = `<div style="color:var(--muted);font-size:12px;padding:6px 0">${esc(t('loading'))}</div>`;
  api('/api/mcp/servers'+query).then(r=>{
    if(seq !== _profileMcpDiscoverSeq) return;
    _renderProfileMcpServerList(profileName, r);
    _setProfileMcpDiscoverHint(true);
    return api('/api/mcp/discover'+query, {
      method:'POST',
      body:JSON.stringify({profile:profileName}),
    });
  }).then(r=>{
    if(!r || seq !== _profileMcpDiscoverSeq) return;
    _renderProfileMcpServerList(profileName, r);
    if(r.errors && Object.keys(r.errors).length){
      const failed = Object.keys(r.errors).join(', ');
      showToast((t('mcp_connect_failed_summary')||'Some MCP servers failed to connect') + ': ' + failed, 'error');
    }
  }).catch(()=>{
    if(seq !== _profileMcpDiscoverSeq) return;
    if(!_profileMcpServersCache[profileName]){
      list.innerHTML = `<div class="mcp-error-state" style="color:#ef4444;font-size:12px;padding:6px 0">${esc(t('mcp_load_failed')||'Failed to load MCP servers.')}</div>`;
    }
  }).finally(()=>{
    if(seq === _profileMcpDiscoverSeq) _setProfileMcpDiscoverHint(false);
  });
}

function openProfileMcpEditFromBtn(btn){
  const profileName = btn && btn.dataset ? btn.dataset.profile : '';
  const serverName = btn && btn.dataset ? decodeURIComponent(btn.dataset.server || '') : '';
  const server = (_profileMcpServersCache[profileName] || []).find(item => item.name === serverName);
  if(profileName && server) openProfileMcpEdit(profileName, server);
}

function _mcpToolCountDisplay(s){
  if(s && s.connect_error) return '—';
  if(!s || s.tool_count===null || typeof s.tool_count==='undefined') return '—';
  return String(s.tool_count);
}

function _mcpServerErrorHtml(s){
  if(!s || !s.connect_error) return '';
  return `<div class="mcp-server-error" role="alert">${esc(s.connect_error)}</div>`;
}

function _renderEditableMcpServerRow(s, profileName, toggleSupported){
  ...
  const toolCount = _mcpToolCountDisplay(s);
  ...
  const toggleBtn = toggleSupported
    ? `<button type="button" class="mcp-toggle-btn ${isEnabled?'mcp-toggle-enabled':'mcp-toggle-disabled'}" onclick="toggleProfileMcpServer('${esc(profileName)}','${encodedName}',${!isEnabled})">${esc(t(isEnabled?'mcp_enabled_yes':'mcp_enabled_no'))}</button>`
    : `<span>${esc(t(isEnabled?'mcp_enabled_yes':'mcp_enabled_no'))}</span>`;
  return `<div class="mcp-server-row${s.connect_error?' mcp-server-row-error':''}">
    <div class="mcp-server-row-head">
      ...
      <span class="mcp-status-badge mcp-status-${esc(status)}">${esc(_mcpStatusLabel(status))}</span>
      <div class="mcp-server-actions">
        <button type="button" class="mcp-action-btn" data-profile="${esc(profileName)}" data-server="${encodedName}" onclick="openProfileMcpEditFromBtn(this)">${esc(t('edit')||'Edit')}</button>
        <button type="button" class="mcp-action-btn danger" onclick="deleteProfileMcpServer('${esc(profileName)}','${encodedName}')">${esc(t('delete_title')||'Delete')}</button>
      </div>
    </div>
    <div class="mcp-server-detail">${esc(detail)}</div>
    ${_mcpServerErrorHtml(s)}
...
function _setProfileHeaderButtons(mode, p, activeName){
  ...
  const syncBtn = $('btnSyncProfileDetail');
  ...
  if (mode === 'read') {
    ...
    if (isActive) hide(actBtn); else show(actBtn);
    if (isDefault || !canSoul) { hide(syncBtn); if (isDefault) hide(delBtn); else show(delBtn); }
    else { show(syncBtn); show(delBtn); }
    hide(cancelBtn); hide(saveBtn);
  } else if (mode === 'create') {
    hide(actBtn); hide(syncBtn); hide(delBtn); show(cancelBtn); show(saveBtn);
  } else {
    [actBtn, syncBtn, delBtn, cancelBtn, saveBtn].forEach(hide);
  }
}
...
async function syncProfileFromDefaultBtn(btn){
  const name = btn && btn.dataset ? btn.dataset.profile : '';
  if (!name) return;
  await syncProfileFromDefault(name);
}

async function syncProfileFromDefault(name, options){
  options = options || {};
  if (!name) return null;
  if (name === 'default' || (_profilesCache && _profilesCache.profiles && _profilesCache.profiles.some(p => p.name === name && p.is_default))) {
    return null;
  }
  if (!options.skipConfirm) {
    const _ok = await showConfirmDialog({
      title: t('profile_sync_from_default_confirm_title', name),
      message: t('profile_sync_from_default_confirm_message'),
      confirmLabel: t('profile_sync_from_default_title') || 'Update',
      focusCancel: true,
    });
    if (!_ok) return null;
  }
  try {
    const result = await api('/api/profile/sync-from-default', { method: 'POST', body: JSON.stringify({ name }) });
    await loadProfilesPanel();
    if (!options.skipReopen) openProfileDetail(name);
    _showProfileSyncResult(name, result);
    return result;
  } catch (e) {
    showToast(t('profile_sync_failed') + e.message);
    return null;
  }
}

async function syncCurrentProfileFromDefault(){
  if (!_currentProfileDetail || _currentProfileDetail.is_default) return;
  await syncProfileFromDefault(_currentProfileDetail.name);
}

async function syncAllProfilesFromDefault(){
  const _ok = await showConfirmDialog({
    title: t('profile_sync_all_from_default_confirm_title'),
    message: t('profile_sync_all_from_default_confirm_message'),
    confirmLabel: t('profile_sync_all_from_default_title') || 'Update all',
    focusCancel: true,
  });
  if (!_ok) return;
  try {
    const result = await api('/api/profile/sync-from-default', { method: 'POST', body: JSON.stringify({}) });
    const count = Array.isArray(result.profiles) ? result.profiles.filter(p => p && p.ok !== false).length : 0;
    await loadProfilesPanel();
    if (_currentProfileDetail && _currentProfileDetail.name) openProfileDetail(_currentProfileDetail.name);
    showToast(t('profile_sync_all_from_default_done', count));
  } catch (e) {
    showToast(t('profile_sync_failed') + e.message);
  }
}

function _parseAuthRolePayload(payload) {
  const legacyAdmin = { role: 'admin', username: null, profile_name: null, multi_user: false };
  if (!payload || typeof payload !== 'object') return { ...legacyAdmin };
  const user = (payload.user && typeof payload.user === 'object') ? payload.user : null;
  const role = payload.role || (user && user.role) || legacyAdmin.role;
  const profileName = payload.profile_name ?? (user && user.profile_name) ?? null;
  const username = payload.username ?? (user && user.username) ?? null;
  return {
    role: String(role || legacyAdmin.role),
...
function _canSwitchProfiles(auth) {
  if (!auth || !auth.multi_user) return true;
  return auth.role === 'admin';
}

function _canManageProfiles(auth) {
  if (!auth || !auth.multi_user) return true;
  return auth.role === 'admin';
}

function _canAccessInsightsLogs(auth) {
  if (!auth || !auth.multi_user) return true;
  return auth.role === 'admin';
}

function _syncInsightsLogsScopeSelects(source) {
  const primary = $('insightsLogsScope');
  const logs = $('insightsLogsScopeLogs');
  if (!source) return;
  const value = source.value || '';
  if (primary && primary !== source) primary.value = value;
  if (logs && logs !== source) logs.value = value;
  if (_currentPanel === 'insights' && typeof loadInsights === 'function') loadInsights();
  if (_currentPanel === 'logs' && typeof loadLogs === 'function') loadLogs(true);
}

function _insightsLogsScopeQuery() {
  const el = $('insightsLogsScope') || $('insightsLogsScopeLogs');
  if (!el || !el.value) return '';
  const value = String(el.value).trim();
  if (!value) return '';
  if (value.startsWith('user:')) {
    return '&username=' + encodeURIComponent(value.slice(5));
  }
  return '&profile=' + encodeURIComponent(value);
}

function _applyInsightsLogsPanelRestrictions(auth) {
  const canAccess = _canAccessInsightsLogs(auth);
  document.querySelectorAll('[data-panel="insights"],[data-panel="logs"]').forEach(el => {
    el.classList.toggle('nav-tab-hidden', !canAccess);
    if (!canAccess) el.style.display = 'none';
    else el.style.removeProperty('display');
  });
  const panelInsights = $('panelInsights');
  const panelLogs = $('panelLogs');
  const mainInsights = $('mainInsights');
  const mainLogs = $('mainLogs');
  if (panelInsights) panelInsights.style.display = canAccess ? '' : 'none';
  if (panelLogs) panelLogs.style.display = canAccess ? '' : 'none';
  if (mainInsights) mainInsights.style.display = canAccess ? '' : 'none';
  if (mainLogs) mainLogs.style.display = canAccess ? '' : 'none';
  const showScope = !!(canAccess && auth && auth.multi_user);
  const scopeRow = $('insightsLogsScopeRow');
  const scopeRowLogs = $('insightsLogsScopeRowLogs');
  if (scopeRow) scopeRow.style.display = showScope ? '' : 'none';
  if (scopeRowLogs) scopeRowLogs.style.display = showScope ? '' : 'none';
  if (!canAccess && (_currentPanel === 'insights' || _currentPanel === 'logs') && typeof switchPanel === 'function') {
    switchPanel('chat');
  }
}

function _fillInsightsLogsScopeSelect(select) {
  if (!select) return;
  const previous = select.value || '';
  while (select.firstChild) select.removeChild(select.firstChild);
  const allOpt = document.createElement('option');
  allOpt.value = '';
  allOpt.textContent = t('insights_logs_scope_all');
  select.appendChild(allOpt);
  return previous;
}

async function _populateInsightsLogsScopeFilter() {
  const select = $('insightsLogsScope');
  const selectLogs = $('insightsLogsScopeLogs');
  if (!select && !selectLogs) return;
  const auth = _authRoleCache || (typeof _fetchAuthRole === 'function' ? await _fetchAuthRole() : null);
  if (!_canAccessInsightsLogs(auth) || !auth || !auth.multi_user) return;
  const previous = _fillInsightsLogsScopeSelect(select) || _fillInsightsLogsScopeSelect(selectLogs) || '';
  const userOptions = [];
  try {
    const data = await _usersAdminJson('/api/admin/users');
    const users = Array.isArray(data.users) ? data.users : [];
    users
      .filter(u => u && u.role === 'user' && u.profile_name)
      .sort((a, b) => String(a.username).localeCompare(String(b.username)))
      .forEach(u => {
        userOptions.push({ value: 'user:' + u.username, label: `${u.username} (${u.profile_name})` });
      });
  } catch (_e) {
    /* admin users list unavailable — keep combined view only */
  }
  for (const target of [select, selectLogs]) {
    if (!target) continue;
...
function _applyProfileSwitcherRestrictions(auth) {
  const chip = $('profileChip');
  const chevron = chip && chip.querySelector('.composer-profile-chevron');
  const label = $('profileChipLabel');
  if (!chip) return;
  const canSwitch = _canSwitchProfiles(auth);
  if (canSwitch) {
    chip.classList.remove('is-readonly');
    chip.disabled = false;
    chip.removeAttribute('aria-disabled');
    chip.onclick = toggleProfileDropdown;
    chip.title = t('profile_switch_title') || 'Switch profile';
    if (chevron) chevron.style.display = '';
    return;
  }
  const profileName = (auth && auth.profile_name) || S.activeProfile || 'default';
  chip.classList.add('is-readonly');
  chip.disabled = true;
  chip.setAttribute('aria-disabled', 'true');
  chip.onclick = null;
  chip.title = profileName;
...
function renderProfileDropdown(data) {
  ...
  if (!dd) return;
  dd.innerHTML = '';
  const auth = _authRoleCache;
  const canSwitch = !auth || _canSwitchProfiles(auth);
  let profiles = data.profiles || [];
  if (!canSwitch && auth && auth.profile_name) {
    profiles = profiles.filter(p => p && p.name === auth.profile_name);
  }
...
async function toggleProfileDropdown() {
  if (typeof _fetchAuthRole === 'function') {
    const auth = await _fetchAuthRole();
    if (!_canSwitchProfiles(auth)) return;
  }
  const dd = $('profileDropdown');
  if (!dd) return;
...
async function switchToProfile(name) {
  if (typeof _fetchAuthRole === 'function') {
    const auth = await _fetchAuthRole();
    if (!_canSwitchProfiles(auth)) {
      const bound = auth && auth.profile_name;
  ...
  try {
    ...
    if (data.default_workspace) {
      ...
      if (S.session && !sessionInProgress) {
        ...
        try {
          const upd = await api('/api/session/update', { method: 'POST', body: JSON.stringify({
          ...
          if (upd && upd.session) {
            S.session.workspace = upd.session.workspace || data.default_workspace;
            if (upd.session.model) S.session.model = upd.session.model;
            if (Object.prototype.hasOwnProperty.call(upd.session, 'model_provider')) {
              S.session.model_provider = upd.session.model_provider || null;
            }
          } else {
    ...
    if (sessionInProgress) {
      ...
      syncTopbar();
      await renderSessionList();
      await _refreshWorkspaceAfterProfileSwitch(_switchGen);
...
function openProfileCreate(){
  if (typeof _fetchAuthRole === 'function') {
    _fetchAuthRole().then(auth => {
      if (!_canManageProfiles(auth)) return;
      _openProfileCreateInner();
    });
    return;
  }
  _openProfileCreateInner();
}
function _openProfileCreateInner(){
  ...
  body.innerHTML = `
    <div class="main-view-content">
      <form class="detail-form" onsubmit="event.preventDefault(); saveProfileForm();">
        ...
        <div class="detail-form-row">
          <div class="detail-form-hint">${esc(t('profile_clone_label') || 'Settings, skills, and config are copied from the default profile. Adjust them after creation from the profile detail view.')}</div>
        </div>
...
async function saveProfileForm(){
  ...
  try {
    await api('/api/profile/create', { method: 'POST', body: JSON.stringify({ name }) });
    _invalidateKanbanProfileCache();
    _profilePreFormDetail = null;
    await loadProfilesPanel();
...
async function loadMemory(force) {
  ...
  try {
    ...
    if (_currentMemorySection === 'soul' && !canSoul) {
      _currentMemorySection = null;
    }
    ...
    if (panel) {
      ...
      for (const s of MEMORY_SECTIONS) {
        if (s.key === 'soul' && !canSoul) continue;
...
async function loadSettingsPanel(){
  try{
    ...
    try{
      ...
      _syncAuthSessionControls(authStatus);
    ...
    await _applyAdminUsersSectionVisibility();
...
// ── Admin users panel (settings → Users) ───────────────────────────────────

let _authRoleCache = null;
let _usersCache = [];
let _usersPanelMode = 'list';
let _currentUserDetail = null;

async function _fetchAuthRole(){
  if(_authRoleCache) return _authRoleCache;
  try{
    const session=await api('/api/auth/session');
    _authRoleCache=_parseAuthRolePayload(session);
    return _authRoleCache;
  }catch(_sessionErr){}
  try{
    const status=await api('/api/auth/status');
    _authRoleCache=_parseAuthRolePayload(status);
    return _authRoleCache;
  }catch(_statusErr){
    _authRoleCache=_parseAuthRolePayload(null);
    return _authRoleCache;
  }
}

function _isAdminAuth(auth){
  return !!(auth&&auth.role==='admin');
}

function _canAccessAgentSoul(auth){
  if(!auth||!auth.multi_user) return true;
  return _isAdminAuth(auth);
}

function _applyProfilesPanelRestrictions(auth){
  const canManage=_canManageProfiles(auth);
  document.querySelectorAll('[data-panel="profiles"]').forEach(el=>{
    el.classList.toggle('nav-tab-hidden',!canManage);
    if(!canManage) el.style.display='none';
    else el.style.removeProperty('display');
  });
  const panelProfiles=$('panelProfiles');
  const mainProfiles=$('mainProfiles');
  if(panelProfiles) panelProfiles.style.display=canManage?'':'none';
  if(mainProfiles) mainProfiles.style.display=canManage?'':'none';
  if(!canManage&&_currentPanel==='profiles'&&typeof switchPanel==='function'){
    switchPanel('chat');
  }
}

async function _applyAdminUsersSectionVisibility(){
  const auth=await _fetchAuthRole();
  const isAdmin=_isAdminAuth(auth);
  const menu=$('settingsMenuUsers');
  const createBtn=$('btnUsersCreate');
  const pane=$('settingsPaneUsers');
  if(menu) menu.style.display=isAdmin?'':'none';
  if(createBtn) createBtn.style.display=isAdmin?'':'none';
  if(!isAdmin){
    if(pane) pane.classList.remove('active');
    if(_settingsSection==='users'){
...
function _usersAdminApi(path,opts){
  const normalized=apiV1Path(path.startsWith('/')?path:'/api/'+path);
  const rel=normalized.startsWith('/')?normalized.slice(1):normalized;
  const url=new URL(rel,document.baseURI||location.href);
  return fetch(url.href,{credentials:'include',...opts});
}

async function _usersAdminJson(path,opts){
  const res=await _usersAdminApi(path,opts);
  let data=null;
  try{data=await res.json();}catch(_e){data=null;}
  if(!res.ok){
    const msg=(data&&data.error)||(data&&data.detail)||res.statusText||'Request failed';
...
async function loadUsersPanel(){
  const body=$('usersPanelBody');
  if(!body) return;
  const isAdmin=await _applyAdminUsersSectionVisibility();
  if(!isAdmin){
    body.innerHTML='';
    return;
  }
  if(_usersPanelMode==='create'){
    _renderUserCreateForm(body);
    return;
  }
  if(_usersPanelMode==='detail'&&_currentUserDetail){
    _renderUserDetail(body,_currentUserDetail);
    return;
  }
  body.innerHTML='<div style="color:var(--muted);font-size:13px;padding:12px 0">'+esc(t('loading')||'Loading…')+'</div>';
  try{
    const data=await _usersAdminJson('/api/admin/users');
    _usersCache=Array.isArray(data.users)?data.users:(Array.isArray(data)?data:[]);
    _usersPanelMode='list';
    _renderUsersList(body);
  }catch(e){
    body.innerHTML='<div style="color:var(--error);font-size:13px;padding:12px 0">'+esc(e.message||String(e))+'</div>';
  }
}
...
function _renderUsersList(container){
  ...
  if(!users.length){
    container.innerHTML='<div style="text-align:center;padding:32px 0;color:var(--muted);font-size:13px">No users yet. Create one to enable multi-user access.</div>';
    return;
  }
  const rows=users.map(u=>{
    const username=esc((u&&u.username)||'');
    const role=esc((u&&u.role)||'user');
    const profile=esc((u&&u.profile_name)||'—');
    return `<tr class="users-table-row" data-username="${username}">
      <td><button type="button" class="users-table-link" onclick="openUserDetail('${username}')">${username}</button></td>
      <td><span class="detail-badge">${role}</span></td>
      <td>${profile}</td>
      <td class="users-table-actions"><button type="button" class="sm-btn" onclick="openUserDetail('${username}')">View</button></td>
    </tr>`;
  }).join('');
  container.innerHTML=`
    <div class="users-table-wrap">
      <table class="users-table">
        <thead><tr><th>Username</th><th>Role</th><th>Profile</th><th></th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
}
...
async function openUserDetail(username){
  ...
  body.innerHTML='<div style="color:var(--muted);font-size:13px;padding:12px 0">'+esc(t('loading')||'Loading…')+'</div>';
  try{
    const detail=await _usersAdminJson('/api/admin/users/'+encodeURIComponent(username));
    _currentUserDetail=detail.user||detail;
    _usersPanelMode='detail';
    _renderUserDetail(body,_currentUserDetail);
  }catch(e){
    body.innerHTML='<div style="color:var(--error);font-size:13px;padding:12px 0">'+esc(e.message||String(e))+'</div>';
  }
...
function _renderUserDetail(container,user){
  const username=esc((user&&user.username)||'');
  const role=(user&&user.role)||'user';
  const profileName=(user&&user.profile_name)||'';
  const sessionCount=user&&typeof user.session_count==='number'?user.session_count:null;
  const created=user&&user.created_at?esc(String(user.created_at)):'—';
  const sessionLine=sessionCount==null?'':`<div class="detail-row"><div class="detail-row-label">Sessions</div><div class="detail-row-value">${sessionCount}</div></div>`;
  container.innerHTML=`
    <div class="users-panel-toolbar">
      <button type="button" class="sm-btn" onclick="backToUsersList()">← Back</button>
      <span style="flex:1"></span>
      <button type="button" class="sm-btn" style="color:var(--error,#e74c3c);border-color:color-mix(in srgb,var(--error,#e74c3c) 35%,transparent)" onclick="deleteAdminUser('${username}')">Delete</button>
    </div>
    <div class="detail-card" style="margin-top:12px">
      <div class="detail-card-title">User</div>
      <div class="detail-row"><div class="detail-row-label">Username</div><div class="detail-row-value"><code>${username}</code></div></div>
      <div class="detail-row"><div class="detail-row-label">Created</div><div class="detail-row-value">${created}</div></div>
      ${sessionLine}
    </div>
    <form class="detail-form" style="margin-top:12px" onsubmit="event.preventDefault(); saveAdminUserDetail();">
      <div class="detail-form-row">
        <label for="userDetailRole">Role</label>
        <select id="userDetailRole">
          <option value="user"${role==='user'?' selected':''}>user</option>
          <option value="admin"${role==='admin'?' selected':''}>admin</option>
        </select>
      </div>
      <div class="detail-form-row">
        <label for="userDetailProfile">Profile</label>
        <select id="userDetailProfile"><option value="">—</option></select>
        <div class="detail-form-hint">Regular users must be bound to exactly one profile.</div>
      </div>
      <div id="userDetailError" class="detail-form-error" style="display:none"></div>
      <button type="submit" class="sm-btn" style="width:100%;padding:8px;font-weight:600">Save changes</button>
    </form>`;
  _populateUserProfileSelect($('userDetailProfile'),profileName);
}

async function _populateUserProfileSelect(selectEl,selected){
  if(!selectEl) return;
  try{
    const data=await api('/api/profiles');
    const profiles=Array.isArray(data.profiles)?data.profiles:[];
    const opts=['<option value="">—</option>'].concat(profiles.map(p=>{
      const name=esc((p&&p.name)||'');
      const sel=(p&&p.name)===selected?' selected':'';
    ...
    if(selected){
      selectEl.innerHTML=`<option value="${esc(selected)}" selected>${esc(selected)}</option>`;
    }
  }
}

async function saveAdminUserDetail(){
  const errEl=$('userDetailError');
  if(errEl){errEl.style.display='none';errEl.textContent='';}
  const user=_currentUserDetail;
  if(!user||!user.username){return;}
  const roleEl=$('userDetailRole');
  const profileEl=$('userDetailProfile');
  const role=roleEl?roleEl.value:'user';
  const profile_name=profileEl?(profileEl.value||'').trim():'';
  if(role==='user'&&!profile_name){
    if(errEl){errEl.textContent='Profile is required for regular users.';errEl.style.display='';}
    return;
  }
  try{
    const updated=await _usersAdminJson('/api/admin/users/'+encodeURIComponent(user.username),{
      method:'PATCH',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({role,profile_name:profile_name||null}),
    });
    _currentUserDetail=updated.user||updated;
    showToast('User updated');
    await loadUsersPanel();
    _usersPanelMode='detail';
    _renderUserDetail($('usersPanelBody'),_currentUserDetail);
  }catch(e){
...
async function deleteAdminUser(username){
  if(!username) return;
  const ok=await showConfirmDialog({title:'Delete user',message:'Remove '+username+'? This cannot be undone.',confirmLabel:'Delete',danger:true,focusCancel:true});
  if(!ok) return;
  try{
    await _usersAdminJson('/api/admin/users/'+encodeURIComponent(username),{method:'DELETE'});
...
function openUserCreateForm(){
  _usersPanelMode='create';
  _currentUserDetail=null;
  const body=$('usersPanelBody');
  if(body) _renderUserCreateForm(body);
}

function _renderUserCreateForm(container){
  container.innerHTML=`
    <div class="users-panel-toolbar">
      <button type="button" class="sm-btn" onclick="backToUsersList()">← Back</button>
    </div>
    <form class="detail-form" style="margin-top:12px" onsubmit="event.preventDefault(); submitAdminUserCreate();">
      <div class="detail-form-row">
        <label for="userCreateUsername">Username</label>
        <input type="text" id="userCreateUsername" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" required>
        <div class="detail-form-hint">Lowercase letters, numbers, hyphens, underscores.</div>
      </div>
      <div class="detail-form-row">
        <label for="userCreatePassword">Password</label>
        <input type="password" id="userCreatePassword" autocomplete="new-password" required>
      </div>
      <div class="detail-form-row">
        <label for="userCreateRole">Role</label>
        <select id="userCreateRole" onchange="_toggleUserCreateProfileRequired()">
          <option value="user" selected>user</option>
          <option value="admin">admin</option>
        </select>
      </div>
      <div class="detail-form-row" id="userCreateProfileRow">
        <label for="userCreateProfile">Profile</label>
        <input type="text" id="userCreateProfile" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" placeholder="defaults to username">
        <div class="detail-form-hint" id="userCreateProfileHint">Defaults to the username. A dedicated Hermes profile and workspace directory are created automatically (one profile per account).</div>
      </div>
      <div id="userCreateError" class="detail-form-error" style="display:none"></div>
      <button type="submit" class="sm-btn" style="width:100%;padding:8px;font-weight:600">Create user</button>
    </form>`;
  _syncUserCreateProfileDefault();
  _toggleUserCreateProfileRequired();
  const u=$('userCreateUsername');
  if(u){
    u.addEventListener('input', _syncUserCreateProfileDefault);
    u.focus();
  }
}

function _syncUserCreateProfileDefault(){
  const usernameEl=$('userCreateUsername');
  const profileEl=$('userCreateProfile');
  if(!usernameEl||!profileEl) return;
  const username=(usernameEl.value||'').trim().toLowerCase();
  const current=(profileEl.value||'').trim();
  const prevAuto=(profileEl.dataset.autoProfile||'').trim();
  if(!current||current===prevAuto){
    profileEl.value=username;
    profileEl.dataset.autoProfile=username;
  }
}

function _toggleUserCreateProfileRequired(){
  const roleEl=$('userCreateRole');
  const row=$('userCreateProfileRow');
  const hint=$('userCreateProfileHint');
  if(!roleEl||!row) return;
  const isUser=roleEl.value==='user';
  row.style.display=isUser?'':'none';
  if(hint) hint.style.display=isUser?'':'none';
  if(isUser) _syncUserCreateProfileDefault();
}

async function submitAdminUserCreate(){
  const errEl=$('userCreateError');
  if(errEl){errEl.style.display='none';errEl.textContent='';}
  const usernameEl=$('userCreateUsername');
  const passwordEl=$('userCreatePassword');
  const roleEl=$('userCreateRole');
  const profileEl=$('userCreateProfile');
  const username=(usernameEl&&usernameEl.value||'').trim().toLowerCase();
  const password=passwordEl?passwordEl.value:'';
  const role=roleEl?roleEl.value:'user';
  let profile_name=profileEl?(profileEl.value||'').trim():'';
  if(!username||!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(username)){
    if(errEl){errEl.textContent='Invalid username.';errEl.style.display='';}
    return;
  }
  if(!password){
  ...
  if(role==='user'){
    ...
    if(!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(profile_name)){
      if(errEl){errEl.textContent='Invalid profile name.';errEl.style.display='';}
      return;
    }
  }
  try{
    await _usersAdminJson('/api/admin/users',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({username,password,role,profile_name:role==='user'?profile_name:null}),
    });
...
async function syncAuthSessionControls(){
  try{
    const authStatus=await api('/api/auth/status');
    _syncAuthSessionControls(authStatus);
  }catch(_e){}
...
function _syncAuthSessionControls(authStatus){
  const active=!!(authStatus&&authStatus.auth_enabled);
  const loggedIn=!!(authStatus&&authStatus.logged_in);
  const showSignOut=active&&loggedIn;
  const auth=_parseAuthRolePayload(authStatus||{});
  const isAdmin=_isAdminAuth(auth);

  const signOutBtn=$('btnSignOut');
  if(signOutBtn) signOutBtn.style.display=showSignOut?'':'none';
  const menuSignOut=$('settingsMenuSignOut');
  if(menuSignOut) menuSignOut.style.display=showSignOut?'':'none';

  const disableBtn=$('btnDisableAuth');
  if(disableBtn) disableBtn.style.display=(showSignOut&&isAdmin&&!_settingsPasswordEnvLocked)?'':'none';
  ...
  const pwField=$('settingsPassword');
  const pwFieldWrap=pwField&&pwField.closest('.settings-field');
  if(pwFieldWrap) pwFieldWrap.style.display=(active&&isAdmin)?'':'none';

  const passkeysBlock=$('passkeysSettingsBlock');
  if(passkeysBlock) passkeysBlock.style.display=(active&&isAdmin)?'':'none';
}

function _setSettingsAuthButtonsVisible(active){
  _syncAuthSessionControls({auth_enabled:!!active,logged_in:!!active});
}
...
async function signOut(){
  try{
    ...
    if(typeof showToast==='function') showToast(t('sign_out_failed')+e.message);
  }
...
function loadMcpServers(){
  ...
  api('/api/mcp/servers').then(r=>{
    _renderSettingsMcpServerList(r);
    return api('/api/mcp/discover',{method:'POST',body:'{}'});
  }).then(r=>{
    if(!r) return;
    _renderSettingsMcpServerList(r);
    if(r.errors && Object.keys(r.errors).length){
    ...
    if(list && !list.querySelector('.mcp-server-row')){
      list.innerHTML=`<div class="mcp-error-state" style="color:#ef4444;font-size:12px;padding:6px 0">${esc(t('mcp_load_failed'))}</div>`;
    }
  });
}
function _renderSettingsMcpServerList(r){
  const list=$('mcpServerList');
  if(!list) return;
