...
function _persistSessionModelCorrection(model, provider, opts){
  if(!S.session) return;
  const request=fetch(new URL(apiV1Path('api/session/update'),document.baseURI||location.href).href,{
...
async function populateModelDropdown(opts={}){
  ...
  if(!sel) return;
  try{
    const _modelsRes=await fetch(new URL(apiV1Path('api/models'),document.baseURI||location.href).href,{credentials:'include'});
...
async function _fetchLiveModels(provider, sel){
  ...
  try{
    const url=new URL(apiV1Path('api/models/live'),document.baseURI||location.href);
...
function renderMd(raw){
  ...
  function _markdownHref(raw){
    ...
    if(/^file:\/\//i.test(href)){
      try{
        ...
        return apiV1Path('api/media?path='+encodeURIComponent(path)+'&inline=1');
      }catch(_){
        return apiV1Path('api/media?path='+encodeURIComponent(href.replace(/^file:\/\//i,''))+'&inline=1');
      }
    }
    return href;
  }
  ...
  s=s.replace(/\x00D(\d+)\x00/g,(_,i)=>{
    ...
    // Local file path
    const apiUrl=apiV1Path('api/media?path='+encodeURIComponent(ref));
...
function _formatMessageFooterTimestamp(tsVal){
  ...
  const normAmPm=s=>String(s||'').replace(/\ba\.m\./gi,'AM').replace(/\bp\.m\./gi,'PM');
  if(_isSameLocalDay(date, now)){
    const opts={hour:'2-digit', minute:'2-digit'};
    return normAmPm(fmt?fmt(date,opts):date.toLocaleTimeString([], opts));
  }
  const opts={month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'};
  return normAmPm(fmt?fmt(date,opts):date.toLocaleString([], opts));
}
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    if(m.attachments&&m.attachments.length){
      ...
      filesHtml=`<div class="msg-files">${m.attachments.map(f=>{
        ...
        const fileUrl=apiV1Path('api/file/raw?session_id='+encodeURIComponent(_attachSid)+'&path='+encodeURIComponent(fname));
...
function loadDiffInline(container){
  ...
  root.querySelectorAll('.diff-inline-load:not([data-loaded])').forEach(el=>{
    ...
    fetch(apiV1Path('api/media?path='+encodeURIComponent(path)))
  ...
  root.querySelectorAll('.excalidraw-inline-load:not([data-loaded])').forEach(el=>{
    ...
    fetch(apiV1Path('api/media?path='+encodeURIComponent(path)))
      ...
      .then(text=>{
        ...
        const downloadUrl=apiV1Path('api/media?path='+encodeURIComponent(path)+'&download=1');
      ...
      fetch(apiV1Path('api/media?path='+encodeURIComponent(path)))
        ...
        .then(buf=>{
          if(buf.byteLength>PDF_MAX_SIZE){
            el.outerHTML=`<div class="pdf-preview-fallback"><a class="msg-media-link" href="${esc(apiV1Path('api/media?path='+encodeURIComponent(path)+'&download=1'))}" download="${esc(fname)}">📎 ${esc(fname)}</a><br><span style="color:var(--muted);font-size:12px">${t('pdf_too_large')}</span></div>`;
            return;
          }
        ...
        .then(pdf=>{
          ...
          pdf.getPage(1).then(page=>{
            ...
            page.render({canvasContext:canvas.getContext('2d'),viewport}).promise.then(()=>{
              ...
              const dlUrl=apiV1Path('api/media?path='+encodeURIComponent(path)+'&download=1');
              const wrap=document.createElement('div');
    ...
    fetch(apiV1Path('api/media?path='+encodeURIComponent(path)))
      ...
      .then(html=>{
        if(html.length>HTML_MAX_SIZE){
          const openUrl=apiV1Path('api/media?path='+encodeURIComponent(path)+'&inline=1');
...
function _showFileContextMenu(e, item){
  ...
  if(item.type==='dir'){
    ...
    dlItem.onclick=()=>{
      menu.remove();
      const url=apiV1Path('/api/folder/download?session_id='+encodeURIComponent(S.session.session_id)
...
async function uploadPendingFiles(){
  ...
  for(let i=0;i<total;i++){
    ...
    try{
      ...
      const url=new URL(apiV1Path(isArchive?'api/upload/extract':'api/upload'),document.baseURI||location.href).href;
