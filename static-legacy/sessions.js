...
function ensureSessionEventsSSE(){
  ...
  try{
    ...
    _sessionEventsSSE = new EventSource(apiV1Path('api/sessions/events'));
...
async function probeGatewaySSEStatus(){
  ...
  try{
    const resp = await fetch(new URL(apiV1Path('api/sessions/gateway/stream?probe=1'), document.baseURI || location.href).href, { credentials:'same-origin' });
...
function startGatewaySSE(){
  ...
  try{
    _gatewaySSE = new EventSource(apiV1Path('api/sessions/gateway/stream'));
