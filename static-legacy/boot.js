...
async function cancelStream(){
  ...
  if(!streamId) return;
  try{
    await fetch(new URL(apiV1Path(`api/chat/cancel?stream_id=${encodeURIComponent(streamId)}`),document.baseURI||location.href).href,{credentials:'include'});
...
(function(){
  ...
  async function _transcribeBlob(blob){
    ...
    try{
      const res=await fetch(apiV1Path('api/transcribe'),{method:'POST',body:form});
...
$('btnExportJSON').onclick=()=>{
  if(!S.session)return;
  const url=apiV1Path(`/api/session/export?session_id=${encodeURIComponent(S.session.session_id)}`);
...
(async()=>{
  ...
  try{
    if(typeof _fetchAuthRole==='function'){
      ...
      if(auth&&auth.profile_name&&typeof _canSwitchProfiles==='function'&&!_canSwitchProfiles(auth)){
        S.activeProfile=auth.profile_name;
      }
      if(typeof _applyProfileSwitcherRestrictions==='function') _applyProfileSwitcherRestrictions(auth);
      if(typeof _applyProfilesPanelRestrictions==='function') _applyProfilesPanelRestrictions(auth);
      if(typeof _applyInsightsLogsPanelRestrictions==='function') _applyInsightsLogsPanelRestrictions(auth);
      if(typeof _populateInsightsLogsScopeFilter==='function') _populateInsightsLogsScopeFilter();
    }
  }catch(_authErr){}
  applyBotName();
  ...
  if(typeof syncAuthSessionControls==='function') void syncAuthSessionControls();
