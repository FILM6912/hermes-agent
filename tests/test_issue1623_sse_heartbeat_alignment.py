...
def test_sse_heartbeat_constant_below_kernel_keepalive_window():
    ...
    src = (REPO / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
...
def test_each_named_sse_handler_uses_constant():
    ...
    routes_src = (REPO / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
    chat_src = (REPO / "app" / "domain" / "chat_streaming.py").read_text(encoding="utf-8")

    # Main agent SSE lives in app.domain.chat_streaming; legacy routes
    # still own terminal/gateway/approval long-poll handlers.
    assert "subscriber.get(timeout=SSE_HEARTBEAT_INTERVAL_SECONDS)" in chat_src, (
        "Expected native chat SSE to call subscriber.get(timeout=SSE_HEARTBEAT_INTERVAL_SECONDS) (#1623)."
    )
    assert "term.output.get(timeout=_SSE_HEARTBEAT_INTERVAL_SECONDS)" in routes_src, (
        "Expected terminal SSE handler to call term.output.get(timeout=_SSE_HEARTBEAT_INTERVAL_SECONDS) (#1623)."
    )

    n_uses = routes_src.count("get(timeout=_SSE_HEARTBEAT_INTERVAL_SECONDS)")
    n_uses += chat_src.count("get(timeout=SSE_HEARTBEAT_INTERVAL_SECONDS)")
    assert n_uses >= 4, (
