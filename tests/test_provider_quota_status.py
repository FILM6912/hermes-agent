...
def _with_config(model=None, providers=None):
    ...
    # Keep the cached config path aligned with the active HERMES_CONFIG_PATH.
    # Prior tests (e.g. dashboard config roundtrip) can leave _cfg_path pointing
    # at a temporary profile; get_config() reloads on profile_path_changed and
    # would discard these in-memory overrides.
...
def _restore_config(old_cfg, old_mtime, old_path):
    config.cfg.clear()
...
def test_openrouter_quota_fetches_key_endpoint_and_sanitizes_response(monkeypatch, tmp_path):
    ...
    old_cfg, old_mtime, old_path = _with_config(model={"provider": "openrouter"})

    import app.domain.providers as providers
    seen = {}
    ...
    try:
        ...
        _restore_config(old_cfg, old_mtime, old_path)
...
def test_unsupported_provider_reports_followup_state(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
    old_cfg, old_mtime, old_path = _with_config(model={"provider": "openai"})
...
def test_codex_account_usage_is_fetched_under_active_profile_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
    old_cfg, old_mtime, old_path = _with_config(model={"provider": "openai-codex"})
...
def test_anthropic_oauth_usage_unavailable_reason_is_reported(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(profiles, "get_active_hermes_home", lambda: tmp_path)
    old_cfg, old_mtime, old_path = _with_config(model={"provider": "anthropic"})
...
def test_openai_api_key_detection_falls_through_after_codex_jwt_config_value(monkeypatch, tmp_path):
    ...
    old_cfg, old_mtime, old_path = _with_config(
...
def test_provider_quota_route_is_registered():
    ...
    routes = (ROOT / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
...
def test_v1_provider_mutation_routes_are_registered():
    """FastAPI v1 provider/model mutations must call native services."""
    providers_ep = (ROOT / "app" / "api" / "v1" / "endpoints" / "providers.py").read_text(
        encoding="utf-8"
    )
    assert '@router.post("/providers")' in providers_ep
    assert '@router.post("/providers/delete")' in providers_ep
    assert '@router.get("/provider/quota")' in providers_ep
    assert "_service.set_provider_key(" in providers_ep
    assert "_service.remove_provider_key(" in providers_ep
    assert "_service.get_provider_quota(" in providers_ep
    assert "_service.list_providers()" in providers_ep
    models_ep = (ROOT / "app" / "api" / "v1" / "endpoints" / "models.py").read_text(
        encoding="utf-8"
    )
    assert '@router.post("/model/set")' in models_ep
    assert '@router.post("/default-model")' in models_ep
    assert '@router.get("/reasoning")' in models_ep
    assert '@router.post("/reasoning")' in models_ep
    assert '@router.get("/model/auxiliary")' in models_ep
    assert "dispatch_legacy_route" not in models_ep
    assert "_service.set_model(" in models_ep
    assert "_service.get_auxiliary_models()" in models_ep
...
def test_provider_quota_card_is_rendered_in_providers_panel():
    ...
    panels = (ROOT / "static-legacy" / "panels.js").read_text(encoding="utf-8")
...
def test_provider_quota_i18n_keys_exist_for_all_locales():
    ...
    i18n = (ROOT / "static-legacy" / "i18n.js").read_text(encoding="utf-8")
    locale_count = len(
        re.findall(r"^  (?:[A-Za-z_][A-Za-z0-9_]*|'[^']+'):\s*\{", i18n, re.MULTILINE)
    )
    keys = sorted(set(re.findall(r"provider_quota_[a-z0-9_]+", (ROOT / "static-legacy" / "panels.js").read_text(encoding="utf-8"))))
...
def test_provider_quota_styles_exist():
    ...
    css = (ROOT / "static-legacy" / "style.css").read_text(encoding="utf-8")
    for token in (
        ".provider-quota-card",
