...
UI_JS = (REPO_ROOT / "static-legacy" / "ui.js").read_text(encoding="utf-8")
PANELS_JS = (REPO_ROOT / "static-legacy" / "panels.js").read_text(encoding="utf-8")
INDEX_HTML = (REPO_ROOT / "static" / "index.html").read_text(encoding="utf-8")
STYLE_CSS = (REPO_ROOT / "static-legacy" / "style.css").read_text(encoding="utf-8")
ROUTES_PY = (REPO_ROOT / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
AUTH_PY = (REPO_ROOT / "app" / "domain" / "auth.py").read_text(encoding="utf-8")
...
def test_system_health_payload_normalizes_safe_aggregate_metrics(monkeypatch):
    from app.domain import system_health
...
def test_system_health_route_registered_and_public(monkeypatch):
    assert 'parsed.path == "/api/system/health"' in ROUTES_PY
    assert "build_system_health_payload()" in ROUTES_PY
    assert '"/api/v1/system/health"' in AUTH_PY

    monkeypatch.setenv("HERMES_WEBUI_PASSWORD", "test-password")
    from app.domain.auth import check_auth

    handler = _FakeHandler()
    assert check_auth(handler, SimpleNamespace(path="/api/system/health", query="")) is True
    handler2 = _FakeHandler()
    assert check_auth(handler2, SimpleNamespace(path="/api/v1/system/health", query="")) is True
...
def test_system_health_backend_uses_no_shell_or_private_process_sources():
    src = (REPO_ROOT / "app" / "domain" / "system_health.py").read_text(encoding="utf-8")
