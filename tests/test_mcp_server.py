...
def _cleanup_state_dir(state_dir: Path):
    ...
    if saved.get("captured"):
        ...
        for attr, val in saved["app.domain.config"].items():
            setattr(_cfg, attr, val)
        if "mcp_server" in sys.modules:
            mcp_mod = sys.modules["mcp_server"]
            for attr, val in saved["mcp_server"].items():
                setattr(mcp_mod, attr, val)
        if "app.domain.models" in sys.modules:
            models_mod = sys.modules["app.domain.models"]
            for attr, val in saved["app.domain.models"].items():
...
def _reimport_mcp():
    ...
    if not _SAVED_CONSTANTS.get("captured"):
        _SAVED_CONSTANTS["app.domain.config"] = {
        ...
        if "app.domain.models" in sys.modules:
            models_mod = sys.modules["app.domain.models"]
            _SAVED_CONSTANTS["app.domain.models"] = {
    ...
    # module reference. The result is that `import app.domain.profiles` returns
    ...
    import app.domain.profiles as fresh_profiles_via_import
...
class TestDeleteProject:
    ...
    async def test_delete_no_auth_refuses_unassign(self):
        ...
        """
        from app.domain.config import SESSION_DIR, SESSION_INDEX_FILE
...
class TestProfileScoping:
    ...
    async def test_legacy_untagged_hidden_from_non_root_profile(self):
        ...
        import app.domain.config as _cfg_mod
...
class TestApiPassword:
    ...
    async def test_password_hash_in_settings_is_ignored(self):
        ...
        from app.domain.config import STATE_DIR as _SD
...
async def test_profiles_match_single_source_of_truth():
    ...
    saved_modules = {
        k: sys.modules[k]
        for k in ('mcp_server', 'app.domain.routes', 'app.domain.profiles')
        if k in sys.modules
    }
    # Also snapshot the attributes on the parent `app.domain` package, because
    # `import app.domain.routes as r` resolves via `sys.modules['app.domain'].routes`,
    # NOT directly via sys.modules['app.domain.routes']. If we don't restore
    # the parent attribute, subsequent `import app.domain.routes as r` calls
    # bind to the fresh re-imported module even though sys.modules
    # holds the original.
    import app.domain as _api_parent
    saved_api_attrs = {}
    ...
    try:
        import app.domain.profiles as _profiles_mod
        import app.domain.routes as _routes_mod
...
async def test_profiles_match_input_matrix(a, b):
    ...
    from app.domain.routes import _profiles_match as routes_match
...
class TestApiWireFormat:
    ...
    async def test_rename_session_posts_to_canonical_path(self):
        ...
        assert req["path"] == "/api/v1/session/rename"
    ...
    async def test_move_session_posts_to_canonical_path(self):
        ...
        assert len(_RecordingHandler.captured) == 1
        req = _RecordingHandler.captured[0]
        assert req["path"] == "/api/v1/session/move"
