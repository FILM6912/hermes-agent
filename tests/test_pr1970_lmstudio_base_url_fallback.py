...
def test_get_provider_base_url_lm_studio_alias_matches_lmstudio_lookup():
    """model.provider: lm-studio must resolve for @lmstudio:model lookups."""
    with _RestoreCfg():
        config.cfg.clear()
        config.cfg.update({
            "model": {
                ...
                "default": "qwopus3.5-9b-coder",
            },
            "providers": {
                "lm-studio": {"api_key": "local-key"},
            },
        })
        assert config._get_provider_base_url("lmstudio") == "http://192.168.99.1:1234/v1"


def test_get_provider_base_url_providers_lm_studio_key():
    """providers.lm-studio.base_url must resolve for lmstudio lookups."""
    with _RestoreCfg():
        config.cfg.clear()
        config.cfg.update({
            "model": {"provider": "lm-studio", "default": "m"},
            "providers": {
                "lm-studio": {"base_url": "http://192.168.99.1:1234/v1", "api_key": "x"},
            },
        })
        assert config._get_provider_base_url("lmstudio") == "http://192.168.99.1:1234/v1"


def test_resolve_model_provider_at_lmstudio_uses_lm_studio_base_url():
    """Dropdown @lmstudio:model must not fall through to LM_BASE_URL default."""
    with _RestoreCfg():
        config.cfg.clear()
        config.cfg.update({
            "model": {
        ...
        model, provider, base_url = config.resolve_model_provider(
            "@lmstudio:qwopus3.5-9b-coder"
        )
        assert model == "qwopus3.5-9b-coder"
