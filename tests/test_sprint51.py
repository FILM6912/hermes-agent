...
from app.domain.config import AGENT_INSTANCES, STREAMS, STREAMS_LOCK, CANCEL_FLAGS
...
class TestCancelStreamEagerRelease:
    ...
    def test_cancel_clears_session_active_stream_id(self):
        ...
        with patch('app.domain.streaming.get_session', return_value=mock_session):
            cancel_stream(stream_id)
    ...
    def test_cancel_handle_get_session_failure(self):
        ...
        with patch('app.domain.streaming.get_session', side_effect=KeyError("Session not found")):
            # Should not raise
            result = cancel_stream(stream_id)
