...
STREAMING = pathlib.Path(__file__).parent.parent / 'app' / 'domain' / 'streaming.py'
MESSAGES_JS = pathlib.Path(__file__).parent.parent / 'static-legacy' / 'messages.js'
...
class TestStreamEndSessionId:
    ...
    def test_background_title_thread_does_not_emit_stream_end(self):
        """Background title runs after stream_end; it must not delay stream closure."""
        fn_start = streaming_src.find("def _run_background_title_update(")
        assert fn_start != -1
        fn_end = streaming_src.find("\ndef ", fn_start + 1)
        fn_body = streaming_src[fn_start:fn_end]
        assert "put_event('stream_end'" not in fn_body

    def test_background_title_path_emits_stream_end_before_thread(self):
        """Initial title generation must not block stream_end on the LLM title call."""
        bg_idx = streaming_src.find("target=_run_background_title_update")
        assert bg_idx != -1
        window = streaming_src[max(0, bg_idx - 400):bg_idx]
        assert "put('stream_end', {'session_id': session_id})" in window
