...
STYLE_CSS  = (REPO_ROOT / "static-legacy" / "style.css").read_text()
SESSIONS_JS = (REPO_ROOT / "static-legacy" / "sessions.js").read_text()
PANELS_JS   = (REPO_ROOT / "static-legacy" / "panels.js").read_text()

try:
    from app.domain import config as _api_config
...
class TestGatewaySessionNullModel(unittest.TestCase):
    ...
    def test_gateway_session_null_model_returns_none_not_unknown(self):
        ...
        models_src = (REPO_ROOT / "app" / "domain" / "models.py").read_text()
    ...
    def test_gateway_watcher_null_model_returns_none_not_unknown(self):
        ...
        gw_src = (REPO_ROOT / "app" / "domain" / "gateway_watcher.py").read_text()
        self.assertNotIn(
            "'model': row['model'] or 'unknown'",
            gw_src,
            "api/gateway_watcher.py must not use `or 'unknown'` for the model "
...
# ── #454 model routing ─────────────────────────────────────────────
@unittest.skipUnless(_config_available, "app.domain.config not importable")
