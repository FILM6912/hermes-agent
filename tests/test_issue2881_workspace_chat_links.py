...
def _render(driver_path: str, markdown: str) -> str:
    result = subprocess.run(
        [NODE, driver_path, str(REPO_ROOT / "static-legacy" / "ui.js")],
        input=markdown,
        capture_output=True,
        text=True,
        timeout=10,
...
def test_workspace_link_click_delegate_opens_workspace_preview():
    ...
    assert "async function openArtifactPath(path)" in (REPO_ROOT / "static-legacy" / "workspace.js").read_text(encoding="utf-8")
    assert "/api/list?session_id=" in (REPO_ROOT / "static-legacy" / "workspace.js").read_text(encoding="utf-8")
    assert "file_open_failed" in (REPO_ROOT / "static-legacy" / "workspace.js").read_text(encoding="utf-8")
