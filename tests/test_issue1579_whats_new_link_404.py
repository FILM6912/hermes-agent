...
def test_current_sha_is_merge_base_not_local_HEAD(tmp_path, monkeypatch):
    ...
    if 'app.domain.updates' in sys.modules:
        del sys.modules['app.domain.updates']
    from app.domain import updates as upd
...
def _read_ui_js():
    return (REPO_ROOT / 'static-legacy' / 'ui.js').read_text(encoding='utf-8')
