...
_SESSIONS_JS = REPO_ROOT / 'static-legacy' / 'sessions.js'
_STREAMING_PY = REPO_ROOT / 'app' / 'domain' / 'streaming.py'
_MESSAGES_JS = REPO_ROOT / 'static-legacy' / 'messages.js'
_UI_JS = REPO_ROOT / 'static-legacy' / 'ui.js'
...
class TestRuntimeRouteInjection(unittest.TestCase):
    ...
    def test_runtime_route_is_forwarded_from_resolver_into_agent_init(self):
        ...
        import app.domain.streaming as streaming
        ...
        with mock.patch.object(streaming, "get_session", return_value=fake_session), \
             ...
             mock.patch("app.domain.config.get_config", return_value={}), \
             mock.patch("app.domain.config._resolve_cli_toolsets", return_value=[]), \
             mock.patch.dict(
                 sys.modules,
                 {
    ...
    def test_clarify_callback_passes_configured_timeout_seconds(self):
        ...
        with mock.patch.object(streaming, "get_session", return_value=fake_session), \
             ...
             mock.patch("app.domain.config._resolve_cli_toolsets", return_value=[]), \
             mock.patch("app.domain.clarify.submit_pending", side_effect=fake_submit_pending), \
             mock.patch.dict(sys.modules, {
                "hermes_cli": fake_hermes_cli,
                "hermes_cli.runtime_provider": fake_rt_module,
                "hermes_state": fake_hermes_state,
...
class TestModelCustomInput(unittest.TestCase):
    ...
    STATIC = pathlib.Path(__file__).parent.parent / 'static-legacy'
...
def test_streaming_persists_reasoning_in_session():
    ...
    src = (REPO / 'app' / 'domain' / 'streaming.py').read_text()
...
def test_done_handler_patches_reasoning_field():
    ...
    src = (REPO / 'static-legacy' / 'messages.js').read_text()
...
def test_rendermessages_reads_reasoning_from_messages():
    ...
    src = (REPO / 'static-legacy' / 'ui.js').read_text()
...
def test_routes_restores_prior_reasoning_metadata_after_followup():
    ...
    src = (REPO / 'app' / 'domain' / 'routes.py').read_text()
