...
def test_profiles_match_exact():
    ...
    from app.domain.routes import _profiles_match
...
def test_profiles_match_default_alias_treated_as_root(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: [SimpleNamespace(name='kinni', is_default=True)],
    )
...
def test_all_profiles_query_flag_true_values():
    ...
    from app.domain.routes import _all_profiles_query_flag
...
def test_keep_latest_messaging_runs_after_profile_filter():
    ...
    repo_root = Path(__file__).parent.parent
    src = (repo_root / 'app' / 'domain' / 'routes.py').read_text(encoding='utf-8')
