...
import app.domain.streaming as streaming  # noqa: F401  imported for fixture parity
from app.domain.models import (
    Session,
    _apply_core_sync_or_error_marker,
    merge_session_messages_append_only,
)
from app.domain.run_journal import append_run_event
...
def test_messages_js_names_browser_sse_disconnect_separately():
    ...
    js = (repo / "static-legacy" / "messages.js").read_text(encoding="utf-8")
...
def test_server_treats_broken_pipe_as_client_disconnect_not_500():
    ...
    if (repo / "app" / "main.py").exists():
        target = repo / "app" / "core" / "legacy_handler.py"
    else:
        target = repo / "server.py"
    source = target.read_text(encoding="utf-8")

    assert "except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):" in source
    assert "do not convert it into a misleading server 500" in source
