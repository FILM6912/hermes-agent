...
def test_parse_multipart_text_file():
    ...
    src = pathlib.Path(__file__).parent.parent.joinpath("app/domain/upload.py").read_text()
...
def test_upload_text_file(cleanup_test_sessions):
    """Upload a text file to workspace/.uploads, not the workspace root."""
    ...
    assert result.get("workspace_rel") == ".uploads/test_upload.txt"

    uploaded_path = pathlib.Path(result["path"])
    assert uploaded_path.exists()
    assert uploaded_path.read_bytes() == b"sprint1 test content"
    assert uploaded_path.parent.name == ".uploads"
    assert uploaded_path.is_relative_to(ws.resolve())

    listing = get(f"/api/list?session_id={sid}&path=.uploads")
...
def test_upload_respects_attachment_dir_env(monkeypatch, tmp_path):
    ...
    from app.domain.upload import _session_attachment_dir, _upload_destination
...
def test_upload_destination_does_not_overwrite_same_filename(monkeypatch, tmp_path):
    ...
    from app.domain.upload import _upload_destination
