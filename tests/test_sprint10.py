...
def test_domain_modules_exist(cleanup_test_sessions):
    """Core app/domain/ module files must exist on disk."""
    base = REPO_ROOT / "app" / "domain"
    for mod in ["__init__.py", "config.py", "helpers.py", "models.py",
                "workspace.py", "upload.py", "streaming.py"]:
        assert (base / mod).exists(), f"Missing app/domain/{mod}"
...
def test_api_config_has_cancel_flags(cleanup_test_sessions):
    src = (REPO_ROOT / "app/domain/config.py").read_text()
...
def test_cron_output_usage_metadata_parses_optional_fields(cleanup_test_sessions):
    from app.domain.routes import _cron_output_usage_metadata
...
def test_cron_output_window_preserves_response_after_large_prompt(cleanup_test_sessions):
    ...
    from app.domain.routes import _cron_output_content_window
