...
PWA_STARTUP = ROOT / "static-legacy" / "pwa-startup.js"
BOOT = ROOT / "static-legacy" / "boot.js"
INDEX = ROOT / "static" / "index.html"
ROUTES = ROOT / "app" / "domain" / "routes.py"
AUTH = ROOT / "app" / "domain" / "auth.py"
...
class TestIndexHtmlIntegration:
    ...
    def test_index_versions_stylesheet(self):
        ...
        assert 'href="static-legacy/style.css"' not in src, (
    ...
    def test_index_route_url_encodes_asset_version(self):
        src = ROUTES.read_text(encoding="utf-8")
        helper_idx = src.find("def _build_shell_html")
        assert helper_idx != -1, (
            "routes.py must define _build_shell_html for vanilla and React SPA shells"
        )
        helper_block = src[helper_idx:helper_idx + 800]
        assert "quote(WEBUI_VERSION, safe=\"\")" in helper_block, (
            "_build_shell_html must URL-encode the cache-busting version token before "
...
class TestSessionManifestRoute:
    ...
    def _get(self, path):
        ...
        from app.domain.routes import handle_get
...
class TestSessionManifestAuthExemption:
    ...
    def test_session_manifest_json_is_public(self, monkeypatch):
        ...
        from app.domain.auth import check_auth, _invalidate_password_hash_cache
