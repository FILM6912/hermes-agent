...
and _attachment_name from app.domain.streaming / api.routes behave correctly
...
from app.domain import profiles as profiles_mod
from app.domain.streaming import (
    _attachment_name,
    _build_native_multimodal_message,
    _catalog_entry_matches_model,
    _infer_multimodal_from_model_name,
    _lookup_custom_provider_supports_vision,
    _model_catalog_entry_supports_multimodal,
    _NATIVE_IMAGE_MAX_BYTES,
    _resolve_image_input_mode,
    _sanitize_messages_for_api,
    _webui_extended_supports_vision,
)
from app.domain.routes import _normalize_chat_attachments
...
class TestAttachmentName:
    def test_dict_with_name(self):
        assert _attachment_name({'name': 'photo.png', 'path': '/tmp/x'}) == 'x'
    ...
    def test_dict_with_path_fallback(self):
        assert _attachment_name({'path': '/ws/snap.png'}) == 'snap.png'
...
class TestModelCatalogMultimodalProbe:
    def test_llamacpp_capabilities_multimodal(self):
        entry = {
            'model': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
            'capabilities': ['completion', 'multimodal'],
        }
        assert _model_catalog_entry_supports_multimodal(entry) is True

    def test_text_only_capabilities(self):
        entry = {'id': 'text-model', 'capabilities': ['completion']}
        assert _model_catalog_entry_supports_multimodal(entry) is False

    def test_catalog_entry_matches_model_tag_suffix(self):
        entry = {'id': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF'}
        assert _catalog_entry_matches_model(
            entry,
            'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
        )

    def test_infer_multimodal_from_qwen36_name(self):
        assert _infer_multimodal_from_model_name('unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16')

    def test_infer_multimodal_false_for_text_model(self):
        assert not _infer_multimodal_from_model_name('meta-llama/Llama-3.1-8B-Instruct')


class TestResolveImageInputMode:
    def test_custom_provider_supports_vision_override(self):
        cfg = {
            ...
            'custom_providers': [{
                'name': 'runpod',
                'base_url': 'https://example.runpod.net/v1',
                'model': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
                'models': {
                    'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16': {
        ...
        assert _lookup_custom_provider_supports_vision(
            cfg,
            provider='custom',
            model='unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
        ) is True
    ...
    def test_probe_unavailable_falls_back_to_model_name_hint(self, monkeypatch):
        cfg = {
            'agent': {'image_input_mode': 'auto'},
            'model': {
                'provider': 'custom',
        ...
        monkeypatch.setattr(
            'app.domain.streaming._probe_openai_compatible_multimodal',
    ...
    def test_probe_false_uses_text_mode_for_custom_provider(self, monkeypatch):
        cfg = {
            'agent': {'image_input_mode': 'auto'},
            'model': {
                'provider': 'custom',
                'default': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
                'base_url': 'https://example.runpod.net/v1',
            },
        }
        monkeypatch.setattr(
        ...
        assert _resolve_image_input_mode(cfg, provider='custom') == 'text'

    def test_lookup_false_does_not_block_custom_name_hint(self, monkeypatch):
        cfg = {
            'agent': {'image_input_mode': 'auto'},
            'model': {
                'provider': 'custom',
        ...
        assert _resolve_image_input_mode(cfg, provider='custom') == 'native'

    def test_auxiliary_vision_does_not_block_qwen36_native(self):
        cfg = {
            'agent': {'image_input_mode': 'auto'},
            'model': {
                'provider': 'custom',
    ...
    def test_webui_extended_supports_vision_qwen36_gguf_tag(self):
        cfg = {
            'model': {
                'provider': 'custom',
                'model': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:bf16',
        ...
        assert _webui_extended_supports_vision(
            cfg,
            provider='custom',
            model='unsloth/Qwen3.6-35B-A3B-MTP-GGUF:bf16',
        )

    def test_patch_extends_agent_lookup_for_qwen36(self, monkeypatch):
        ...
        profiles_mod.patch_extended_vision_capability_lookup()
        cfg = {
            'model': {
                'provider': 'custom',
                'model': 'unsloth/Qwen3.6-35B-A3B-MTP-GGUF:bf16',
        ...
        assert image_routing._lookup_supports_vision('custom', cfg['model']['model'], cfg) is True
...
class TestBuildNativeMultimodalMessage:
    ...
    def test_auto_mode_with_auxiliary_vision_skips_native_embed(self):
        with TemporaryDirectory() as d:
            root = Path(d)
            img = root / 'pic.png'
            _make_png(img)
            ...
            result = _build_native_multimodal_message(
                '', 'describe', atts, str(root), cfg=cfg, provider='openrouter', model='qwen/qwen3-35b',
            )
            assert isinstance(result, str)
            assert result == 'describe'

    def test_auto_mode_embeds_when_model_supports_vision(self):
        with TemporaryDirectory() as d:
            root = Path(d)
            img = root / 'pic.png'
            _make_png(img)
            ...
            result = _build_native_multimodal_message(
                '', 'describe', atts, str(root), cfg=cfg, provider='openrouter', model='local/vision-model',
            )
            assert isinstance(result, list)
            assert result[1]['type'] == 'image_url'

    def test_auto_mode_embeds_when_custom_catalog_reports_multimodal(self, monkeypatch):
        with TemporaryDirectory() as d:
            root = Path(d)
            img = root / 'pic.png'
            _make_png(img)
            ...
            result = _build_native_multimodal_message(
                '', 'describe', atts, str(root), cfg=cfg,
                provider='custom', model='unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16',
            )
            assert isinstance(result, list)
            assert result[1]['type'] == 'image_url'

    def test_explicit_text_mode_skips_current_turn_embedding(self):
        with TemporaryDirectory() as d:
            root = Path(d)
            img = root / 'pic.png'
            _make_png(img)
            ...
            result = _build_native_multimodal_message('', 'describe', atts, str(root), cfg=cfg)
            assert isinstance(result, str)
            assert result == 'describe'

    def test_profile_subfolder_uploads_embed_for_virtual_workspace(
        self,
        tmp_path: Path,
        ...
        admin_uploads = shared_root / 'admin' / '.uploads'
        ...
        monkeypatch.setattr(
            ...
            lambda path: shared_root.resolve() if str(path).strip() == '/workspace' else Path(path).expanduser().resolve(),
        )
        ...
        result = _build_native_multimodal_message('', 'describe', atts, '/workspace')
        assert isinstance(result, list)
        assert result[1]['type'] == 'image_url'
    ...
    def test_sync_chat_history_sanitizer_receives_config(self):
        ...
        src = Path('app/domain/routes.py').read_text()
        assert '_sanitize_messages_for_api(' in src
        assert 'cfg=get_config()' in src
        assert 'provider=_provider' in src
        assert 'model=_model' in src
...
class TestAgentNativeVisionPayload:
    """WebUI embeds image_url parts, but Hermes Agent strips them unless patched."""

    @staticmethod
    def _reset_vision_lookup_patch() -> None:
        import agent.image_routing as image_routing

        original = getattr(
            image_routing._lookup_supports_vision,
            "_hermes_webui_original",
            None,
        )
        if original is not None:
            image_routing._lookup_supports_vision = original
    ...
    def _make_agent():
        ...
        agent.model = "unsloth/Qwen3.6-35B-A3B-MTP-GGUF:BF16"
        agent._anthropic_image_fallback_cache = {}
        return agent

    def test_agent_strips_image_url_without_extended_vision_patch(self):
        self._reset_vision_lookup_patch()
        user_content = [
            {"type": "text", "text": "[Workspace::v1: /workspace] what is in the image"},
            {
                "type": "image_url",
        ...
        out = agent._prepare_messages_for_non_vision_model(
    ...
    def test_startup_runtime_patches_keep_qwen36_image_url_in_agent_payload(self):
        from app.core.startup import apply_agent_runtime_patches
...
from app.domain.streaming import _is_valid_image
...
class TestAttachmentRootIntegration:
    ...
    via _attachment_root() from app.domain.upload, single source of truth.
    """
