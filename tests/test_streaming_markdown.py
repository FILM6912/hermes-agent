...
MESSAGES_JS = (REPO / "static-legacy" / "messages.js").read_text(encoding="utf-8")
INDEX_HTML = (REPO / "static" / "index.html").read_text(encoding="utf-8")


# ── Helpers ───────────────────────────────────────────────────────────────────

def _run_node_smd_script(script_body: str) -> subprocess.CompletedProcess[str]:
    """Run a Node ESM script against streaming-markdown from the vendor bundle.

    smd.min.js ships as ESM syntax but keeps a .js extension, so Node treats it
    as CommonJS unless imported from an .mjs entrypoint. Copy to a temp .mjs
    file for runtime contract checks.
    """
    smd_src = REPO / "static" / "vendor" / "smd.min.js"
    with tempfile.TemporaryDirectory(dir=REPO) as tmpdir:
        tmp = pathlib.Path(tmpdir)
        shutil.copy2(smd_src, tmp / "smd.min.mjs")
        driver = tmp / "driver.mjs"
        driver.write_text(
            f"import * as smd from './smd.min.mjs';\n{script_body}\n",
            encoding="utf-8",
        )
...
"""
        completed = _run_node_smd_script(script)
