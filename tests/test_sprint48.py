...
class TestXmlToolCallStrip:
    ...
    def _load_fn(self):
        ...
        for mod in ('app.domain.config', 'app.domain.helpers', 'app.domain.models', 'app.domain.workspace'):
            if mod not in sys.modules:
                sys.modules[mod] = types.ModuleType(mod)

        # Provide minimal symbols that streaming.py needs at import time.
        cfg = sys.modules.setdefault('app.domain.config', types.ModuleType('app.domain.config'))
