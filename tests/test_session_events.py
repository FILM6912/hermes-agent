...
ROUTES = Path("app/domain/routes.py").read_text(encoding="utf-8")
SESSION_EVENTS = Path("app/domain/session_events.py").read_text(encoding="utf-8")
PROFILES = Path("app/domain/profiles.py").read_text(encoding="utf-8")
...
def test_session_event_queue_is_bounded_and_latest_wins():
    from app.domain import session_events
