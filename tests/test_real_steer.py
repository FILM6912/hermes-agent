...
def _clear_caches():
    ...
    from app.domain.config import SESSION_AGENT_CACHE, SESSION_AGENT_CACHE_LOCK, STREAMS, STREAMS_LOCK
...
class TestHandleChatSteerHappyPath:
    ...
    def test_accepts_when_agent_cached_and_running(self, _clear_caches):
        ...
        with patch("app.domain.streaming.get_session", return_value=sess):
...
class TestHandleChatSteerFallbacks:
    ...
    def test_session_not_found(self, _clear_caches):
        ...
        with SESSION_AGENT_CACHE_LOCK:
            SESSION_AGENT_CACHE[sid] = (agent, "sig")
        with patch("app.domain.streaming.get_session", side_effect=KeyError(sid)):
            handler = _make_handler()
            _handle_chat_steer(handler, {"session_id": sid, "text": "hint"})
        body = _captured_response(handler)
        assert body["accepted"] is False
...
class TestRouting:
    ...
    def test_route_registered(self):
        src = (Path(__file__).parent.parent / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
...
class TestFrontendWiring:
    ...
    @classmethod
    def setup_class(cls):
        cls.cmds = (Path(__file__).parent.parent / "static-legacy" / "commands.js").read_text(encoding="utf-8")
        cls.msgs = (Path(__file__).parent.parent / "static-legacy" / "messages.js").read_text(encoding="utf-8")
        cls.i18n = (Path(__file__).parent.parent / "static-legacy" / "i18n.js").read_text(encoding="utf-8")
...
class TestLeftoverDelivery:
    ...
    def test_leftover_drain_call_in_streaming(self):
        ...
        src = (Path(__file__).parent.parent / "app" / "domain" / "streaming.py").read_text(encoding="utf-8")
