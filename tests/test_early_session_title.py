...
def test_prompt_provisional_title_still_counts_as_provisional_after_response():
    ...
    from app.domain.streaming import _is_provisional_title
...
def test_attachment_decorated_message_still_counts_as_provisional():
    from app.domain.streaming import _is_provisional_title
    ...
    messages = [
        ...
        {"role": "assistant", "content": "เห็นเป็นโลโก้ F-Flow"},
    ]
    provisional = "เห็นอะไร"
...
def test_thai_fallback_uses_assistant_opening_when_llm_fails():
    ...
    assert title == 'สวัสดีค่ะ มีอะไรให้ช่วยไหมคะ'


def test_messages_js_applies_chat_start_title():
    src = Path("static-legacy/messages.js").read_text(encoding="utf-8")
