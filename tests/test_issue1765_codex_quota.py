from app.domain import streaming
...
def test_frontend_renders_apperror_details_in_collapsible_block():
    messages_js = (streaming.Path(__file__).resolve().parent.parent / 'static-legacy' / 'messages.js').read_text()
    ui_js = (streaming.Path(__file__).resolve().parent.parent / 'static-legacy' / 'ui.js').read_text()
    style_css = (streaming.Path(__file__).resolve().parent.parent / 'static-legacy' / 'style.css').read_text()
