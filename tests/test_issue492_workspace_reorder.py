...
from app.domain.routes import _handle_workspace_reorder
...
class TestWorkspaceReorderEndpoint:
    ...
    @patch("app.domain.routes.save_workspaces")
    @patch("app.domain.routes.load_workspaces")
...
class TestWorkspaceReorderFrontend:
    ...
    def test_i18n_keys_present_in_all_locales(self):
        ...
        with open("static-legacy/i18n.js", "r", encoding="utf-8") as f:
            content = f.read()
    ...
    def test_grip_vertical_icon_exists(self):
        with open("static-legacy/icons.js", "r", encoding="utf-8") as f:
            content = f.read()
        assert "'grip-vertical'" in content

    def test_renderWorkspacesPanel_has_drag_attrs(self):
        with open("static-legacy/panels.js", "r", encoding="utf-8") as f:
            content = f.read()
        for attr in ("dragstart", "dragover", "dragend",
                      "ws-drag-handle", "/api/workspaces/reorder", "singleWorkspace"):
            assert attr in content, f"Missing: {attr}"

    def test_css_drag_classes_exist(self):
        with open("static-legacy/style.css", "r", encoding="utf-8") as f:
            content = f.read()
