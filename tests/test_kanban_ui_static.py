...
STYLE = (ROOT / "static-legacy" / "style.css").read_text(encoding="utf-8")
...
def test_kanban_has_sidebar_panel_and_main_board_mounts():
    ...
    assert 'id="mainKanban" class="main-view' in INDEX
...
def test_kanban_new_task_header_button_opens_modal():
    ...
    assert '<select id="kanbanTaskModalPriority">' in INDEX
    ...
    assert "_kanbanPopulatePrioritySelect" in PANELS
...
def test_kanban_dispatcher_inflight_guard_prevents_double_click_toast_confusion():
    ...
    assert 'id="kanbanBoardTabs"' in INDEX
    assert 'kanban_dispatch' in PANELS or 'kanban_dispatch' in INDEX
...
def test_kanban_modal_locale_parity():
    ...
    modal_keys = [
        ...
        "kanban_priority",
        "kanban_priority_low",
        "kanban_priority_normal",
        "kanban_priority_high",
        "kanban_priority_urgent",
...
def test_kanban_lifecycle_controls_do_not_offer_manual_running_start():
    ...
    assert 'id="btnKanbanRunDispatcher"' in INDEX
    assert 'data-i18n="kanban_dispatch"' in INDEX
...
const fs = require('fs');
const vm = require('vm');
const src = fs.readFileSync('static-legacy/panels.js', 'utf8');
function esc(value) {
...
def test_kanban_board_switcher_handlers_in_panels():
    ...
    for fn in (
        ...
        "function _renderKanbanBoardTabs",
