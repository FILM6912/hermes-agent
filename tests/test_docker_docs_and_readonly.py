...
def test_single_container_workspace_already_uses_home_env_var():
    ...
    so it doesn't drift back. Workspace mounts onto ~/.hermes/workspace so
    HERMES_WORKSPACE in .env is the single host-side control."""
    src = (REPO / "docker-compose.yml").read_text(encoding="utf-8")
    assert "${HERMES_WORKSPACE:-${HOME}/workspace}:/home/hermeswebui/.hermes/workspace" in src
    assert "${HERMES_HOME:-${HOME}/.hermes}:/home/hermeswebui/.hermes" in src
