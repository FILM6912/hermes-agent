...
def test_same_session_profile_switch_rebuilds_agent_under_new_soul_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("app.domain.config.get_config", lambda: {})
    monkeypatch.setattr("app.domain.config._resolve_cli_toolsets", lambda _cfg: [])
    monkeypatch.setattr("app.domain.config.load_settings", lambda: {})
