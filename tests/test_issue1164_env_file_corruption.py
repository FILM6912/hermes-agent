...
- onboarding.py now imports _write_env_file from providers.py (which holds
  _ENV_LOCK from app.domain.streaming for the entire load→modify→write cycle).
...
class TestEnvFileCommentPreservation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from app.domain.providers import _write_env_file
...
class TestOnboardingUsesProviderWriteEnv(unittest.TestCase):
    ...
    def test_onboarding_imports_write_env_from_providers(self):
        """app.domain.onboarding._write_env_file must be the same object as
        api.providers._write_env_file (shared implementation with lock)."""
        from app.domain import onboarding, providers
        self.assertIs(
    ...
    def test_providers_write_env_holds_env_lock(self):
        """providers._write_env_file must acquire _ENV_LOCK from app.domain.streaming."""
        import inspect
        from app.domain.providers import _write_env_file
        ...
        self.assertIn("from app.domain.streaming import _ENV_LOCK", source,
                      "_ENV_LOCK must be imported from app.domain.streaming")
