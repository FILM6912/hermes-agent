...
def _csrf_host():
    """Host header matching the isolated test server (includes port)."""
...
class TestCSRF:
    ...
    def _csrf_allowed(headers):
        ...
        from app.domain.routes import _check_csrf
    ...
    def test_cross_origin_post_rejected(self):
        ...
        body, status = post(
            "/api/session/new",
            {},
            headers={"Origin": "http://evil.com", "Host": _csrf_host()},
        )
    ...
    def test_same_origin_post_allowed(self):
        ...
        body, status = post(
            "/api/session/new",
            {},
            headers={"Origin": f"http://{_csrf_host()}", "Host": _csrf_host()},
        )
    ...
    def test_same_origin_referer_allowed(self):
        ...
        body, status = post(
            "/api/session/new",
            {},
            headers={"Referer": f"http://{_csrf_host()}/", "Host": _csrf_host()},
        )
...
class TestCSRFHelpers:
    ...
    def test_normalize_host_only(self):
        from app.domain.routes import _normalize_host_port
    ...
    def test_ports_match_identical(self):
        from app.domain.routes import _ports_match
...
class TestLoginRateLimit:
    def test_rate_limit_triggers_429(self):
        ...
        from app.domain.auth import _login_attempts, _LOGIN_WINDOW
        ...
        import time
        from app.domain import auth as _auth
...
class TestSanitizeError:
    def test_unix_path_stripped(self):
        from app.domain.helpers import _sanitize_error
...
class TestHMACLength:
    def test_session_token_sig_is_64_chars(self):
        ...
        from app.domain.auth import create_session
    ...
    def test_verify_session_rejects_old_16char_sig(self):
        ...
        import hashlib
        from app.domain.auth import _signing_key, verify_session, _sessions
        import time
        import secrets
...
class TestContentDisposition:
    def test_html_file_forced_download(self, webui_server, tmp_path):
        ...
        from app.domain.routes import _handle_file_raw
    ...
    def test_dangerous_mime_types_set_complete(self):
        ...
        import ast
        import pathlib
        routes_src = pathlib.Path(__file__).parent.parent / "app" / "domain" / "routes.py"
...
class TestPasswordHashing:
    ...
    def test_save_settings_stores_64char_hex_hash(self):
        ...
        from app.domain.config import save_settings, load_settings, SETTINGS_FILE
        import json
...
class TestSSRFCheck:
    def test_ssrf_guard_code_present_in_config(self):
        ...
        src = pathlib.Path(__file__).parent.parent / "app" / "domain" / "config.py"
        text = src.read_text()
...
class TestENVLock:
    ...
    def test_env_lock_importable_in_routes(self):
        """app.domain.routes must be able to import _ENV_LOCK from app.domain.streaming."""
        # If routes.py fails to import, this will raise ImportError
        import importlib
        import app.domain.routes  # noqa: F401 -- just checking import works
