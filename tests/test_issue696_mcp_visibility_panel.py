...
def test_mcp_panel_renders_status_badges_tool_counts_and_empty_error_states():
    js = read("static-legacy/panels.js")
    ...
    assert "profile-detail-tabs" in js
    assert "loadProfileMcpServers" in js
    assert "saveProfileMcpForm" in js
    assert "switchProfileMcpInputMode" in js
    assert "profileMcpJson" in js
    assert "mcp-server-error" in js
    assert "deleteProfileMcpServer" in js
