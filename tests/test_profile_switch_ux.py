...
class TestProfileSwitchSpinner:
    ...
    JS = (REPO_ROOT / "static-legacy" / "panels.js").read_text(encoding="utf-8")
...
class TestParallelizedFetches:
    ...
    def test_workspace_load_is_awaited_only_when_visible(self):
        """Profile switches should refresh the tree via a shared helper."""
        fn = self._get_switch_fn()
        assert "_refreshWorkspaceAfterProfileSwitch(_switchGen)" in fn
        assert "awaitWorkspaceLoad: false" in fn
        sessions_js = (REPO_ROOT / "static-legacy" / "sessions.js").read_text(encoding="utf-8")
        assert "if(options&&options.awaitWorkspaceLoad) await dirLoad" in sessions_js

    def test_refresh_helper_clears_stale_tree_before_reload(self):
        panels_js = self.JS
        assert "function _refreshWorkspaceAfterProfileSwitch" in panels_js
        start = panels_js.index("async function _refreshWorkspaceAfterProfileSwitch")
        end = panels_js.index("function _refreshProfileSwitchBackground", start)
        fn = panels_js[start:end]
        assert "S.entries = []" in fn
        assert "loadDir('.')" in fn


class TestSpinnerCss:
    """Verify the spinner CSS class is defined correctly."""

    CSS = (REPO_ROOT / "static-legacy" / "style.css").read_text(encoding="utf-8")
...
class TestProfileSessionListFlip:
    ...
    JS = (REPO_ROOT / "static-legacy" / "sessions.js").read_text(encoding="utf-8")
    CSS = (REPO_ROOT / "static-legacy" / "style.css").read_text(encoding="utf-8")
