...
def _reload_profiles_module(base_home: Path):
    ...
    _saved = {name: sys.modules[name] for name in ["app.domain.config", "app.domain.profiles"]
              if name in sys.modules}

    for name in ["app.domain.config", "app.domain.profiles"]:
        if name in sys.modules:
            del sys.modules[name]

    profiles = importlib.import_module("app.domain.profiles")

    # Restore original modules and package attributes so the cache stays
    # consistent for the rest of the suite.
    sys.modules.update(_saved)
    domain_pkg = sys.modules.get("app.domain")
    if domain_pkg is not None:
        for name, module in _saved.items():
            setattr(domain_pkg, name.rsplit(".", 1)[-1], module)
