...
def test_git_status_non_git_workspace(tmp_path):
    from app.domain.workspace_git import git_status
...
def test_git_status_reports_untracked_files_inside_directories(tmp_path):
    from app.domain.workspace_git import git_discard, git_status
...
def test_git_diff_large_untracked_file_is_bounded(tmp_path):
    from app.domain.workspace_git import DIFF_SIZE_LIMIT, git_diff, git_status
...
def test_git_stage_unstage_discard_and_commit(tmp_path):
    from app.domain.workspace_git import git_commit, git_discard, git_stage, git_status, git_unstage
...
def test_git_commit_selected_ignores_unrelated_real_index(tmp_path):
    from app.domain.workspace_git import git_commit_selected, git_status
...
def test_git_commit_selected_handles_untracked_and_mixed_paths(tmp_path):
    from app.domain.workspace_git import git_commit_selected
...
def test_selected_commit_message_prompt_uses_selected_diff(tmp_path):
    from app.domain.workspace_git import selected_commit_message_prompt
...
def test_git_fetch_pull_and_push_with_upstream(tmp_path):
    from app.domain.workspace_git import git_fetch, git_pull, git_push, git_status
...
def test_git_branches_lists_local_remote_and_upstream(tmp_path):
    from app.domain.workspace_git import git_branches
...
def test_git_checkout_local_new_remote_dirty_and_invalid_refs(tmp_path):
    from app.domain.workspace_git import GitWorkspaceError, git_branches, git_checkout
...
def test_git_checkout_detached_requires_explicit_mode(tmp_path):
    from app.domain.workspace_git import git_branches, git_checkout
...
def test_git_stash_and_checkout_is_explicit(tmp_path):
    from app.domain.workspace_git import git_stash_and_checkout, git_status
...
def test_git_stash_and_checkout_reports_restore_conflicts_without_dropping_stash(tmp_path):
    from app.domain.workspace_git import git_stash_and_checkout
...
def test_git_env_scrub_removes_redirecting_vars_and_preserves_temp_index(monkeypatch):
    from app.domain.workspace_git import _clean_git_env
...
def test_git_error_classifier_identifies_non_fast_forward_push():
    from app.domain.workspace_git import _classify_git_error
...
def test_git_commit_hook_failure_returns_hook_failed_code(tmp_path):
    from app.domain.workspace_git import GitWorkspaceError, git_commit, git_stage
...
def test_destructive_workspace_git_flag_defaults_off_and_accepts_truthy(monkeypatch):
    from app.domain.workspace_git import WORKSPACE_GIT_DESTRUCTIVE_ENV, workspace_git_destructive_enabled
...
def test_git_commit_route_rejects_active_stream(monkeypatch, tmp_path):
    from app.domain import routes
    from app.domain.config import STREAMS, STREAMS_LOCK
    from app.domain.workspace_git import WORKSPACE_GIT_DESTRUCTIVE_ENV
