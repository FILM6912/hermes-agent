...
def _set_paths(monkeypatch, tmp_path):
    import app.domain.passkeys as passkeys
...
def test_auth_status_reports_passkey_availability_source_contract():
    src = open("app/domain/auth.py", encoding="utf-8").read()
    ...
    assert "def get_auth_status_payload" in src


def test_v1_auth_status_and_login_use_auth_service():
    auth_ep = open("app/api/v1/endpoints/auth.py", encoding="utf-8").read()
    assert "AuthService" in auth_ep
    assert "_service.get_status(" in auth_ep
    assert "_service.login(" in auth_ep
    assert '"/api/auth/status"' not in auth_ep
    assert '"/api/auth/login"' not in auth_ep


def test_v1_auth_mutations_use_auth_service():
    auth_ep = open("app/api/v1/endpoints/auth.py", encoding="utf-8").read()
    assert "dispatch_legacy_route" not in auth_ep
    assert "_service.logout(" in auth_ep
    assert "_service.passkey_authentication_options(" in auth_ep
    assert "_service.passkey_login(" in auth_ep
    assert "_service.passkey_registration_options(" in auth_ep
    assert "_service.passkey_register(" in auth_ep
    assert "_service.passkey_delete(" in auth_ep
    assert "_service.list_passkeys(" in auth_ep
    assert '"/api/auth/logout"' not in auth_ep
    assert '"/api/auth/passkey/options"' not in auth_ep


def test_login_page_has_default_hidden_passkey_button_and_script_wiring():
    routes = open("app/domain/routes.py", encoding="utf-8").read()
    login_js = open("static-legacy/login.js", encoding="utf-8").read()
    assert 'id="passkey-login"' in routes and 'style="display:none"' in routes
    assert 'id="username"' in routes
    assert "{{LOGIN_USERNAME_ATTRS}}" in routes
    assert "api/auth/passkey/options" in login_js
    assert "navigator.credentials.get" in login_js
    assert "s.multi_user" in login_js
    assert "username: uname" in login_js
...
def test_passkey_feature_flag_via_config(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(app.domain.config, "get_config", lambda: {"webui_passkey_enabled": True})
    assert auth.are_passkeys_enabled() is True


def test_passwordless_settings_and_last_passkey_guard_are_wired():
    routes = open("app/domain/routes.py", encoding="utf-8").read()
    panels = open("static-legacy/panels.js", encoding="utf-8").read()
