...
def test_current_webui_version_returns_runtime_version(with_runtime_version):
    ...
    from app.domain.config import _current_webui_version
...
def test_current_webui_version_returns_none_when_module_missing(monkeypatch):
    ...
    """
    monkeypatch.delitem(sys.modules, "app.domain.updates", raising=False)
    from app.domain.config import _current_webui_version
...
def test_is_valid_models_cache_remains_shape_only():
    ...
    from app.domain.config import _is_valid_models_cache
    cache = _shape_cache()
...
def test_is_loadable_disk_cache_rejects_non_dict():
    ...
    from app.domain.config import _is_loadable_disk_cache
...
def test_save_models_cache_to_disk_skipped_when_disabled(isolated_cache, with_runtime_version, monkeypatch):
    """HERMES_WEBUI_MODELS_DISK_CACHE=0 must not write models_cache.json."""
    from app.domain import config

    with_runtime_version("v0.50.293")
    monkeypatch.setattr(config, "_MODELS_DISK_CACHE_ENABLED", False)
    config._save_models_cache_to_disk(_shape_cache())
    assert not isolated_cache.exists()


def test_load_models_cache_from_disk_ignored_when_disabled(isolated_cache, with_runtime_version, monkeypatch):
    """HERMES_WEBUI_MODELS_DISK_CACHE=0 must not serve stale on-disk groups."""
    from app.domain import config
