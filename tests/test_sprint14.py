...
def test_file_move_across_directories():
    """Moving a file into another folder updates its path on disk."""
    created = []
    try:
        sid, sess = make_session(created)
        post("/api/file/create-dir", {"session_id": sid, "path": "dest_folder"})
        post("/api/file/create", {"session_id": sid, "path": "move_me.txt", "content": "hello"})
        ...
        assert status == 200
        assert d["ok"] is True
        assert d["new_path"] == "dest_folder/move_me.txt"
    finally:
        for s in created:
            post("/api/session/delete", {"session_id": s})


def test_file_move_rejects_folder_into_child():
    """Cannot move a folder into itself or its descendants."""
    created = []
    try:
        sid, sess = make_session(created)
        post("/api/file/create-dir", {"session_id": sid, "path": "parent/child"})
        d, status = post("/api/file/move", {
            "session_id": sid,
            "path": "parent",
