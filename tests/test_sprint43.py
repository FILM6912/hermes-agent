...
_FASTAPI_MIGRATION = APP_MAIN.exists()
GATEWAY_WATCHER_PY = (REPO_ROOT / "app" / "domain" / "gateway_watcher.py").read_text()
CONFIG_PY = (REPO_ROOT / "app" / "domain" / "config.py").read_text()
BOOTSTRAP_PY = (REPO_ROOT / "bootstrap.py").read_text()
SERVER_PY = (REPO_ROOT / "server.py").read_text()
ROUTES_PY = (REPO_ROOT / "app" / "domain" / "routes.py").read_text()
AUTH_PY = (REPO_ROOT / "app" / "domain" / "auth.py").read_text()
PROFILES_PY = (REPO_ROOT / "app" / "domain" / "profiles.py").read_text()
STREAMING_PY = (REPO_ROOT / "app" / "domain" / "streaming.py").read_text()
WORKSPACE_PY = (REPO_ROOT / "app" / "domain" / "workspace.py").read_text()
STATE_SYNC_PY = (REPO_ROOT / "app" / "domain" / "state_sync.py").read_text()
...
class TestBareExceptLogging(unittest.TestCase):
    ...
    MODULES_REQUIRING_LOGGER = [
        ("app/domain/auth.py", AUTH_PY),
        ("app/domain/config.py", CONFIG_PY),
        ("app/domain/gateway_watcher.py", GATEWAY_WATCHER_PY),
        ("app/domain/profiles.py", PROFILES_PY),
        ("app/domain/streaming.py", STREAMING_PY),
        ("app/domain/workspace.py", WORKSPACE_PY),
        ("app/domain/state_sync.py", STATE_SYNC_PY),
        ("app/domain/routes.py", ROUTES_PY),
    ]
...
class TestQuietHTTPServer(unittest.TestCase):
    ...
    @unittest.skipIf(_FASTAPI_MIGRATION, "FastAPI migration: uvicorn is the primary test server")
...
class TestFastAPIServerMigration(unittest.TestCase):
    """When app.main exists, the FastAPI/uvicorn entrypoint replaces ThreadingHTTPServer."""

    @unittest.skipUnless(_FASTAPI_MIGRATION, "app/main.py not present yet")
    def test_app_main_exports_fastapi_app(self):
        src = APP_MAIN.read_text()
        self.assertIn("FastAPI", src, "app/main.py must define a FastAPI application")
        self.assertIn("app =", src, "app/main.py must export an ASGI app named app")

    @unittest.skipUnless(_FASTAPI_MIGRATION, "app/main.py not present yet")
    def test_conftest_prefers_uvicorn_launch(self):
        conftest_src = (REPO_ROOT / "tests" / "conftest.py").read_text()
        self.assertIn("uvicorn", conftest_src, "tests/conftest.py must launch uvicorn when app.main exists")
        self.assertIn("app.main:app", conftest_src, "tests/conftest.py must target app.main:app")
...
class TestSessionTitleRedaction(unittest.TestCase):
    ...
    def test_redact_text_imported_in_routes(self):
        """routes.py must import _redact_text from app.domain.helpers."""
        self.assertIn(
            "_redact_text",
            ROUTES_PY,
            "routes.py: _redact_text must be imported from app.domain.helpers",
        )
