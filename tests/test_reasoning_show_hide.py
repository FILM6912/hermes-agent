...
class TestBootJsShowThinking:
    ...
    def test_settings_path_initialises_show_thinking(self):
        src = read('static-legacy/boot.js')
...
class TestMessagesJsLiveThinkingGate:
    ...
    def test_live_thinking_gated(self):
        src = read('static-legacy/messages.js')
        assert 'window._showThinking===false' in src, (
...
class TestReasoningCommand:
    ...
    def test_reasoning_in_commands_array(self):
        src = read('static-legacy/commands.js')
    ...
    def test_cmd_reasoning_i18n_key_exists(self):
        i18n = read('static-legacy/i18n.js')
...
class TestReasoningConfigHelpers:
    ...
    def test_parse_reasoning_effort_matches_cli_semantics(self):
        from app.domain.config import parse_reasoning_effort, VALID_REASONING_EFFORTS
        # Empty → None
    ...
    def test_valid_reasoning_efforts_matches_hermes_constants(self):
        ...
        from app.domain.config import VALID_REASONING_EFFORTS
    ...
    def test_get_reasoning_status_defaults_to_show_true(self, tmp_path, monkeypatch):
        ...
        assert st['reasoning_effort'] is None
...
class TestSessionMessageReasoningApiShape:
    """GET /api/session assistant rows must use null when no reasoning trace."""

    def test_redact_session_data_normalizes_missing_reasoning_to_null(self, monkeypatch):
        import app.domain.helpers as helpers
        ...
        assert out["messages"][1]["reasoning"] is None
        assert out["messages"][2]["reasoning"] == "thinking text"

    def test_redact_session_data_drops_reasoning_content_key(self, monkeypatch):
        import app.domain.helpers as helpers
        ...
        out = helpers.redact_session_data(
            {
                "messages": [
                    {
                        "role": "assistant",
        ...
        assert out["messages"][0]["reasoning"] == "planning step"
        assert "reasoning_content" not in out["messages"][0]

    def test_redact_session_data_adds_reasoning_null_when_key_missing(self, monkeypatch):
        import app.domain.helpers as helpers
        ...
        out = helpers.redact_session_data(
            {"messages": [{"role": "assistant", "content": "only text"}]},
        )
