...
def _restore_auth_sessions():
    ...
    """
    import app.domain.auth as _auth
...
class TestGetTitleRefreshInterval:
    def test_returns_int_for_valid_setting(self):
        # _get_title_refresh_interval does a local import: `from app.domain.config import load_settings`
        # so patch the source module, not api.streaming
        with patch('app.domain.config.load_settings', return_value={'auto_title_refresh_every': '5'}):
            assert _get_title_refresh_interval() == 5

    def test_returns_zero_for_off_setting(self):
        with patch('app.domain.config.load_settings', return_value={'auto_title_refresh_every': '0'}):
            assert _get_title_refresh_interval() == 0

    def test_returns_zero_when_key_absent(self):
        with patch('app.domain.config.load_settings', return_value={}):
            assert _get_title_refresh_interval() == 0

    def test_returns_zero_on_exception(self):
        with patch('app.domain.config.load_settings', side_effect=Exception('boom')):
            assert _get_title_refresh_interval() == 0

    def test_valid_values_10_and_20(self):
        for val in ('10', '20'):
            with patch('app.domain.config.load_settings', return_value={'auto_title_refresh_every': val}):
...
class TestRunBackgroundTitleRefresh:
    ...
    def test_skips_when_title_changed_before_call(self):
        ...
        with patch('app.domain.streaming.get_session') as mock_get, \
    ...
    def test_skips_if_session_not_found(self):
        put, events = self._make_put_event()
        with patch('app.domain.streaming.get_session', side_effect=KeyError('not found')):
    ...
    def test_emits_title_event_on_new_title(self):
        ...
        fake_sessions = {'sid': s}
        with patch('app.domain.streaming.get_session', return_value=s), \
             patch('app.domain.streaming._aux_title_configured', return_value=True), \
             patch('app.domain.streaming._generate_llm_session_title_via_aux',
                   return_value=('New Refreshed Title', 'llm_ok', 'raw')), \
             patch('app.domain.streaming.SESSIONS', fake_sessions), \
             patch('app.domain.streaming.LOCK', threading.Lock()):
            _run_background_title_refresh('sid', 'u', 'a', 'Old Title', put)
    ...
    def test_saves_refreshed_title_outside_global_lock(self):
        ...
        with patch('app.domain.streaming.get_session', return_value=s), \
             ...
             patch('app.domain.streaming._generate_llm_session_title_via_aux',
                   return_value=('New Refreshed Title', 'llm_ok', 'raw')), \
             patch('app.domain.streaming.SESSIONS', fake_sessions), \
             patch('app.domain.streaming.LOCK', lock):
            _run_background_title_refresh('sid', 'u', 'a', 'Old Title', put)
        title_events = [(n, d) for n, d in events if n == 'title']
        assert len(title_events) == 1
        assert title_events[0][1]['title'] == 'New Refreshed Title'
    ...
    def test_exceptions_are_silently_swallowed(self):
        ...
        put, events = self._make_put_event()
        with patch('app.domain.streaming.get_session', side_effect=RuntimeError('oops')):
            # Should not raise
            _run_background_title_refresh('sid', 'u', 'a', 'title', put)
        assert events == []
...
class TestMaybeScheduleTitleRefresh:
    ...
    def test_does_nothing_when_disabled(self):
        with patch('app.domain.streaming._get_title_refresh_interval', return_value=0):
            spawned = []
    ...
    def test_does_nothing_when_title_is_empty(self):
        with patch('app.domain.streaming._get_title_refresh_interval', return_value=5):
            spawned = []
            with patch('threading.Thread', side_effect=lambda **kw: spawned.append(kw) or MagicMock()):
    ...
    def test_does_nothing_when_no_exchange_content(self):
        ...
        with patch('app.domain.streaming._get_title_refresh_interval', return_value=5), \
             patch('app.domain.streaming._latest_exchange_snippets', return_value=('', '')):
            spawned = []
            with patch('threading.Thread', side_effect=lambda **kw: spawned.append(kw) or MagicMock()):
                session = _make_session(messages=[_user_msg('q'), _asst_msg('a')] * 5)
                _maybe_schedule_title_refresh(session, self._noop_put, None)
