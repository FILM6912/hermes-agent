...
class TestAuxTitleConfigured(unittest.TestCase):
    def _call(self, tg_config):
        from app.domain.streaming import _aux_title_configured
...
class TestGenerateTitleRawViaAuxTimeout(unittest.TestCase):
    ...
    def _run_with_config(self, tg_config, expected_timeout):
        from app.domain.streaming import generate_title_raw_via_aux
...
class TestTitleAuxExtraBody(unittest.TestCase):
    """Auxiliary title calls must not send unsupported extra_body to strict providers."""

    def test_glm_route_omits_reasoning_extra_body(self):
        from app.domain.streaming import _title_aux_extra_body

        self.assertEqual(
            _title_aux_extra_body('zai', 'glm-5-turbo', 'https://api.z.ai/api/paas/v4'),
            {},
        )

    def test_openrouter_route_disables_reasoning(self):
        from app.domain.streaming import _title_aux_extra_body

        self.assertEqual(
            _title_aux_extra_body('openrouter', 'qwen/qwen3-32b', 'https://openrouter.ai/api/v1'),
            {'reasoning': {'enabled': False}},
        )

    def test_generate_title_raw_via_aux_omits_reasoning_for_glm(self):
        from app.domain.streaming import generate_title_raw_via_aux

        mock_resp = types.SimpleNamespace(
            choices=[
                types.SimpleNamespace(
                    message=types.SimpleNamespace(content='Thai Greeting'),
                    finish_reason='stop',
                )
            ]
        )
        ...
        with patch('agent.auxiliary_client.call_llm', side_effect=fake_call_llm, create=True):
            result, status = generate_title_raw_via_aux(
                user_text='สวัสดี',
                assistant_text='สวัสดีค่ะ มีอะไรให้ช่วยไหมคะ',
                provider='zai',
                model='glm-5-turbo',
                base_url='https://api.z.ai/api/paas/v4',
            )

        self.assertEqual(result, 'Thai Greeting')
        self.assertEqual(status, 'llm_aux')
...
class TestReasoningModelTitleGeneration(unittest.TestCase):
    ...
    def test_title_budget_defaults_to_reasoning_safe_value(self):
        ...
        from app.domain.streaming import _title_completion_budget, _title_retry_completion_budget
    ...
    def test_agent_route_short_circuits_on_empty_reasoning_without_retrying(self):
        ...
        from app.domain.streaming import generate_title_raw_via_agent
...
class TestBackgroundTitleProfileRouting(unittest.TestCase):
    def test_profile_env_context_logs_fail_open_resolution_errors(self):
        ...
        with patch.object(
            ...
            with patch.dict(os.environ, {'HERMES_HOME': 'default-home'}, clear=False):
                with self.assertLogs('app.domain.profiles', level='DEBUG') as logs:
    ...
    def test_background_title_generation_uses_session_profile_home(
        ...
        events = []
        try:
            with patch('app.domain.profiles.get_hermes_home_for_profile', return_value='profile-home'):
                with patch('app.domain.streaming._generate_llm_session_title_via_aux', side_effect=fake_aux_title):
                    with patch.dict(os.environ, {'HERMES_HOME': 'default-home'}, clear=False):
                        _run_background_title_update(
    ...
    def test_background_profile_env_routes_load_config_and_provider_credentials(self):
        ...
        import app.domain.profiles as profiles
        from app.domain.config import _thread_ctx
        try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('app.domain.profiles.get_hermes_home_for_profile', return_value=profile_home):
                runtime_env = {
                    'PROFILE_ONLY_KEY': 'profile-only',
                    'OPENROUTER_API_KEY': 'profile-openrouter-key',
                }
                with patch('app.domain.profiles.get_profile_runtime_env', return_value=runtime_env):
...
class TestAuxTitleTimeoutEdgeCases(unittest.TestCase):
    ...
    def _call(self, tg_config, default=15.0):
        from app.domain.streaming import _aux_title_timeout
        with _patch_tg_config(tg_config):
...
class TestSessionModelTitleRouting(unittest.TestCase):
    """Session chat model is preferred over auxiliary.title_generation config.

    Pins the behaviour so the routing tuple in _run_background_title_update
    stays synchronised with _generate_session_title_llm.
    """
    ...
    @patch('app.domain.streaming.get_session')
    def test_agent_failure_falls_back_to_configured_aux(
        self, mock_get_session, mock_agent_title, mock_aux_title, mock_configured,
    ):
        """When the session agent route fails, configured aux title_generation is tried."""
        from app.domain.streaming import _run_background_title_update
        ...
        mock_agent = MagicMock(provider='openai', model='gpt-4o', base_url='https://api.openai.com/v1')
        mock_agent_title.return_value = (None, 'llm_error', '')
        mock_aux_title.side_effect = [
            (None, 'llm_error_aux', ''),
            ('Weather Report', 'llm_aux', 'Weather Report'),
        ]
        ...
        with patch(
            'app.domain.streaming._get_aux_title_config',
            return_value={'provider': 'custom', 'model': 'glm-5-turbo', 'base_url': 'https://api.z.ai/v4'},
        ):
            _run_background_title_update(
    ...
    @patch('app.domain.streaming._generate_llm_session_title_for_agent')
    @patch('app.domain.streaming.get_session')
    def test_agent_failure_tries_session_model_aux_before_configured_aux(
        self, mock_get_session, mock_agent_title, mock_aux_title, mock_configured,
    ):
        """Session-model aux route is tried before dedicated title_generation config."""
        from app.domain.streaming import _run_background_title_update
        ...
        mock_agent_title.return_value = (None, 'llm_error', '')
        mock_aux_title.side_effect = [
            ('Chicken Joke', 'llm_aux', 'Chicken Joke'),
        ]
        ...
        with patch(
            ...
            _run_background_title_update(
                ...
                placeholder_title='Untitled',
                put_event=lambda event_type, data: None,
                agent=mock_agent,
            )

        mock_agent_title.assert_called_once()
        mock_aux_title.assert_called_once()
        _, kwargs = mock_aux_title.call_args
        self.assertTrue(kwargs.get('use_agent_model'))

    @patch('app.domain.streaming._aux_title_configured', return_value=True)
    @patch('app.domain.streaming._generate_llm_session_title_via_aux')
    @patch('app.domain.streaming._generate_llm_session_title_for_agent')
    @patch('app.domain.streaming.get_session')
    def test_agent_success_skips_aux_routes(
        self, mock_get_session, mock_agent_title, mock_aux_title, mock_configured,
    ):
        """When the session agent route succeeds, aux routes must NOT be called."""
        from app.domain.streaming import _run_background_title_update
        ...
        mock_get_session.return_value = mock_session
        mock_agent_title.return_value = ('Chat Summary', 'llm', 'Chat Summary')

        events = []

        _run_background_title_update(
            session_id='empty-reasoning-session',
            user_text='Summarize this chat.',
        ...
        title_events = [data for event_type, data in events if event_type == 'title']
        self.assertEqual(title_events[0]['title'], 'Chat Summary')

    @patch('app.domain.streaming._aux_title_configured', return_value=True)
    @patch('app.domain.streaming._generate_llm_session_title_via_aux')
    @patch('app.domain.streaming._generate_llm_session_title_for_agent')
    def test_same_session_and_configured_route_skips_duplicate_aux_call(
        self, mock_agent_title, mock_aux_title, mock_configured,
    ):
        """When session model matches auxiliary.title_generation, do not hit the same route twice."""
        from app.domain.streaming import _generate_session_title_llm

        mock_agent = MagicMock(provider='custom', model='glm-5-turbo', base_url='https://api.z.ai/v4')
        mock_agent_title.return_value = (None, 'llm_error', '')
        mock_aux_title.return_value = ('Thai Greeting Topic', 'llm_aux', 'Thai Greeting Topic')

        with patch(
            'app.domain.streaming._get_aux_title_config',
            return_value={'provider': 'custom', 'model': 'glm-5-turbo', 'base_url': 'https://api.z.ai/v4'},
        ):
            title, status, _ = _generate_session_title_llm(
                'สวัสดี',
                'สวัสดีค่ะ มีอะไรให้ช่วยไหมคะ',
                agent=mock_agent,
            )

        self.assertEqual(title, 'Thai Greeting Topic')
        mock_agent_title.assert_called_once()
        mock_aux_title.assert_called_once()
        self.assertFalse(mock_aux_title.call_args.kwargs.get('use_agent_model'))
        self.assertEqual(status, 'llm_aux')

    @patch('app.domain.streaming._aux_title_configured', return_value=True)
    @patch('app.domain.streaming._generate_session_title_llm')
    @patch('app.domain.streaming.get_session')
    def test_thai_local_fallback_persisted_when_llm_fails(
        self, mock_get_session, mock_title_llm, mock_configured,
    ):
        """Thai exchanges should still get a readable title when every LLM route fails."""
        from app.domain.streaming import _run_background_title_update

        user_text = 'สวัสดี'
        assistant_text = 'สวัสดีค่ะ มีอะไรให้ช่วยไหมคะ'
        mock_session = MagicMock()
        mock_session.title = user_text
        mock_session.llm_title_generated = False
        mock_session.messages = [
        ...
        mock_get_session.return_value = mock_session
        mock_title_llm.return_value = (None, 'llm_error_aux', '')

        events = []

        _run_background_title_update(
            session_id='thai-fallback-session',
            user_text=user_text,
            assistant_text=assistant_text,
            placeholder_title=user_text,
            put_event=lambda event_type, data: events.append((event_type, data)),
            agent=MagicMock(),
        )

        self.assertEqual(mock_session.title, assistant_text)
        title_events = [data for event_type, data in events if event_type == 'title']
        self.assertEqual(title_events[0]['title'], assistant_text)

    @patch('app.domain.streaming._aux_title_configured', return_value=True)
    @patch('app.domain.streaming._generate_llm_session_title_via_aux')
    @patch('app.domain.streaming._generate_llm_session_title_for_agent')
    @patch('app.domain.streaming.get_session')
    def test_configured_aux_not_used_when_agent_succeeds(
        self, mock_get_session, mock_agent_title, mock_aux_title, mock_configured,
    ):
        """Configured auxiliary.title_generation must not override a working session model."""
        from app.domain.streaming import _run_background_title_update
        ...
        mock_agent_title.return_value = ('Greeting', 'llm', 'Greeting')
        mock_aux_title.return_value = ('Configured Aux Title', 'llm_aux', '')
