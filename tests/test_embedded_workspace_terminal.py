...
def test_terminal_is_opened_by_slash_command_not_permanent_composer_icon():
    ...
    commands_js = _read("static-legacy/commands.js")
    ...
    assert 'static-legacy/terminal.js' in html
...
def test_terminal_surface_uses_composer_flyout_card_pattern():
    html = _read("static/index.html")
    style_css = _read("static-legacy/style.css")
...
def test_terminal_uses_controlled_desktop_resize_handle():
    html = _read("static/index.html")
    style_css = _read("static-legacy/style.css")
    terminal_js = _read("static-legacy/terminal.js")
...
def test_terminal_routes_are_registered():
    routes = _read("app/domain/routes.py")
    for path in (
        "/api/terminal/start",
        "/api/terminal/input",
        "/api/terminal/output",
...
def test_terminal_process_does_not_mutate_global_terminal_cwd(tmp_path, monkeypatch):
    from app.domain.terminal import close_terminal, start_terminal
...
def test_terminal_output_preserves_control_sequences_for_xterm():
    import codecs
    from app.domain.terminal import _decode_terminal_output
