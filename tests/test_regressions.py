...
def test_streaming_py_imports_has_pending(cleanup_test_sessions):
    ...
    """
    src = (REPO_ROOT / "app/domain/streaming.py").read_text()
...
def test_server_py_sse_loop_breaks_on_cancel(cleanup_test_sessions):
    ...
    Sprint 11: logic moved from server.py to api/routes.py; phase 3 native
    chat SSE lives in app/services/chat_stream.py.
    """
    import re
    ...
    chat_cancel = (
        "_TERMINAL_SSE_EVENTS" in combined
        and "cancel" in combined
        and re.search(r"if event in _TERMINAL_SSE_EVENTS:\s*\n\s*break", combined)
    )
    inline_cancel = re.search(
        r"if event in \([^)]*cancel[^)]*\):\s*break",
        combined,
    )
    assert chat_cancel or inline_cancel, (
        "Chat SSE must break on cancel (native _TERMINAL_SSE_EVENTS or inline tuple)"
    )
...
# ── General: domain modules all importable ─────────────────────────────────

def test_all_domain_modules_importable(cleanup_test_sessions):
    """All app/domain/ modules must be importable without NameError or ImportError.
...
def test_loadSession_resets_busy_state_for_idle_session(cleanup_test_sessions):
    ...
    """
    src = (REPO_ROOT / "static-legacy/sessions.js").read_text()
...
def test_done_handler_guards_setbusy_with_inflight_check(cleanup_test_sessions):
    ...
    """
    src = (REPO_ROOT / "static-legacy/messages.js").read_text()
...
def test_refresh_handler_does_not_drop_tool_messages_needed_by_todos(cleanup_test_sessions):
    ...
    """
    sessions_src = (REPO_ROOT / "static-legacy/sessions.js").read_text()
    ui_src = (REPO_ROOT / "static-legacy/ui.js").read_text()
    panels_src = (REPO_ROOT / "static-legacy/panels.js").read_text()
...
def test_server_delete_prunes_session_index(cleanup_test_sessions):
    ...
    src = (REPO_ROOT / "server.py").read_text()
    routes_src = (REPO_ROOT / "app" / "domain" / "routes.py").read_text() if (REPO_ROOT / "app" / "domain" / "routes.py").exists() else ""
    # Find the delete handler in either file
    for label, text in [("server.py", src), ("app/domain/routes.py", routes_src)]:
...
def test_server_delete_removes_session_bak_snapshot(cleanup_test_sessions):
    ...
    routes_src = (REPO_ROOT / "app" / "domain" / "routes.py").read_text()
    delete_idx = max(
...
def test_session_scoped_message_queue_frontend_wiring(cleanup_test_sessions):
    ...
    """
    ui_src = (REPO_ROOT / "static-legacy/ui.js").read_text()
    messages_src = (REPO_ROOT / "static-legacy/messages.js").read_text()
    sessions_src = (REPO_ROOT / "static-legacy/sessions.js").read_text()
...
def test_live_stream_tokens_persist_partial_assistant_for_session_switch(cleanup_test_sessions):
    ...
    assert "restoreLiveTurnHtmlForSession(sid)" in (REPO_ROOT / "static-legacy/sessions.js").read_text(), \
...
def test_ui_js_can_upgrade_thinking_spinner_into_live_reasoning_card(cleanup_test_sessions):
    ...
    """
    src = (REPO_ROOT / "static-legacy/ui.js").read_text()
...
def test_skills_slash_command_defined():
    ...
    """
    src = (REPO_ROOT / "static-legacy/commands.js").read_text()
...
def test_provider_oauth_authenticated_accepts_credential_pool_entries(
    ...
    from app.domain.onboarding import _provider_oauth_authenticated
...
def test_status_from_runtime_marks_openai_codex_ready_from_credential_pool(
    ...
    from app.domain.onboarding import _status_from_runtime
    import app.domain.onboarding as _ob
