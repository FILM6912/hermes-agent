...
WORKSPACE_JS = ROOT / "static-legacy" / "workspace.js"
SESSIONS_JS = ROOT / "static-legacy" / "sessions.js"
UI_JS = ROOT / "static-legacy" / "ui.js"
PANELS_JS = ROOT / "static-legacy" / "panels.js"
...
def _api_v1_path_helper() -> str:
    """Return the apiV1Path() helper defined at the top of workspace.js."""
    lines = _source(WORKSPACE_JS).splitlines()
    end = next(
        (idx for idx, line in enumerate(lines) if line.startswith("async function api(")),
        -1,
    )
    assert end > 0, "apiV1Path() must precede async function api() in workspace.js"
    return "\n".join(lines[:end]).strip()


def _api_helper() -> str:
    """Return the api() helper from workspace.js (brace-extraction is unreliable here)."""
    lines = _source(WORKSPACE_JS).splitlines()
    start = next(
        (idx for idx, line in enumerate(lines) if line.startswith("async function api(")),
        -1,
        ...
        (
            ...
            if idx > start and line.startswith("function recordClientSSEError")
        ),
        -1,
    )
    assert start >= 0 and end > start, "api() block must exist in workspace.js"
    return "\n".join(lines[start:end]).strip()


def _extract_js_function(src: str, name: str, *, async_fn: bool = True) -> str:
    prefix = "async " if async_fn else ""
    marker = f"{prefix}function {name}("
    ...
    paren_depth = 0
    close_paren = -1
    for idx in range(start + len(marker), len(src)):
        ch = src[idx]
        if ch == "(":
            paren_depth += 1
        elif ch == ")":
