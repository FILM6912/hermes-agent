...
def test_run_journal_default_fsyncs_terminal_events_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("app.domain.run_journal.os.fsync", lambda fd: fsync_calls.append(fd))
...
def test_stale_interrupted_event_reports_non_terminal_journal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("app.domain.run_journal._default_session_dir", lambda: tmp_path)
