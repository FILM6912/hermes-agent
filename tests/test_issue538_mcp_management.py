...
class TestMcpList:
    ...
    @patch('app.domain.routes.get_config')
    ...
    @patch('app.domain.routes._mcp_runtime_status_by_name')
    @patch('app.domain.routes.get_config')
    ...
    def test_mcp_auth_summary_bearer(self):
        summary = _mcp_auth_summary(SAMPLE_MCP['web-reader'])
        assert summary['auth_configured'] is True
        assert summary['auth_type'] == 'bearer'

    def test_mcp_auth_summary_oauth(self):
        summary = _mcp_auth_summary({'url': 'http://x', 'auth': 'oauth'})
        assert summary['auth_type'] == 'oauth'
        assert summary['auth_configured'] is True
...
class TestMcpSave:
    ...
    @patch('app.domain.routes._save_yaml_config_file')
    @patch('app.domain.routes._get_config_path', return_value='/tmp/test.yaml')
    @patch('app.domain.routes.get_config')
    ...
    @patch('app.domain.routes._get_config_path', return_value='/tmp/test.yaml')
    @patch('app.domain.routes.get_config')
    def test_add_http_server_with_bearer(self, mock_cfg, mock_path, mock_save, mock_reload):
        mock_cfg.return_value = {}
        h = _make_handler()
        h.command = 'PUT'
        body = {
        ...
        _handle_mcp_server_update(h, 'http-auth', body)
        saved = mock_save.call_args[0][1]
        assert saved['mcp_servers']['http-auth']['url'] == 'http://localhost:4000/mcp'
        assert saved['mcp_servers']['http-auth']['headers']['Authorization'] == 'Bearer secret-token'

    @patch('app.domain.routes.reload_config')
    @patch('app.domain.routes._save_yaml_config_file')
    @patch('app.domain.routes._get_config_path', return_value='/tmp/test.yaml')
    @patch('app.domain.routes.get_config')
    def test_update_http_preserves_bearer_when_masked(self, mock_cfg, mock_path, mock_save, mock_reload):
        mock_cfg.return_value = {
            'mcp_servers': {
                'http-auth': {
                    'url': 'http://localhost:4000/mcp',
...
class TestMcpToggle:
    ...
    def test_response_payload(self, mock_cfg, mock_path, mock_save, mock_reload):
        ...
        assert payload == {'ok': True, 'name': 'srv', 'enabled': False, 'profile': 'default'}
