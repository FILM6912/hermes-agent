...
def test_config_exports_stream_goal_related():
    """app.domain.config must export STREAM_GOAL_RELATED for the streaming gate."""
    from app.domain.config import STREAM_GOAL_RELATED
...
def test_config_exports_pending_goal_continuation():
    """app.domain.config must export PENDING_GOAL_CONTINUATION for auto-marking
    continuation streams as goal-related."""
    from app.domain.config import PENDING_GOAL_CONTINUATION
...
def test_streaming_source_code_gates_on_stream_goal_related():
    ...
    streaming_py = (Path(__file__).resolve().parents[1] / "app" / "domain" / "streaming.py").read_text()

    # Must import STREAM_GOAL_RELATED
    assert "STREAM_GOAL_RELATED" in streaming_py, (
        "streaming.py must import STREAM_GOAL_RELATED from app.domain.config"
    )
...
def test_routes_reads_pending_goal_continuation():
    ...
    from pathlib import Path
    routes_py = (Path(__file__).resolve().parents[1] / "app" / "domain" / "routes.py").read_text()
...
def test_goal_evaluate_after_turn_only_increments_for_user_initiated(monkeypatch):
    ...
    from app.domain import goals as webui_goals
