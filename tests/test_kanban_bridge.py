...
def test_routes_dispatches_api_kanban_get_to_bridge():
    src = open("app/domain/routes.py", encoding="utf-8").read()
...
def test_kanban_only_mine_bulk_dispatch_and_block_unblock(monkeypatch):
    bridge = _load_bridge(monkeypatch)
    monkeypatch.setattr("app.domain.profiles.get_active_profile_name", lambda: "webui-test", raising=False)
...
def test_routes_dispatches_canonical_kanban_patch_and_delete_verbs():
    src = open("app/domain/routes.py", encoding="utf-8").read()
    legacy = open("app/core/legacy_handler.py", encoding="utf-8").read()
    assert '"PATCH": handle_patch' in legacy
    assert '"DELETE": handle_delete' in legacy
    assert 'parsed.path.startswith("/api/kanban/")' in src
