...
def _rmtree_best_effort(path: pathlib.Path, *, attempts: int = 5) -> None:
    """Remove a directory tree; tolerate races from parallel test runs."""
    if not path.exists():
        return
...
# Phase 5: match production default — native /api/v1 only unless legacy is enabled.
os.environ.setdefault('HERMES_WEBUI_LEGACY_API', '0')
os.environ.setdefault('HERMES_WEBUI_CLI_SESSIONS_CACHE_TTL', '0')


def _legacy_api_bridge_enabled() -> bool:
    return os.environ.get("HERMES_WEBUI_LEGACY_API", "0").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _normalize_legacy_api_url(url: str) -> str:
    """Map client /api/* paths to /api/v1/* when legacy catch-alls are off."""
    if _legacy_api_bridge_enabled():
        return url
...
def _request_targets_test_server(url: str) -> bool:
...
def _legacy_api_aware_urlopen(url, *args, **kwargs):
    if isinstance(url, urllib.request.Request):
        full = url.full_url
        if _request_targets_test_server(full):
            normalized = _normalize_legacy_api_url(full)
            if normalized != full:
    ...
    elif isinstance(url, str) and _request_targets_test_server(url):
        url = _normalize_legacy_api_url(url)
    return _REAL_URLOPEN(url, *args, **kwargs)


urllib.request.urlopen = _legacy_api_aware_urlopen

# ── Server script: always relative to repo root ───────────────────────────
SERVER_SCRIPT = REPO_ROOT / 'server.py'
APP_MAIN = REPO_ROOT / 'app' / 'main.py'
if not SERVER_SCRIPT.exists() and not APP_MAIN.exists():
    raise RuntimeError(
        f"Neither server.py nor app/main.py found under {REPO_ROOT}. "
        "Is conftest.py in the tests/ subdirectory of the repo?"
    )


def _test_server_launch_cmd() -> list[str]:
    """Launch uvicorn when FastAPI app exists; otherwise fall back to server.py."""
    if APP_MAIN.exists():
        return [
            VENV_PYTHON,
            '-m',
    ...
    return [VENV_PYTHON, str(SERVER_SCRIPT)]
...
def _reapply_network_block_wrappers():
    """Re-install outbound socket blocks after tests that touch socket.socket."""
    _hermes_test_socket.create_connection = _hermes_blocked_create_connection
    _hermes_test_socket.socket.connect = _hermes_blocked_socket_connect
    yield
    _hermes_test_socket.create_connection = _hermes_blocked_create_connection
...
def allow_outbound_network(monkeypatch):
    ...
    try:
        ...
        # monkeypatch teardown can leave the real connect on class attributes;
        # re-apply the module-level wrappers for subsequent tests.
        _hermes_test_socket.create_connection = _hermes_blocked_create_connection
        _hermes_test_socket.socket.connect = _hermes_blocked_socket_connect
...
def test_server():
    ...
    # Clean slate
    _rmtree_best_effort(TEST_STATE_DIR)
    ...
    server_startup_timeout = 45 if APP_MAIN.exists() else 20
    server_cwd = str(REPO_ROOT) if APP_MAIN.exists() else WORKDIR
    proc = subprocess.Popen(
        _test_server_launch_cmd(),
        cwd=server_cwd,
        env=env,
    ...
    if not _wait_for_server(TEST_BASE, timeout=server_startup_timeout):
        proc.kill()
        launch_cmd = ' '.join(_test_server_launch_cmd())
        pytest.fail(
            f"Test server on port {TEST_PORT} did not start within {server_startup_timeout}s.\n"
            f"  launch    : {launch_cmd}\n"
            f"  python    : {VENV_PYTHON}\n"
            f"  agent dir : {HERMES_AGENT}\n"
            f"  workdir   : {server_cwd}\n"
        )
...
@pytest.fixture(autouse=True)
def _reset_password_auth_after_test():
    """Prevent password/passkey tests from leaving auth enabled for later tests."""
    yield
    try:
        import json
        from app.domain.auth import _invalidate_password_hash_cache

        settings_path = TEST_STATE_DIR / "settings.json"
        if settings_path.exists():
            data = json.loads(settings_path.read_text(encoding="utf-8"))
            dirty = False
            for key in ("password_hash", "passkeys", "passkey_credentials"):
                if key in data:
                    data.pop(key, None)
                    dirty = True
            if dirty:
