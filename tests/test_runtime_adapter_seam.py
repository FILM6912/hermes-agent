...
def test_runtime_adapter_interface_and_legacy_journal_methods_exist():
    runtime = importlib.import_module("app.domain.runtime_adapter")
...
def test_legacy_journal_adapter_observe_and_get_run_use_journal_and_live_state(tmp_path):
    runtime = importlib.import_module("app.domain.runtime_adapter")
    run_journal = importlib.import_module("app.domain.run_journal")
...
def test_chat_cancel_route_uses_adapter_only_when_flag_enabled():
    routes = importlib.import_module("app.domain.routes")
    root = routes.Path(__file__).parent.parent
    routes_src = (root / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
    owner_src = (root / "app" / "domain" / "chat_streaming.py").read_text(encoding="utf-8")
    cancel_idx = routes_src.index('if parsed.path == "/api/chat/cancel":')
    cancel_route = routes_src[cancel_idx:routes_src.index('if parsed.path == "/api/chat/stream":', cancel_idx)]
    cancel_owner_idx = owner_src.index("def cancel_chat_stream")
    cancel_body = owner_src[cancel_owner_idx:cancel_owner_idx + 900]

    assert "cancel_chat_stream" in cancel_route
...
def test_approval_and_clarify_routes_use_adapter_only_when_flag_enabled():
    routes = importlib.import_module("app.domain.routes")
    src = (routes.Path(__file__).parent.parent / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
