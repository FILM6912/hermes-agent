...
class TestBuildNousFeaturedSet:
    ...
    def test_small_catalog_is_no_op(self):
        from app.domain.config import _build_nous_featured_set, _NOUS_FEATURED_THRESHOLD
    ...
    def test_large_catalog_is_capped_to_target(self):
        from app.domain.config import _build_nous_featured_set, _NOUS_FEATURED_TARGET
    ...
    def test_priority_vendors_get_picked_first(self):
        from app.domain.config import _build_nous_featured_set, _NOUS_VENDOR_PRIORITY
        catalog = _build_big_catalog()
    ...
    def test_curated_static_flagships_are_preserved(self):
        from app.domain.config import _build_nous_featured_set, _PROVIDER_MODELS
...
class TestApiModelsLargeCatalog:
    ...
    def test_picker_caps_large_catalog_and_exposes_extras(self, monkeypatch, tmp_path):
        ...
        try:
            ...
            grp = nous_groups[0]
            from app.domain.config import _NOUS_FEATURED_TARGET
...
class TestFrontendExtrasContract:
    ...
    def test_ui_js_hydrates_dynamic_labels_from_extra_models(self):
        ...
        src = (Path(__file__).resolve().parent.parent / "static-legacy" / "ui.js").read_text(encoding="utf-8")
    ...
    def test_commands_js_loads_slash_args_from_extra_models(self):
        from pathlib import Path
        src = (Path(__file__).resolve().parent.parent / "static-legacy" / "commands.js").read_text(encoding="utf-8")
    ...
    def test_panels_js_uses_models_total_for_count(self):
        from pathlib import Path
        src = (Path(__file__).resolve().parent.parent / "static-legacy" / "panels.js").read_text(encoding="utf-8")
