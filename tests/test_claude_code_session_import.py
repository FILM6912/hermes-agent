...
def test_get_cli_sessions_reuses_short_ttl_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_WEBUI_CLI_SESSIONS_CACHE_TTL", "60")
    models.clear_cli_sessions_cache()
    # Keep cache keys stable while the session-scoped test server mutates SESSION_INDEX_FILE.
    monkeypatch.setattr(models, "_path_stat_cache_key", lambda _path: (0, 0), raising=False)
    monkeypatch.setattr(
        models,
        "_sqlite_file_stat_cache_key",
        lambda _db_path: ((0, 0), None, None),
        raising=False,
    )
...
def test_read_only_source_badge_ui_guards_are_present():
    ...
    messages_js = (REPO_ROOT / "static-legacy" / "messages.js").read_text(encoding="utf-8")
