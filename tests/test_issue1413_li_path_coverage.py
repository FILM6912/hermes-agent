"""
Regression test for #1413 — every li('NAME', ...) call in static-legacy/*.js must
reference an icon name registered in LI_PATHS in static-legacy/icons.js.
...
What the test does
------------------
1. Parse static-legacy/icons.js to extract every key in the LI_PATHS object literal.
2. Walk every other static-legacy/*.js file and collect every li('NAME', ...) call.
...
STATIC_DIR = Path(__file__).parent.parent / "static-legacy"
...
def test_all_li_calls_reference_registered_icons() -> None:
    """Every li('NAME', ...) in static-legacy/*.js must have NAME in LI_PATHS."""
    ...
    assert len(call_sites) > 10, (
        "Sanity check: parser found only %d li() call sites across static-legacy/*.js"
        % len(call_sites)
    )
    ...
    if missing:
        ...
        pytest.fail(
            ...
            "static-legacy/icons.js. The button/container will render empty in "
...
def test_specific_icons_present_after_1413_fix() -> None:
    ...
    for icon in ("volume-2", "chevron-up", "hash", "cpu", "dollar-sign"):
        assert icon in registered, (
            ...
            f"not be removed. Re-adding required: see static-legacy/icons.js."
        )
