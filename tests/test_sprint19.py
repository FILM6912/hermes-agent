...
def _header(headers: dict, name: str) -> str | None:
    """Case-insensitive header lookup (urllib lowercases response header names)."""
...
def test_login_route_injects_webui_version_for_login_script():
    ...
    src = Path(__file__).resolve().parents[1].joinpath("app", "domain", "routes.py").read_text(encoding="utf-8")
...
def test_security_headers_on_json():
    ...
    assert _header(headers, "Referrer-Policy") == "same-origin"
...
def test_permissions_policy_does_not_disable_microphone():
    ...
    assert status == 200
    policy = _header(headers, "Permissions-Policy") or ""
