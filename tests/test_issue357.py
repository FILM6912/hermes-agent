...
class TestDockerfileUvPreinstall:
    ...
    def test_dockerfile_installs_uv_at_build_time(self):
        """Dockerfile must ship uv at build time (not only at runtime)."""
        has_official_binary = "ghcr.io/astral-sh/uv" in DOCKERFILE and "/usr/local/bin/" in DOCKERFILE
        has_install_script = "RUN curl" in DOCKERFILE and "uv/install.sh" in DOCKERFILE
        assert has_official_binary or has_install_script, (
            "Dockerfile must install uv at build time via COPY --from=ghcr.io/astral-sh/uv "
            "or RUN curl .../uv/install.sh"
        )

    def test_dockerfile_uv_installed_system_wide(self):
        """uv must land in /usr/local/bin so hermeswebui can find it on PATH."""
        has_copy_to_system = re.search(
            r"COPY --from=ghcr\.io/astral-sh/uv[^\n]*\s+/usr/local/bin/",
            DOCKERFILE,
        )
        ...
        has_system_dir = has_copy_to_system is not None or (
            uv_install_line is not None
            and (
    ...
    def test_dockerfile_uv_installed_before_copy(self):
        ...
        uv_pos = DOCKERFILE.find("ghcr.io/astral-sh/uv")
        if uv_pos == -1:
    ...
    def test_dockerfile_uv_installed_as_root_or_before_user_switch(self):
        ...
        uv_line_idx = next(
            ...
            if "ghcr.io/astral-sh/uv" in line or "uv/install.sh" in line
        )
        ...
        for i in range(uv_line_idx - 1, -1, -1):
            if lines[i].strip().startswith("USER "):
                ...
                break
        assert user_before in (None, "root"), (
...
class TestWorkspacePermissions:
    ...
    def test_workspace_uses_root_init_chown(self):
        ...
        assert 'chown_shared_workspace_bind_mount' in root_section, (
            "docker_init.bash must recursively chown the shared workspace bind mount "
            "during root init so per-account .uploads/ dirs are writable (#357)"
        )
...
class TestRelativeHermesPaths:
    """Relative ./webui and ./workspace must resolve against HERMES_HOME, not CWD."""

    def test_resolve_hermes_path_helper_exists(self):
        assert "resolve_hermes_path()" in INIT_SCRIPT
        assert './*) echo "$(_hermes_home_base)/${raw#./}" ;;' in INIT_SCRIPT

    def test_state_dir_resolved_before_mkdir(self):
        assert "_resolve_webui_path_env HERMES_WEBUI_STATE_DIR" in INIT_SCRIPT
        assert 'mkdir -p "$HERMES_WEBUI_STATE_DIR"' in INIT_SCRIPT
        assert 'chown hermeswebui:hermeswebui "$HERMES_WEBUI_STATE_DIR"' in INIT_SCRIPT

    def test_default_workspace_resolved_in_root_init(self):
        root_section = INIT_SCRIPT[
            INIT_SCRIPT.find('if [ "A${whoami}" == "Aroot" ]; then'):
            INIT_SCRIPT.find('exec su')
        ]
        assert "_resolve_webui_path_env HERMES_WEBUI_DEFAULT_WORKSPACE" in root_section
