...
def _invoke(session, query=None):
    ...
    with patch("app.domain.routes.get_session", return_value=session), \
         patch("app.domain.routes._clear_stale_stream_state", return_value=False), \
         patch("app.domain.routes._lookup_cli_session_metadata", return_value={}), \
         patch("app.domain.routes.redact_session_data", side_effect=lambda raw: raw), \
