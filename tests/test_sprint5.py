...
def test_workspace_add_rejected_when_profile_already_has_workspace(cleanup_test_sessions):
...
def test_workspace_remove_blocked(cleanup_test_sessions):
    _, ws = make_session_tracked(cleanup_test_sessions)
    result, status = post("/api/workspaces/remove", {"path": str(ws)})
    assert status == 400
    assert "cannot be removed" in result.get("error", "").lower()

def test_workspace_rename(cleanup_test_sessions):
    make_session_tracked(cleanup_test_sessions)
    listed, _ = get("/api/workspaces")
    ws_path = listed["workspaces"][0]["path"]
    result, status = post("/api/workspaces/rename", {"path": ws_path, "name": "My Workspace"})
    assert status == 200
    assert {w["path"]: w["name"] for w in result["workspaces"]}.get(ws_path) == "My Workspace"
