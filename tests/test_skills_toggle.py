...
PANELS_JS = (Path(__file__).resolve().parent.parent / "static-legacy" / "panels.js").read_text("utf-8")
I18N_JS = (Path(__file__).resolve().parent.parent / "static-legacy" / "i18n.js").read_text("utf-8")
STYLE_CSS = (Path(__file__).resolve().parent.parent / "static-legacy" / "style.css").read_text("utf-8")
...
def test_toggle_path_registered():
    ...
    routes_source = (Path(__file__).resolve().parent.parent / "app" / "domain" / "routes.py").read_text("utf-8")
...
def test_disabled_list_round_trip(tmp_path):
    ...
    from app.domain.config import _load_yaml_config_file, _save_yaml_config_file
...
def test_normalize_names_list():
    ...
    from app.domain.routes import _normalize_names_list
...
def test_toggle_name_in_list():
    ...
    from app.domain.routes import _toggle_name_in_list
...
def test_platform_disabled_write_through(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "config.yaml"
    monkeypatch.setattr("app.domain.routes._get_config_path", lambda: config_path)
    ...
    monkeypatch.setattr(
        "app.domain.routes._find_skill_in_dirs",
