...
import app.domain.onboarding as mod
...
_COMMON_PATCHES = [
    ("app.domain.onboarding.load_settings",        lambda: {}),
    ("app.domain.onboarding.get_config",           lambda: {}),
    ("app.domain.onboarding.verify_hermes_imports",lambda: (True, [], [])),
    ("app.domain.onboarding.load_workspaces",      lambda: []),
    ("app.domain.onboarding.get_last_workspace",   lambda: "/tmp"),
    ("app.domain.onboarding.get_available_models", lambda: []),
    ("app.domain.onboarding.is_auth_enabled",      lambda: False),
    ("app.domain.onboarding._build_setup_catalog", lambda cfg: {}),
    ("app.domain.onboarding._get_config_path",     lambda: __import__("pathlib").Path("/tmp/fake.yaml")),
]
...
class TestSkipOnboardingEnvVar(unittest.TestCase):
    ...
    def _run_status(self, runtime, env_override):
        runtime_patches = [("app.domain.onboarding._status_from_runtime", lambda cfg, ok: runtime)]
    ...
    def test_settings_completed_still_works_without_env_var(self):
        ...
        runtime_patches = [("app.domain.onboarding._status_from_runtime", lambda cfg, ok: _READY_RUNTIME)]
        settings_patch = [("app.domain.onboarding.load_settings", lambda: {"onboarding_completed": True})]
...
class TestApplyOnboardingKeySync(unittest.TestCase):
    ...
    def test_api_key_set_in_os_environ_after_apply(self):
        ...
        with patch("app.domain.onboarding._load_yaml_config", return_value=mock_cfg), \
             patch("app.domain.onboarding._save_yaml_config"), \
             patch("app.domain.onboarding._write_env_file"), \
             patch("app.domain.onboarding.reload_config"), \
             patch("app.domain.onboarding.get_onboarding_status", return_value={"completed": True}), \
             patch("app.domain.onboarding._get_config_path", return_value=pathlib.Path("/tmp/fake.yaml")), \
             patch("app.domain.onboarding._load_env_file", return_value={}), \
             patch("app.domain.onboarding._provider_api_key_present", return_value=False), \
             patch("app.domain.onboarding._get_active_hermes_home", return_value=pathlib.Path("/tmp")):
    ...
    def test_no_key_provided_does_not_set_environ(self):
        ...
        with patch("app.domain.onboarding._load_yaml_config", return_value=mock_cfg), \
             ...
             patch("app.domain.onboarding.get_onboarding_status", return_value={"completed": True}), \
             patch("app.domain.onboarding._get_config_path", return_value=pathlib.Path("/tmp/fake.yaml")), \
             patch("app.domain.onboarding._load_env_file", return_value={"OPENAI_API_KEY": "sk-existing-key"}), \
             patch("app.domain.onboarding._provider_api_key_present", return_value=True), \
             patch("app.domain.onboarding._get_active_hermes_home", return_value=pathlib.Path("/tmp")):
...
class TestApplyOnboardingSkipGuard(unittest.TestCase):
    ...
    def test_apply_setup_blocked_when_skip_env_set(self):
        ...
        with patch.dict(os.environ, {"HERMES_WEBUI_SKIP_ONBOARDING": "1"}, clear=False), \
             patch("app.domain.onboarding._save_yaml_config", save_yaml_mock), \
             patch("app.domain.onboarding._write_env_file", write_env_mock), \
             patch("app.domain.onboarding.save_settings"), \
             patch("app.domain.onboarding.get_onboarding_status", return_value={"completed": True}):
            mod.apply_onboarding_setup({
                "provider": "openai",
                "model": "gpt-4o",
    ...
    def test_apply_setup_proceeds_normally_without_skip_env(self):
        ...
        with patch.dict(os.environ, env, clear=True), \
             patch("app.domain.onboarding._load_yaml_config", return_value=mock_cfg), \
             patch("app.domain.onboarding._save_yaml_config", save_yaml_mock), \
             patch("app.domain.onboarding._write_env_file"), \
             patch("app.domain.onboarding.reload_config"), \
             patch("app.domain.onboarding.get_onboarding_status", return_value={"completed": True}), \
             patch("app.domain.onboarding._get_config_path", return_value=pathlib.Path("/tmp/fake.yaml")), \
             patch("app.domain.onboarding._load_env_file", return_value={"OPENAI_API_KEY": "existing"}), \
             patch("app.domain.onboarding._provider_api_key_present", return_value=True), \
             patch("app.domain.onboarding._get_active_hermes_home", return_value=pathlib.Path("/tmp")):
            mod.apply_onboarding_setup({
                "provider": "openai",
