...
def test_workspace_add_accepts_real_dir():
    """Real directories under a trusted root pass workspace validation."""
    from app.domain.workspace import validate_workspace_to_add
    ...
    try:
        resolved = validate_workspace_to_add(str(tmp))
        assert resolved == tmp.resolve()
        # Single-workspace policy: HTTP add rejects once the profile workspace exists.
        d, status = post("/api/workspaces/add", {"path": str(tmp), "name": "test-ws"})
        assert status == 400
        assert "one workspace" in d.get("error", "").lower()
    finally:
        import shutil
