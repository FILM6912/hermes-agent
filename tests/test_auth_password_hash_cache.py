...
# We deliberately do NOT delete app.domain.config from sys.modules (unlike some
# sibling test files that need a fresh config import).  Deleting app.domain.config
...
# The auth module re-executes `from app.domain.config import STATE_DIR, load_settings`
# at import time, but app.domain.config is already in sys.modules — Python just
# rebinds the names from the existing module, keeping the conftest STATE_DIR
# untouched.
import app.domain.auth
importlib.reload(app.domain.auth)
