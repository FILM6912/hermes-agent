...
def _extract_handler(name: str) -> str:
    """Return the source of the handler function `name` from app/domain/routes.py."""
    marker = f"def {name}("
    idx = ROUTES_PY.find(marker)
    assert idx != -1, f"{name} not found in app/domain/routes.py"
...
def test_messaging_session_loader_prefers_longer_sidecar_transcript():
    ...
    handler = _extract_handler("build_session_detail_payload")
