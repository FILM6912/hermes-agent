...
def _read_streaming():
    return Path(__file__).parents[1].joinpath("app", "domain", "streaming.py").read_text(encoding="utf-8")


def _read_routes():
    return Path(__file__).parents[1].joinpath("app", "domain", "routes.py").read_text(encoding="utf-8")
