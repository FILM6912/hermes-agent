...
class TestGatewayApprovalUnblocking:
    ...
    def test_approval_pending_reads_gateway_queue(self):
        """Gateway-queue head must be visible to approval pending/SSE snapshot."""
        sid = f"unit-gw-pending-{uuid.uuid4().hex[:8]}"
        cmd = "python -c \"import os; os.remove('x')\""
        entry = _ApprovalEntry(
            {
                "command": cmd,
                "pattern_key": "execute_code",
                "pattern_keys": ["execute_code"],
                "description": "execute_code script",
            }
        )
        with _lock:
            _gateway_queues.setdefault(sid, []).append(entry)
        try:
            ...
            head, total = _approval_head_snapshot(sid)
            assert head is not None
            assert head["command"] == cmd
            assert total == 1
        finally:
            resolve_gateway_approval(sid, "deny")
