...
def test_workspace_api_401_redirect_uses_relative_login_path():
    src = read("static-legacy/workspace.js")
...
def test_direct_frontend_fetches_are_relative_to_current_mount():
    for path in ("static-legacy/boot.js", "static-legacy/sessions.js", "static-legacy/ui.js"):
        src = read(path)
        assert "fetch('/api/" not in src, (
            f"{path} must not fetch root /api/* because /hermes/ is subpath mounted."
        )
        assert 'fetch("/api/' not in src
    assert "fetch('/health'" not in read("static-legacy/ui.js")
    assert "new URL('health'" in read("static-legacy/ui.js")
