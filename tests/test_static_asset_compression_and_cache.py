...
def _patch_static_root(monkeypatch, static_root):
    ...
    # _serve_static serves only under `static/`; `static-legacy/` is archive-only,
...
def isolated_static(tmp_path, monkeypatch):
    ...
    # _serve_static resolves assets via REPO_ROOT / "static".
    monkeypatch.setattr(routes, "REPO_ROOT", tmp_path, raising=False)
    yield static_root
