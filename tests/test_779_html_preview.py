...
def _get_routes_content():
    return open("app/domain/routes.py", encoding="utf-8").read()


def _get_workspace_js():
    return open("static-legacy/workspace.js", encoding="utf-8").read()
...
def test_open_in_browser_uses_blob_url_for_brave():
    content = _get_workspace_js()
    assert "function _openPreviewInBrowserTab" in content
    assert "URL.createObjectURL" in content
    assert "_htmlForBrowserTab" in content
    idx = content.find("function _openPreviewInBrowserTab")
    block = content[idx : idx + 1200]
    assert "HTML_EXTS.has(ext)" in block
    assert "PDF_EXTS.has(ext)" in block
    assert "new Blob" in block


def test_file_view_route_exists():
    content = _get_routes_content()
    assert 'parsed.path == "/api/file/view"' in content


def test_file_view_serves_inline_bytes_directly():
    """Open-in-browser view must serve file bytes directly, not iframe/embed shells."""
    content = _get_routes_content()
    idx = content.find("def _handle_file_view")
    assert idx != -1
    block = content[idx : idx + 1600]
    assert "_serve_inline_html_preview" in block
    assert "plain_inline_disposition=True" in block
    assert "inline_filename=False" in block
    assert "<iframe src=" not in block
    assert "<embed src=" not in block
...
def test_mime_map_includes_html_and_htm():
    ...
    from app.domain.config import MIME_MAP
