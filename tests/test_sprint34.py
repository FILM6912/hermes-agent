...
class TestStatusFromRuntimeOAuth:
    ...
    def test_copilot_not_ready_without_credentials(self, tmp_path):
        ...
        with unittest.mock.patch(
            "app.domain.onboarding._provider_oauth_authenticated",
            return_value=False,
        ):
            result = self._call("copilot", "gpt-5.4", tmp_path)
...
def test_control_center_resets_active_section_on_close():
    ...
    src = open(pathlib.Path(__file__).parent.parent / 'static-legacy' / 'panels.js').read()
...
def test_control_center_tab_highlight_on_open():
    ...
    css = open(pathlib.Path(__file__).parent.parent / 'static-legacy' / 'style.css').read()
...
class TestApplyOnboardingSetupUnsupportedProvider:
    ...
    def _call(self, provider: str) -> dict:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with unittest.mock.patch("app.domain.onboarding._get_active_hermes_home",
                                     return_value=pathlib.Path(tmp)), \
                 unittest.mock.patch("app.domain.onboarding._get_config_path",
                                     return_value=pathlib.Path(tmp) / "config.yaml"), \
                 unittest.mock.patch("app.domain.onboarding.save_settings") as mock_save, \
                 unittest.mock.patch("app.domain.onboarding.get_onboarding_status",
