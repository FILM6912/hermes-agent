...
class TestApplyBaseURL:
    ...
    def test_requires_base_url_writes_user_url(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("app.domain.onboarding._get_config_path", lambda: config_path)
        monkeypatch.setattr("app.domain.onboarding._get_active_hermes_home", lambda: tmp_path)
        monkeypatch.setattr("app.domain.onboarding._load_yaml_config", lambda p: {})
        monkeypatch.setattr(
            "app.domain.onboarding._normalize_model_for_provider", lambda prov, m: m
        )
        monkeypatch.setattr("app.domain.onboarding._write_env_file", lambda p, d: None)
        monkeypatch.setattr("app.domain.onboarding._save_yaml_config", lambda p, c: None)
        monkeypatch.setattr("app.domain.onboarding._provider_api_key_present", lambda *a: True)
        monkeypatch.setattr("app.domain.onboarding.reload_config", lambda: None)

        saved_cfg = {}
        def mock_save(p, cfg):
            saved_cfg.update(cfg)
        monkeypatch.setattr("app.domain.onboarding._save_yaml_config", mock_save)
...
class TestI18nCategoryKeys:
    def test_en_has_all_category_keys(self):
        with open("static-legacy/i18n.js", encoding="utf-8") as f:
            content = f.read()
...
class TestApplyBaseURLSpecialized:
    ...
    def _run_setup(self, tmp_path, monkeypatch, provider):
        ...
        monkeypatch.setattr("app.domain.onboarding._get_active_hermes_home", lambda: tmp_path)
        monkeypatch.setattr("app.domain.onboarding._load_yaml_config", lambda p: {})
        monkeypatch.setattr("app.domain.onboarding._normalize_model_for_provider", lambda prov, m: m)
        monkeypatch.setattr("app.domain.onboarding._write_env_file", lambda p, d: None)
        monkeypatch.setattr("app.domain.onboarding._provider_api_key_present", lambda *a: True)
        monkeypatch.setattr("app.domain.onboarding.reload_config", lambda: None)
