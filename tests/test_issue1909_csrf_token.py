...
def test_index_shell_injects_session_bound_csrf_token(monkeypatch):
    ...
    import app.domain.extensions as extensions
...
def test_auth_status_includes_session_bound_csrf_token(monkeypatch):
...
def test_login_response_includes_session_bound_csrf_token(monkeypatch):
    from app.repositories.auth import AuthRepository

    cookie = _signed_cookie("i" * 64)
    token = auth.csrf_token_for_session(cookie)
    monkeypatch.setattr(auth, "is_auth_enabled", lambda: True)
    monkeypatch.setattr(auth, "verify_password", lambda _password: True)
    monkeypatch.setattr(auth, "create_session", lambda **kwargs: cookie)

    try:
        payload, status_code, cookie_val = AuthRepository().login(
            "secret",
            client_ip="127.0.0.1",
        )
