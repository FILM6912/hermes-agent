...
class TestDetectWebUIVersion:
    ...
    def test_git_failure_falls_back_to_version_file(self, tmp_path):
        """When git fails (Docker image), falls back to app/domain/_version.py."""
        result = self._fresh_detect(
...
class TestWebUIVersionConstant:
    ...
    def test_webui_version_is_set(self):
        """WEBUI_VERSION is a non-empty string exported from app.domain.updates."""
        import app.domain.updates as upd
        assert hasattr(upd, 'WEBUI_VERSION'), 'WEBUI_VERSION not exported from app.domain.updates'
...
class TestSettingsEndpointVersion:
    ...
    def test_api_settings_includes_webui_version(self):
        ...
        with patch('app.domain.routes.load_settings', return_value=dict(minimal_settings)), \
    ...
    def test_api_settings_webui_version_not_empty(self):
        ...
        with patch('app.domain.routes.load_settings', return_value={}), \
             patch('app.domain.routes.j', side_effect=fake_j):
            routes.handle_get(handler, parsed)
    ...
    def test_api_settings_no_password_hash(self):
        ...
        with patch('app.domain.routes.load_settings', return_value={'password_hash': 'secret123'}), \
             patch('app.domain.routes.j', side_effect=fake_j):
            routes.handle_get(handler, parsed)
...
class TestPanelsJSVersionBadge:
    ...
    def _read_js(self):
        return (REPO_ROOT / 'static-legacy' / 'panels.js').read_text(encoding='utf-8')
...
class TestServerVersionHeader:
    """FastAPI uses uvicorn; version is resolved in api.updates for settings/UI."""

    def test_server_py_is_thin_uvicorn_launcher(self):
    ...
    def test_webui_version_resolved_in_api_updates(self):
        src = (REPO_ROOT / 'app/domain/updates.py').read_text(encoding='utf-8')
        assert 'WEBUI_VERSION' in src
        assert '_detect_webui_version' in src
        assert "return 'unknown'" in src

    def test_webui_version_importable(self):
        from app.domain.updates import WEBUI_VERSION

        assert WEBUI_VERSION
