...
auth = importlib.import_module("app.domain.auth")
...
class TestSessionPruning(unittest.TestCase):
    ...
    def test_session_ttl_is_24_hours(self):
        ...
        from app.domain.auth import _sessions, SESSION_TTL, _session_expiry
        # find our entry
...
class TestSessionTtlResolution(unittest.TestCase):
    ...
    def test_env_var_overrides_settings(self):
        ...
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), 3600)

    def test_human_readable_env_duration(self):
        """Human-readable env TTL format should be accepted."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "300d 10m 55s"
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), (86400 * 300) + (60 * 10) + 55)

    def test_human_readable_year_duration(self):
        """Year unit should be accepted when within max range."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "1y"
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), 86400 * 365)

    def test_human_readable_over_one_year_duration(self):
        """Durations over one year should be accepted within new upper bound."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "1y 8d"
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), (86400 * 365) + (86400 * 8))

    def test_human_readable_compact_duration(self):
        """Compact duration tokens without spaces should be accepted."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "12h30m"
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), (3600 * 12) + (60 * 30))
    ...
    def test_clamps_maximum(self):
        """Values above 10 years fall through to settings/default (do not honor)."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "400000000"
        auth.load_settings = lambda: {}
        from app.domain.auth import _resolve_session_ttl
        # Out-of-range env values are rejected; falls through to default 30 days.
        self.assertEqual(_resolve_session_ttl(), auth.SESSION_TTL)
    ...
    def test_invalid_human_duration_falls_through(self):
        """Garbage between duration tokens should be rejected."""
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "10m and 5s"
        auth.load_settings = lambda: {}
        from app.domain.auth import _resolve_session_ttl
        self.assertEqual(_resolve_session_ttl(), auth.SESSION_TTL)
    ...
    def test_session_uses_dynamic_ttl(self):
        ...
        os.environ["HERMES_WEBUI_SESSION_TTL"] = "3600"
        token_hex = auth.create_session().split(".")[0]
        from app.domain.auth import _sessions, _session_expiry
        for t, entry in _sessions.items():
            if t == token_hex:
                exp = _session_expiry(entry)
