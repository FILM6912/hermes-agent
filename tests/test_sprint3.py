...
def test_workspace_add_rejects_external_paths_with_single_workspace_policy(tmp_path):
    """External paths cannot be registered once a profile already owns its one workspace."""
    ...
    assert "one workspace" in result.get("error", "").lower()
...
def test_v1_session_update_requires_session_id():
    result, status = post("/api/v1/session/update", {"model": "openai/gpt-5.4-mini"})
    assert status == 400


def test_v1_session_move_requires_session_id():
    result, status = post("/api/v1/session/move", {"project_id": None})
    assert status == 400


def test_v1_session_draft_requires_session_id():
    result, status = post("/api/v1/session/draft", {"text": "hello"})
    assert status == 400


def test_v1_session_delete_requires_session_id():
    result, status = post("/api/v1/session/delete", {})
    assert status == 400
