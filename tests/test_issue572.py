...
def _call_provider_api_key_present(provider: str, cfg: dict = None, env_values: dict = None):
    from app.domain.onboarding import _provider_api_key_present
...
class TestProviderApiKeyPresentFallback:
    ...
    def test_minimax_cn_logged_in_returns_true(self):
        ...
        with mock.patch("app.domain.onboarding._SUPPORTED_PROVIDER_SETUPS", {
    ...
    def test_supported_provider_still_works_without_fallback(self):
        ...
        from app.domain.onboarding import _provider_api_key_present, _SUPPORTED_PROVIDER_SETUPS
...
class TestStatusFromRuntimeUnsupportedProvider:
    ...
    def _run(self, provider: str, model: str, api_key_present: bool, oauth_present: bool = False):
        ...
        with (
            mock.patch("app.domain.onboarding._HERMES_FOUND", True),
            mock.patch("app.domain.onboarding._load_env_file", return_value={}),
            mock.patch("app.domain.onboarding._get_active_hermes_home", return_value=pathlib.Path("/tmp")),
            mock.patch("app.domain.onboarding._provider_api_key_present", return_value=api_key_present),
            mock.patch("app.domain.onboarding._provider_oauth_authenticated", return_value=oauth_present),
        ):
