...
function apiV1Path(path){
  ...
  if(qIdx!==-1){ base=path.slice(0,qIdx); suffix=path.slice(qIdx); }
  else {
    const hIdx=path.indexOf('#');
    if(hIdx!==-1){ base=path.slice(0,hIdx); suffix=path.slice(hIdx); }
  }
...
async function api(path, opts={}) {
  if(String(path).includes('models')) return apiModels;
  return {};
}
...
fetch = async function(url) {
  ...
  if (href.includes('api/v1/models') || href.includes('api/models')) {
...
def _run_populate_driver(
    ...
    *,
    ...
    result = subprocess.run(
        [NODE, driver_path, str(REPO / "static-legacy" / "ui.js"), json.dumps(payload)],
        capture_output=True,
        text=True,
        timeout=10,
    )
