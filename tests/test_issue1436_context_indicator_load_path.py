...
ROUTES = Path(__file__).resolve().parent.parent / "app" / "domain" / "routes.py"
UI_JS = Path(__file__).resolve().parent.parent / "static-legacy" / "ui.js"
...
class TestIssue1436BackendFallback:
    ...
    def _invoke_get_session(self, session_obj, *, fallback_returns=0):
        ...
        with patch("app.domain.routes.get_session", return_value=session_obj), \
             patch("app.domain.routes.j", side_effect=fake_j), \
    ...
    def test_resolved_model_load_refreshes_stale_persisted_context_length(self):
        ...
        with patch("app.domain.routes.get_session", return_value=s), \
             patch("app.domain.routes.j", side_effect=fake_j), \
             patch.dict("sys.modules", {"agent.model_metadata": fake_module}):
            routes.handle_get(handler, parsed)
    ...
    def test_session_model_update_refreshes_context_metadata(self):
        ...
        with patch("app.domain.routes._check_csrf", return_value=True), \
             patch("app.domain.routes.read_body", return_value=body), \
             patch("app.domain.routes.get_session", return_value=s), \
             patch("app.domain.routes.resolve_trusted_workspace", return_value="/tmp"), \
             patch("app.domain.routes.j", side_effect=fake_j), \
             patch.dict("sys.modules", {"agent.model_metadata": fake_module}):
...
class TestIssue1436SourceMarkers:
    ...
    def test_routes_load_path_imports_get_model_context_length(self):
        ...
        # The GET /api/session load path now delegates to the shared
        # build_session_detail_payload() helper (consumed by both the legacy
        # bridge and the native v1 endpoint), but the lazy import must remain
        # in routes.py so WebUI still works with older/missing agent bundles by
        # swallowing metadata-resolution failures.
        start = src.find("def build_session_detail_payload(")
        end = src.find("\ndef handle_get(", start)
        block = src[start:end]
        assert "_resolve_context_length_for_session_model" in block, (
            "build_session_detail_payload must resolve model context "
    ...
    def test_routes_load_path_marks_fix_with_issue_number(self):
        ...
        src = ROUTES.read_text(encoding="utf-8")
        # The load-path logic lives in the shared build_session_detail_payload()
        # helper; pin the #1436 trail there.
        start = src.find("def build_session_detail_payload(")
        end = src.find("\ndef handle_get(", start)
        block = src[start:end]
        assert "#1436" in block, (
            "build_session_detail_payload must reference #1436 in a comment"
        )
