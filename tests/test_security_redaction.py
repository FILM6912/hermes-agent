...
def test_redact_value_str():
    ...
    from app.domain.helpers import _redact_value
...
def _create_session_with_credentials() -> str:
    ...
    try:
        ...
        from app.domain.config import SESSION_DIR as sessions_dir
...
def test_api_session_redacts_messages():
    ...
    # The /api/session payload is built by the shared
    # build_session_detail_payload() helper (used by both the legacy bridge and
    # the native v1 endpoint); redaction must happen there.
    src = inspect.getsource(routes.build_session_detail_payload)
    # Verify redact_session_data is applied to the session payload
    assert "redact_session_data" in src, (
        "api/routes.py build_session_detail_payload must call "
        "redact_session_data() on the /api/session response"
    )
