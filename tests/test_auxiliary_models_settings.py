...
class TestAuxiliaryModelsI18n:
    ...
    def test_all_locales_have_auxiliary_keys(self):
        """Count of each key should equal the number of locales (13 with Thai)."""
        for key in self.REQUIRED_KEYS:
            count = I18N_JS.count(f"{key}:")
            assert count == 13, (
                f"i18n key '{key}' found {count} times — expected 13 (one per locale)"
            )
...
class TestAuxiliaryModelsBackend:
    ...
    ROUTES_PY = (ROOT / "app" / "domain" / "routes.py").read_text(encoding="utf-8")
