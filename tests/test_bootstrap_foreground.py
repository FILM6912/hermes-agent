...
class TestMainForegroundRouting:
    ...
    def test_default_path_uses_popen(self, stub_main_dependencies, clean_env, monkeypatch):
        ...
        popen_argv = popen_calls[0][0][0]
        assert "-m" in popen_argv and "uvicorn" in popen_argv and "app.main:app" in popen_argv
    ...
    def test_foreground_flag_uses_execv(self, stub_main_dependencies, clean_env, monkeypatch):
        ...
        assert "-m" in argv and "uvicorn" in argv and "app.main:app" in argv
...
class TestForegroundEnvAndCwd:
    ...
    def test_foreground_exports_resolved_env_vars(self, setup, monkeypatch, clean_env):
        ...
        # Post-execv server inherits these — verify we set them on os.environ
...
class TestBuildServerLaunchArgv:
    """bootstrap._build_server_launch_argv prefers uvicorn when app.main exists."""

    def test_prefers_uvicorn_when_app_main_present(self, import_bootstrap):
        argv = import_bootstrap._build_server_launch_argv(
            "/usr/bin/python3", host="127.0.0.1", port=8787
        )
        assert argv[:4] == ["/usr/bin/python3", "-m", "uvicorn", "app.main:app"]
        assert "--host" in argv and argv[argv.index("--host") + 1] == "127.0.0.1"
        assert "--port" in argv and argv[argv.index("--port") + 1] == "8787"

    def test_falls_back_to_server_py_without_app_main(self, import_bootstrap, monkeypatch, tmp_path):
        monkeypatch.setattr(import_bootstrap, "APP_MAIN", tmp_path / "missing" / "main.py")
        monkeypatch.setattr(import_bootstrap, "SERVER_SCRIPT", tmp_path / "server.py")
        argv = import_bootstrap._build_server_launch_argv(
            "/usr/bin/python3", host="127.0.0.1", port=8787
        )
        assert argv == ["/usr/bin/python3", str(tmp_path / "server.py")]

    def test_invalid_tls_paths_fall_back_to_server_py(self, import_bootstrap, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_WEBUI_TLS_CERT", str(tmp_path / "missing.crt"))
        monkeypatch.setenv("HERMES_WEBUI_TLS_KEY", str(tmp_path / "missing.key"))
        monkeypatch.setattr(import_bootstrap, "SERVER_SCRIPT", tmp_path / "server.py")
        argv = import_bootstrap._build_server_launch_argv(
            "/usr/bin/python3", host="127.0.0.1", port=8787
        )
