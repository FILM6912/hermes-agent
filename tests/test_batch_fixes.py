...
class TestRootWorkspaceUnblocked:
    ...
    def test_root_not_in_blocked_system_roots(self):
        src = read("app/domain/workspace.py")
    ...
    def test_split_guard_present(self):
        ...
        assert "strip_attachment_path_suffix" in src, (
            "base_text split must strip attachment @path suffixes via "
            "strip_attachment_path_suffix to avoid empty-string on plain messages"
        )
...
class TestCronSkillCacheInvalidation:
    ...
    def _panels_src(self):
        return read("static-legacy/panels.js")
