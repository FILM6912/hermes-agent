...
Option A rewrite (2026-05-08): imports app.domain.models and app.domain.profiles
...
# ── Ensure the repo root is on sys.path so app.domain.* imports work ────────
...
from app.domain.models import load_projects, save_projects
from app.domain.profiles import get_active_profile_name, _is_root_profile, _profiles_match

# ── Apply --profile override before any module uses get_active_profile_name
if _profile_arg is not None:
    import app.domain.profiles as _profiles
    _profiles._active_profile = _profile_arg

# ── API auth state ─────────────────────────────────────────────────────────
# Mirror the env-var contract used by app.domain.config:32-33 so a non-default
...
# ═══════════════════════════════════════════════════════════════════════════
#  Helpers — filesystem (project CRUD via canonical app.domain.models)
# ═══════════════════════════════════════════════════════════════════════════
...
async def handle_delete_project(arguments: dict) -> list[TextContent]:
    ...
    # os.replace(), which bypassed _write_session_index() in app.domain.models
        ...
        for p in SESSION_DIR.glob("*.json"):
            ...
            try:
                ...
                if session_data.get("project_id") == project_id:
                    sid = p.stem
                    result = _api_post("/api/v1/session/move",
...
async def handle_rename_session(arguments: dict) -> list[TextContent]:
    ...
    result = _api_post("/api/v1/session/rename",
