...
>
> **Frontend:** React + Vite SPA in `frontend/` (UI base: Agent-UI, FILM6912/Agent-UI on GitHub), built to `static/dist/` and served by FastAPI. Hermes WebUI backend `/api/v1/*` is unchanged. Prior react-luminous / Agent-UI is archived — see section 2.5 and `AGENTS.md`.
...
The server stack stays deliberately small: FastAPI (`app.main:app`) plus business
logic under `app/domain/` (formerly top-level `api/`). The **browser UI** is a
Vite-built React SPA (`frontend/` → `static/dist/`) based on
[Agent-UI](https://github.com/FILM6912/Agent-UI) — flat zinc shell, shadcn-style
primitives, left nav rail, and the same three-panel workbench layout (sessions |
chat | workspace). All data still flows through the **Hermes API** (`/api/v1/*`).

Legacy vanilla modules live under `static-legacy/` for rollback reference; new UI
work belongs in `frontend/src/`. Do not commit `frontend/node_modules/` or
`static/dist/` (built in CI/Docker).

**Archive — react-luminous:** branch `feat/react-luminous-frontend` and the
in-repo **Agent-UI** experiment (glass panels, Framer Motion shell)
are historical reference only. Do not extend that stack for new features; follow
Agent-UI conventions and `docs/UIUX-GUIDE.md`.
...
## 2. File Inventory
    ...
    <repo>/
    server.py              Deprecated thin uvicorn launcher + Handler test shim.
                           Prefer ``python -m uvicorn app.main:app`` or bootstrap.py.
    app/                   FastAPI application package.
      __init__.py          Package marker
      main.py              ASGI entrypoint: uvicorn app.main:app (primary)
      api/v1/              Native /api/v1/* routers (endpoints)
      domain/              Business logic (formerly top-level api/; config, models, streaming, …)
      middleware/          Auth, security headers, request plumbing
      repositories/        Data-access layer (profiles, sessions, settings, workspace)
      services/            Request handlers for v1 routes
    ...
    frontend/              React + Vite + TypeScript source (see §2.5)
      package.json         npm scripts: dev, build, typecheck
      vite.config.ts       outDir ../static/dist; dev proxy /api → backend
      src/                 App, routes, features, stores, Agent-UI shell + Hermes features
    static/
      dist/                Vite production build (gitignored; produced by npm run build)
      index.html           Legacy HTML shell (replaced by dist/index.html after cutover)
      style.css            Legacy CSS (see static-legacy/ after archive)
      *.js                 Legacy vanilla modules (ui, sessions, messages, …)
    static-legacy/         Optional archive of pre-React static assets (post-cutover)
    tests/
...
## 2.5 React frontend (Agent-UI base, Hermes API backend)

The React app lives in `frontend/`. Its **UI base** is
[Agent-UI](https://github.com/FILM6912/Agent-UI) (FILM6912/Agent-UI): application
chrome (login, nav rail, bordered panels, shadcn primitives). **Backend** remains
Hermes WebUI: FastAPI `app.main:app` and `/api/v1/*` (JSON + SSE) — Agent-UI does
not replace the API layer; `frontend/src/lib/api.ts` and feature modules call Hermes
endpoints only.

Vite emits hashed assets and `index.html` into `static/dist/`
(`vite.config.ts`: `build.outDir = '../static/dist'`, `base: '/'`).

### Archive: react-luminous

Branch `feat/react-luminous-frontend` and the earlier **Agent-UI**
in-repo shell (glass blur, grain, heavy Framer Motion) are **archived reference
only**. They are not the active design direction. New work uses Agent-UI shell
semantics documented in `docs/UIUX-GUIDE.md` and `DESIGN.md` (§ Agent-UI).

### Stack

| Layer | Choice | Notes |
|-------|--------|--------|
| UI base | [Agent-UI](https://github.com/FILM6912/Agent-UI) | Shell chrome, shadcn/Tailwind tokens; adapted in `frontend/src/` |
| Build | Vite 6 | `npm run build` runs `tsc -b` then `vite build` |
| Framework | React 19 + TypeScript | Components under `frontend/src/` |
| Routing | React Router | `/login`, `/`, `/session/:id` |
| Backend | Hermes `/api/v1/*` | Unchanged FastAPI handlers; Vite dev proxies `/api` |
| Client state | Zustand | Replaces the legacy global `S` object pattern |
| Styling | Tailwind v4 + CSS tokens | `index.css` (shadcn), `theme-bridge.css`, `tokens.css`, `global.css` |

Hermes API contracts are unchanged: all UI data through `/api/v1/*` (JSON + SSE).
...
After cutover, `app/domain/routes.py` serves:

- `/`, `/session/*`, `/login` (client routes) → `static/dist/index.html` (SPA fallback)
- `/assets/*` (and Vite-emitted paths under `dist/`) → hashed bundles from `static/dist/`
- `/api/v1/*` → unchanged FastAPI handlers

Until cutover completes, production may still serve legacy `static/index.html`; run
`npm run build` in `frontend/` before testing the Agent-UI shell against uvicorn.
...
frontend/src/
  main.tsx, App.tsx
  routes/           LoginPage, ShellPage (3-panel)
  lib/              api.ts (apiV1Path, fetch), sse.ts, auth.ts
  stores/           session, chat, workspace, settings, auth
  features/         chat, sessions, workspace, settings, onboarding, admin
  components/       shell/, ui/, motion/
  styles/           index.css, theme-bridge.css, tokens.css, global.css
...
| Command | Purpose |
|---------|---------|
| `npm ci` | Install deps (CI/Docker: `npm ci && npm run build` in `frontend/`) |
| `npm run dev` | Vite dev server; proxies `/api` to `http://localhost:8787` (adjust if backend port differs) |
| `npm run build` | Typecheck + production bundle → `static/dist/` |
| `npm run preview` | Serve `static/dist/` locally without uvicorn |
| `npm run typecheck` | `tsc -b --noEmit` only |

### Local dev workflow

Two processes are typical:

1. **Backend:** `python3 bootstrap.py` (or `uvicorn app.main:app`) on e.g. port 8787/8789 with isolated `HERMES_HOME` / `HERMES_WEBUI_STATE_DIR` when experimenting.
2. **Frontend:** `cd frontend && npm run dev` — open the Vite URL (default `http://localhost:5173`); API calls hit the proxy target in `vite.config.ts`.

For production-like checks without Vite: `npm run build` then open the backend URL so uvicorn serves `static/dist/`.

### Docker

Multi-stage images run `npm ci && npm run build` in `frontend/` and copy `static/dist/`
into the runtime image. Node.js 20 with `npm` and `npx` is copied from the frontend
build stage into the runtime image for stdio MCP servers. `static/dist/` is gitignored;
do not rely on a committed bundle.

### Rollback

Revert the migration commit or check out a pre-React tag/branch. Re-enable legacy
`static/` serving if `static-legacy/` was archived. Set `HERMES_WEBUI_LEGACY_API=1`
only affects API path bridging, not the UI stack.
...
`app/main.py` builds the FastAPI app served via `uvicorn app.main:app`, wires the
`app/middleware` stack, and mounts the `app/api/v1` routers, which dispatch into
`app/services/*` and `app/repositories/*`. Those services call into `app/domain/*`
business-logic modules (config, helpers, models, workspace, upload, streaming), which
in turn import Hermes internals. Legacy `/api/*` routes still bridge through
`app.domain.routes` when `HERMES_WEBUI_LEGACY_API=1`. New code should import
`app.domain.*`. The deprecated `server.py` is only a thin launcher/rollback shim:

    app/domain/streaming.py imports:
      run_agent.AIAgent              Main agent class. Wraps LLM + tool execution.
    app/domain/config.py imports:
      yaml                           Config loading.
    server.py imports:
...
Current structure:
    ...
    <repo>/
      server.py               Deprecated thin uvicorn launcher (~446 lines)
      app/
        main.py               FastAPI entrypoint
        domain/               Business logic (formerly top-level api/)
          routes.py           Legacy GET + POST route handlers (~9772 lines)
...
Route extraction to `app/domain/routes.py` (formerly `api/routes.py`) completed in
Sprint 11. Business logic now lives under `app/domain/`.
`server.py` remains a deprecated thin launcher.
...
1. `require()` and `bad()` helpers in `app/domain/helpers.py` for parameter validation.
...
2. Inspect the relevant module under `app/` (`domain/`, `api/v1/`, `services/`,
   `repositories/`, `middleware/`) or `static/`. Business logic lives in
   `app/domain/`. `server.py` is only a
   deprecated thin uvicorn launcher.
...
`app/domain/workspace.py` has two distinct trust functions — do not collapse them:
