...
import { AuthenticatedPreviewShell } from "@/features/preview/components/AuthenticatedPreviewShell"
      ...
      >
        {openedFile ? (
          ...
          >
            ...
            {openedFile.kind === "pdf" ? (
              <AuthenticatedPreviewShell sourceUrl={openedFile.url}>
                {(url) => (
                  <PDFViewer
                    file={url}
...
function FileSystemGalleryView(props: FileSystemViewProps) {
  ...
  const { url: authenticatedActiveUrl, state: authUrlState } =
    useAuthenticatedPreviewUrl(
      activeViewerKind && activeFileUrl ? activeFileUrl : null,
    )
  const resolvedActiveFileUrl = authenticatedActiveUrl
  const isAuthLoading =
    activeViewerKind !== null &&
    activeFileUrl !== null &&
    authUrlState === "loading"
  const isStageLoading =
    activeFile !== null &&
    (!isSettled ||
      (activeViewerKind !== null && isResolving) ||
      isAuthLoading)
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex min-h-0 min-w-0 flex-1 items-center justify-center p-3">
          {activeEntry ? (
            ...
            ) : activeViewerKind === "image" && resolvedActiveFileUrl ? (
              <img
                src={resolvedActiveFileUrl}
                alt={activeEntry.name}
                className="max-h-full max-w-full rounded-lg object-contain"
              />
            ) : activeViewerKind === "pdf" && resolvedActiveFileUrl ? (
              <div className="size-full overflow-hidden rounded-lg border">
                <PDFViewer
                  key={activeEntry.path}
                  file={resolvedActiveFileUrl}
                  className="h-full"
                  showToolbar={false}
                />
              </div>
            ) : activeViewerKind === "docx" && resolvedActiveFileUrl ? (
              <div className="size-full overflow-hidden rounded-lg border">
                <DocxViewerPreview
                  key={activeEntry.path}
                  src={resolvedActiveFileUrl}
            ...
            ) : activeViewerKind === "xlsx" && resolvedActiveFileUrl ? (
              <div className="size-full overflow-hidden rounded-lg border">
                <XlsxViewerPreview
                  key={activeEntry.path}
                  src={resolvedActiveFileUrl}
