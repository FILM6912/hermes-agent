...
export function XlsxViewerPreview({
  ...
  showUpload?: boolean
  src?: string
  workbookBuffer?: ArrayBuffer | null
  toolbarActions?: React.ReactNode
}) {
  ...
  return (
    <XlsxViewerContent
      ...
      toolbarActions={toolbarActions}
      url={workbookBuffer ? undefined : src}
      externalWorkbookBuffer={workbookBuffer}
    />
  )
}
...
function XlsxViewerContent({
  ...
  toolbarActions?: React.ReactNode
  url?: string
  externalWorkbookBuffer?: ArrayBuffer | null
}) {
  ...
  React.useEffect(() => {
    ...
    async function loadWorkbook(): Promise<void> {
      if (externalWorkbookBuffer) {
        setWorkbookBuffer(externalWorkbookBuffer)
  ...
  if (!url && !uploadedWorkbook && !externalWorkbookBuffer && !workbookBuffer) {
    return (
      <div
        className={cn(
          "flex h-[640px] min-h-0 flex-col overflow-hidden bg-background",
...
function XlsxWorkbookLoadedViewer({
  ...
  const controller = useXlsxViewerController(
    React.useMemo(
      ...
      [fileName, readOnly, workbookBuffer]
    )
  )
