...
COMPACTION_REFERENCE_ROLE = "system"
...
def normalize_compaction_message_role(message):
    """Rewrite synthetic compaction markers to ``role=system`` for model context."""
    if not isinstance(message, dict) or not is_context_compression_marker(message):
        return message
    if str(message.get("role") or "").strip().lower() == COMPACTION_REFERENCE_ROLE:
        return message
    updated = dict(message)
    updated["role"] = COMPACTION_REFERENCE_ROLE
    return updated


def normalize_compaction_message_roles(messages):
    """Normalize every compaction marker in a transcript list."""
    if not isinstance(messages, list):
        return messages
    out = []
    changed = False
    for message in messages:
        normalized = normalize_compaction_message_role(message)
