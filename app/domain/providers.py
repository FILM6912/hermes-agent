...
def _agent_fetch_account_usage_for_home(provider: str, home: Path, *, api_key: str | None = None) -> Any:
    try:
        from app.domain.config import PYTHON_EXE
    except Exception:
        ...
        if pid == "nous":
            try:
                ...
                if live_ids:
                    ...
                    from app.domain.config import _format_nous_label, _build_nous_featured_set
...
def _clean_provider_key_from_config(provider_id: str) -> None:
    ...
    """
    from app.domain.config import _cfg_lock
