...
from app.domain.helpers import redact_session_data, _redact_text
from app.domain.compression_anchor import (
    is_context_compression_marker,
    normalize_compaction_message_role,
    normalize_compaction_message_roles,
    visible_messages_for_anchor,
)
from app.domain.metering import meter
from app.domain.run_journal import RunJournalWriter
from app.domain.turn_journal import append_turn_journal_event_for_stream
from app.domain.usage import prompt_cache_hit_percent
from app.domain.models import get_state_db_session_messages, reconciled_state_db_messages_for_session
...
# Latest live SSE ``put`` for a session while background title generation runs.
# A follow-up turn can register a newer sink so late title events reach the
# client that is still subscribed to the active stream.
SESSION_TITLE_EVENT_SINKS: dict[str, object] = {}
SESSION_TITLE_SINKS_LOCK = threading.Lock()
...
def _resolve_custom_provider_runtime_overrides(
    ...
    _cp_key, _cp_base = resolve_custom_provider_connection(resolved_provider, config_obj)
...
def _resolve_webui_api_max_retries(cfg: dict | None) -> int:
    """Browser sessions should surface provider errors quickly, not block on long 429 backoffs."""
    raw_env = os.getenv("HERMES_WEBUI_API_MAX_RETRIES", "").strip()
    if raw_env:
        try:
            val = int(raw_env)
            if val >= 0:
    ...
    if isinstance(cfg, dict):
        ...
        if isinstance(agent_cfg, dict) and agent_cfg.get("webui_api_max_retries") is not None:
            try:
                val = int(agent_cfg["webui_api_max_retries"])
                if val >= 0:
                    return val
...
def _apply_webui_api_max_retries(agent, cfg: dict | None) -> None:
    """Cap hermes-agent API retries for WebUI so one chat cannot stall for minutes."""
    if agent is None or not hasattr(agent, "_api_max_retries"):
        return
    cap = _resolve_webui_api_max_retries(cfg)
    current = getattr(agent, "_api_max_retries", cap)
    try:
        current = int(current)
    except (TypeError, ValueError):
        current = cap
    agent._api_max_retries = min(max(current, 0), cap)
...
def _resolve_prefill_path(raw: str) -> Path:
    ...
    if not path.is_absolute():
        try:
            from app.domain.config import _get_config_path
...
def _aiagent_import_error_detail() -> str:
    ...
    return "\n".join(lines)
from app.domain.models import get_session, title_from
from app.domain.workspace import set_last_workspace
...
def _build_agent_thread_env(
    ...
    account_workspace_root: str | None = None,
) -> dict:
    ...
    **kwargs alongside an explicit TERMINAL_CWD raises TypeError before the
    ...
    virtual_token = str(workspace_virtual or '').strip()
    if virtual_token:
        env['HERMES_WEBUI_ACTIVE_WORKSPACE_VIRTUAL'] = virtual_token
    root_token = str(account_workspace_root or '').strip()
    if root_token:
        env['HERMES_WEBUI_ACCOUNT_WORKSPACE_ROOT'] = root_token

    # execute_code (project mode) and terminal must share the same Python env.
    # Without VIRTUAL_ENV/PATH, Docker often runs execute_code via /app/venv while
    # the terminal shell picks /usr/local/bin/python3 from PATH — pip installs in
    # one interpreter are invisible to the other.
    try:
    ...
    if venv_root:
        ...
        bin_dir = Path(venv_root) / ('Scripts' if os.name == 'nt' else 'bin')
        if bin_dir.is_dir():
            bin_prefix = str(bin_dir)
            current_path = str(env.get('PATH') or os.environ.get('PATH', '')).strip()
            if current_path and not current_path.startswith(bin_prefix + os.pathsep):
                env['PATH'] = f"{bin_prefix}{os.pathsep}{current_path}"
            elif not current_path:
                env['PATH'] = bin_prefix

    from app.core.outbound_proxy import apply_outbound_proxy_env

    return apply_outbound_proxy_env(env)


def _resolve_agent_session_workspace(
    workspace,
    *,
    profile: str | None = None,
    profile_home: str | Path | None = None,
) -> str:
    """Resolve chat-start workspace to a trusted disk path for agent tools."""
    try:
        ...
        from app.domain.workspace import (
            ...
            get_request_user_access,
        )

        home = Path(profile_home).expanduser() if profile_home else None
        if home is None and profile:
            home = get_hermes_home_for_profile(profile)

        access = get_request_user_access()
        token = str(workspace or "").strip()

        if nested_workspaces_enabled() and home is not None:
            account_root = resolve_main_user_workspace_root(
                token if is_virtual_workspace_path(token) else None,
                profile_home=home,
                access=access,
                profile_name=profile,
            )
            if is_virtual_workspace_path(token) or not token:
                if not token or token.rstrip("/") == "/workspace":
                    return str(account_root)
                return str(
                    virtual_path_to_disk(
                        token,
                        home,
                        terminal_cwd=str(account_root),
                        active_workspace_virtual=token,
                    )
                )
...
def _apply_process_agent_env_unlocked(thread_env: dict) -> None:
    """Write per-run agent env keys into os.environ (caller must hold ``_ENV_LOCK``)."""
    for key, value in (thread_env or {}).items():
        if value is None:
...
def _pin_process_agent_env(thread_env: dict) -> None:
    """Apply per-run agent env so Hermes tools see the active session workspace.

    File and terminal tools in hermes-agent read process-global ``os.environ``
    rather than the WebUI thread-local context.  Re-pin before each tool
    invocation so concurrent sessions cannot clobber ``TERMINAL_CWD`` mid-run.
    """
    if not thread_env:
        return
    with _ENV_LOCK:
        _apply_process_agent_env_unlocked(thread_env)
...
def _display_attachment_record(att) -> dict | str:
    """Persist attachment metadata for chat reload (path when available)."""
    if isinstance(att, dict):
        path = str(att.get('path') or '').strip()
        workspace_rel = str(att.get('workspace_rel') or '').strip()
        name = _attachment_name(att)
        if path or workspace_rel:
            record = {
                'name': name or Path(path or workspace_rel).name,
            }
            if path:
                record['path'] = path
            if workspace_rel:
                record['workspace_rel'] = workspace_rel
            return record
        if name:
            return name
    name = _attachment_name(att)
...
def _attachment_name(att) -> str:
    if isinstance(att, dict):
        ...
        return str(att.get('name') or att.get('filename') or '').strip()
    text = str(att or '').strip()
    if not text:
        return ''
...
_VISION_PROBE_CACHE: dict[tuple[str, str], tuple[bool | None, float]] = {}
_VISION_PROBE_LOCK = threading.Lock()
_VISION_PROBE_TTL_SECONDS = 3600.0


def _model_catalog_entry_supports_multimodal(entry: dict) -> bool:
    """Return True when an OpenAI-compatible /v1/models row advertises vision input."""
    caps = entry.get('capabilities')
    if isinstance(caps, list):
        normalized = {str(item).strip().lower() for item in caps}
        if normalized & {'multimodal', 'vision', 'image', 'images'}:
            return True
    for key in ('modalities', 'input_modalities'):
        values = entry.get(key)
        if isinstance(values, list):
            normalized = {str(item).strip().lower() for item in values}
            if normalized & {'multimodal', 'vision', 'image', 'images', 'image_url'}:
                return True
    return False


def _openai_compatible_catalog_ids(entry: dict) -> list[str]:
    ids: list[str] = []
    ...
    if isinstance(aliases, list):
        ids.extend(str(item).strip() for item in aliases if str(item).strip())
    return ids


def _catalog_entry_matches_model(entry: dict, model_token: str) -> bool:
    """Return True when a /v1/models row corresponds to the active model id."""
    token = str(model_token or '').strip()
    if not token:
        return False
    ids = _openai_compatible_catalog_ids(entry)
    if token in ids:
        return True
    for catalog_id in ids:
        if token.startswith(f'{catalog_id}:') or catalog_id.startswith(f'{token}:'):
            return True
    return False


def _coerce_capability_bool(raw) -> bool | None:
    try:
        from agent.image_routing import _coerce_capability_bool as _coerce
...
def _active_model_id_from_cfg(
    model_cfg: dict | None,
    *,
    explicit: str | None = None,
) -> str:
    """Resolve the active model id from config, including ``model.default``."""
    token = str(explicit or '').strip()
    if token:
        return token
    if not isinstance(model_cfg, dict):
        return ''
    return str(model_cfg.get('model') or model_cfg.get('default') or '').strip()


def _custom_provider_model_meta(
    cfg: dict | None,
    *,
    provider: str | None = None,
    model: str | None = None,
) -> tuple[dict | None, dict | None]:
    """Return ``(provider_entry, per_model_entry)`` for the active custom runtime."""
    if not isinstance(cfg, dict):
        return None, None

    model_cfg = cfg.get('model') if isinstance(cfg.get('model'), dict) else {}
    resolved_model = _active_model_id_from_cfg(model_cfg, explicit=model)
    resolved_provider = str(provider or model_cfg.get('provider') or '').strip().lower()
    if not resolved_model:
        return None, None

    slug = ''
    if resolved_provider.startswith('custom:'):
        slug = resolved_provider.split(':', 1)[1].strip().lower()

    matched_entry: dict | None = None
    matched_model_entry: dict | None = None
    for entry in cfg.get('custom_providers') or []:
        ...
        matches_slug = bool(slug and name == slug)
        matches_model = bool(
            resolved_model
            and (
                entry_model == resolved_model
                or resolved_model in models_map
            )
        )
        if slug:
            if not matches_slug:
                continue
        elif resolved_provider == 'custom' and resolved_model:
            if not matches_model and entry_model and entry_model != resolved_model:
                continue
        per_model = models_map.get(resolved_model)
        per_model_entry = per_model if isinstance(per_model, dict) else None
        matched_entry = entry
        matched_model_entry = per_model_entry
        if matches_slug or matches_model or entry_model == resolved_model:
            break

    return matched_entry, matched_model_entry


def _lookup_custom_provider_supports_vision(
    cfg: dict | None,
    *,
    provider: str | None = None,
    model: str | None = None,
) -> bool | None:
    """Resolve vision capability from ``custom_providers`` config metadata."""
    provider_entry, model_entry = _custom_provider_model_meta(
        cfg,
        provider=provider,
        model=model,
    )
    for entry in (model_entry, provider_entry):
        if not isinstance(entry, dict):
            continue
        declared = _coerce_capability_bool(entry.get('supports_vision'))
        if declared is not None:
            return declared
        if _model_catalog_entry_supports_multimodal(entry):
            return True
    return None


_MULTIMODAL_MODEL_NAME_HINTS = (
    re.compile(r'qwen3\.6', re.IGNORECASE),
    re.compile(r'qwen[._-]?2(?:\.5)?[._-]?vl', re.IGNORECASE),
    re.compile(r'qwen[._-]?3[._-]?vl', re.IGNORECASE),
    re.compile(r'(?:^|[/_-])vl(?:[/_-]|$|:)', re.IGNORECASE),
    re.compile(r'llava|minicpm-v|pixtral|gpt-4o|gpt-4-vision|gemini.*(?:vision|flash|pro)', re.IGNORECASE),
)


def _infer_multimodal_from_model_name(model_id: str | None) -> bool:
    """Best-effort name-based hint when catalog probes are unavailable."""
    token = str(model_id or '').strip()
    if not token:
        return False
    return any(pattern.search(token) for pattern in _MULTIMODAL_MODEL_NAME_HINTS)


def _custom_openai_runtime_from_cfg(
    cfg: dict | None,
    *,
    provider: str | None = None,
    model: str | None = None,
) -> tuple[str | None, str | None, str | None]:
    """Resolve base URL, API key, and model id for custom OpenAI-compatible providers."""
    if not isinstance(cfg, dict):
    ...
    if not base_url:
        return None, None, resolved_model or None
    if not api_key:
        api_key = _KEYLESS_CUSTOM_API_KEY
    return base_url, api_key, resolved_model or None


def _probe_openai_compatible_multimodal(
    base_url: str,
    model_id: str,
    api_key: str,
) -> bool | None:
    """Best-effort probe of ``GET /v1/models`` for multimodal/vision capability."""
    base = str(base_url or '').strip().rstrip('/')
    model_token = str(model_id or '').strip()
    if not base or not model_token:
        return None

    cache_key = (base, model_token)
    now = time.time()
    with _VISION_PROBE_LOCK:
        cached = _VISION_PROBE_CACHE.get(cache_key)
        if cached is not None and now - cached[1] < _VISION_PROBE_TTL_SECONDS:
            return cached[0]
    ...
    try:
        ...
        request = urllib.request.Request(
            f'{base}/models',
            headers={'Authorization': f'Bearer {api_key or _KEYLESS_CUSTOM_API_KEY}'},
        )
        with urllib.request.urlopen(request, timeout=4) as response:
            payload = json.loads(response.read().decode('utf-8'))
    except Exception:
        logger.debug('OpenAI-compatible vision probe failed for %s', base, exc_info=True)
        result = None
    else:
        entries: list[dict] = []
        ...
        for entry in entries:
            if not _catalog_entry_matches_model(entry, model_token):
                continue
            matched = True
            if _model_catalog_entry_supports_multimodal(entry):
                result = True
                break
        if result is None:
            result = False if matched else None

    with _VISION_PROBE_LOCK:
        _VISION_PROBE_CACHE[cache_key] = (result, now)
    return result


def _webui_extended_supports_vision(
    cfg: dict | None,
    *,
    provider: str | None = None,
    model: str | None = None,
) -> bool:
    """WebUI vision hints beyond agent ``_lookup_supports_vision`` / models.dev.

    Covers custom-provider metadata, OpenAI-compatible ``/v1/models`` probes,
    ``model.supports_vision``, and known multimodal model-name patterns (e.g.
    Qwen3.6) so local GGUF endpoints are not misclassified as text-only.
    """
    ...
    if isinstance(cfg, dict):
        ...
        if isinstance(model_cfg, dict):
            resolved_model = _active_model_id_from_cfg(model_cfg, explicit=resolved_model)
            if not resolved_provider:
                resolved_provider = str(model_cfg.get("provider") or "").strip()

    if not resolved_model and not resolved_provider:
        return False

    custom_lookup = _lookup_custom_provider_supports_vision(
        cfg,
        provider=resolved_provider,
        model=resolved_model,
    )
    if custom_lookup is True:
        return True

    probed: bool | None = None
    provider_token = str(resolved_provider or "").strip().lower()
    if provider_token == "custom" or provider_token.startswith("custom:"):
        base_url, api_key, model_id = _custom_openai_runtime_from_cfg(
            cfg,
            provider=resolved_provider,
            model=resolved_model,
        )
        if base_url and model_id:
            probed = _probe_openai_compatible_multimodal(base_url, model_id, api_key or "")
            if probed is True:
                return True
            if probed is False:
                return False
            if _infer_multimodal_from_model_name(model_id):
                return True

    top_override = _coerce_capability_bool(
        model_cfg.get("supports_vision") if isinstance(model_cfg, dict) else None,
    )
    if top_override is True:
        return True
    if _infer_multimodal_from_model_name(resolved_model):
        return True

    if custom_lookup is False:
        return False
    if top_override is False:
        return False
    if probed is False:
        return False
    return False
...
def _resolve_image_input_mode(
    ...
    *,
    ...
    """Return ``"native"`` or ``"text"`` using Hermes Agent image routing."""
    resolved_provider = str(provider or "").strip()
    resolved_model = str(model or "").strip()
    model_cfg: dict = {}
    if isinstance(cfg, dict):
    ...
    if cfg is None and not resolved_provider and not resolved_model:
        return "native"
    ...
    try:
        ...
        lookup = _lookup_supports_vision(resolved_provider, resolved_model, cfg)
        if lookup is True or _webui_extended_supports_vision(
            cfg,
            provider=resolved_provider,
            model=resolved_model,
        ):
        ...
        # Auxiliary vision is a fallback for non-vision main models. When the
        # active model is natively multimodal (custom catalog, probe, or name
        # hint), keep pixels on the main model instead of forcing vision_analyze.
        if _explicit_aux_vision_override(cfg):
            return "text"

        mode = decide_image_input_mode(resolved_provider, resolved_model, cfg)
        if mode == "native":
            return "native"
        if lookup is False:
            return "text"
        ...
        logger.debug("image routing lookup failed; falling back to cfg-only mode", exc_info=True)
...
def _should_embed_native_images_for_turn(
    cfg: dict | None = None,
    *,
    provider: str | None = None,
    model: str | None = None,
) -> bool:
    """Embed current-turn uploads only when the active model supports native vision."""
    return _resolve_image_input_mode(cfg, provider=provider, model=model) == "native"


def _is_custom_openai_compatible_provider(provider: str | None) -> bool:
    """True for OpenAI-compatible custom endpoints (``custom`` / ``custom:slug``)."""
    token = str(provider or "").strip().lower()
    return token == "custom" or token.startswith("custom:")


def _preserve_upload_path_hints_for_native_turn(provider: str | None) -> bool:
    """Keep ``@.uploads/`` tokens for custom endpoints that use ``vision_analyze``.

    Many local GGUF / OpenAI-compatible servers accept images only through the
    ``vision_analyze`` tool on ``@path`` hints, not inline ``image_url`` parts on
    the main chat completion request. Stripping upload paths for a native vision
    turn leaves the model with ``[1 image]`` text and no pixels.
    """
    return _is_custom_openai_compatible_provider(provider)
...
def _build_native_multimodal_message(
    ...
    *,
    ...
    Native embedding is gated by ``decide_image_input_mode`` (same as Hermes
    Agent/CLI): only vision-capable main models receive ``image_url`` parts.
    Text-only models keep path hints and use ``vision_analyze`` instead.
    """
    if not attachments:
        return workspace_ctx + msg_text

    if not _should_embed_native_images_for_turn(cfg, provider=provider, model=model):
        return workspace_ctx + msg_text
    ...
    for att in attachments or []:
        if not isinstance(att, dict):
            continue
        raw_path = str(
            att.get('workspace_rel') or att.get('path') or ''
        ).strip()
        if not raw_path:
            continue
        try:
            from app.domain.upload import (
                path_allowed_for_native_multimodal_image,
                resolve_agent_attachment_path,
            )

            resolved_path = resolve_agent_attachment_path(
                raw_path,
                session_workspace=workspace,
                profile_home=profile_home,
            )
            path = Path(resolved_path).expanduser().resolve()
            if not path_allowed_for_native_multimodal_image(
                path,
                workspace,
                profile_home=profile_home,
            ):
...
_EMPTY_RECOVERY_USER_PREFIX = (
    "You just executed tool calls but returned an empty response."
)


def _is_empty_recovery_user_nudge(msg) -> bool:
    if not isinstance(msg, dict) or msg.get('role') != 'user':
        return False
    if msg.get('_empty_recovery_synthetic'):
        return True
    text = _message_text(msg.get('content', '')).strip()
    return text.startswith(_EMPTY_RECOVERY_USER_PREFIX)


def _is_empty_recovery_assistant_placeholder(msg) -> bool:
    if not isinstance(msg, dict) or msg.get('role') != 'assistant':
        return False
    text = _message_text(msg.get('content', '')).strip()
    if text != '(empty)':
        return False
    return bool(msg.get('_empty_recovery_synthetic'))


def _empty_recovery_assistant_notice() -> str:
    return (
        "โมเดลยังไม่ตอบหลังเรียกเครื่องมือ — กำลังดึงให้ดำเนินการต่อ..."
    )


def _remap_message_for_display(msg) -> dict | None:
    """Hide or remap synthetic agent rows for the WebUI transcript."""
    if not isinstance(msg, dict):
        return None
    if _is_empty_recovery_assistant_placeholder(msg):
        return None
    if _is_empty_recovery_user_nudge(msg):
        remapped = {
            'role': 'assistant',
            'content': _empty_recovery_assistant_notice(),
            '_display_synthetic': True,
        }
        if msg.get('timestamp') is not None:
            remapped['timestamp'] = msg['timestamp']
        return remapped
    if _is_agent_injected_user_turn(msg):
        return None
    if msg.get('role') == 'user':
        content = msg.get('content')
        if isinstance(content, str):
            from app.domain.upload import strip_attachment_paths_for_display

            display_content = strip_attachment_paths_for_display(content)
            if display_content != content:
                remapped = dict(msg)
                remapped['content'] = display_content
                return remapped
    return msg


def _remap_messages_for_display(messages) -> list:
    """Return transcript rows safe for WebUI rendering (no agent-only @path hints)."""
    remapped = []
...
def prepare_session_dict_for_api_display(session_dict: dict) -> dict:
    """Shape session payloads for API/SSE clients without mutating stored state."""
    result = dict(session_dict)
    if isinstance(result.get('messages'), list):
        result['messages'] = _remap_messages_for_display(result['messages'])
    pending = result.get('pending_user_message')
    if isinstance(pending, str):
        from app.domain.upload import strip_attachment_paths_for_display

        result['pending_user_message'] = strip_attachment_paths_for_display(pending)
    return result


def _is_agent_injected_user_turn(msg) -> bool:
    """True for synthetic user rows the agent uses for steering, not human chat.

    Hermes Agent appends continuation nudges (network cut-off, length limit,
    oversized tool retry, empty-response recovery) and WebUI background-process
    notifications as ``role=user`` messages prefixed with ``[System:`` or
    ``[IMPORTANT:``. They must stay in ``context_messages`` for the model but
    should not render as user bubbles in the WebUI transcript.
    """
    if _is_empty_recovery_user_nudge(msg):
        return True
    if not isinstance(msg, dict) or msg.get('role') != 'user':
    ...
    if not text:
        return False
    return text.startswith('[System:') or text.startswith('[IMPORTANT:')
...
def _workspace_context_prefix(
    ...
    *,
    ...
    return f"[Workspace::v1: {_escape_workspace_prefix_path(visible)}]\n"
...
_SHORT_GENERIC_USER_TITLE_PATTERNS = (
    re.compile(
        r'^(?:what(?:\s+do you)?\s+see|describe (?:this|the image|it)|'
        r'what(?:\'s| is) in (?:this|the) (?:image|picture|photo)|'
        r'look at this|analyze this image)\b',
        re.IGNORECASE,
    ),
    re.compile(
        r'^(?:'
...
def _is_short_generic_user_prompt(text: str) -> bool:
    """True when the user's opening line is too vague to use as a session title."""
    s = re.sub(r'\s+', ' ', str(text or '')).strip()
    if not s:
        return False
    for pattern in _SHORT_GENERIC_USER_TITLE_PATTERNS:
        if pattern.search(s):
            return True
    if len(s) <= 12 and (
        '?' in s
...
def _first_user_message_for_title(messages):
    for m in messages or []:
        if not isinstance(m, dict) or m.get('role') != 'user':
            continue
        if _is_agent_injected_user_turn(m):
            continue
        text = _message_text(m.get('content'))
        if text or m.get('attachments'):
            return m
    return None


def _attachment_summary_for_title(msg) -> str:
    """Summarize uploaded files/images for title-generation prompts."""
    if not isinstance(msg, dict):
        return ''
    names = []
    for att in msg.get('attachments') or []:
        if isinstance(att, dict):
            name = att.get('name') or att.get('filename') or att.get('path') or ''
        else:
            name = str(att)
    ...
    if isinstance(content, list):
        for part in content:
            if isinstance(part, dict) and (
                ...
                image_count += 1
    if not names and image_count:
        if image_count == 1:
            return '[User attached 1 image]'
        return f'[User attached {image_count} images]'
    if not names:
        return ''
    if len(names) == 1:
        return f'[User attached: {names[0]}]'
    shown = ', '.join(names[:3])
    if len(names) > 3:
        shown += f', +{len(names) - 3} more'
    return f'[User attached: {shown}]'


def _enrich_user_text_for_title(user_text: str, messages) -> str:
    user_msg = _first_user_message_for_title(messages)
    attachment_note = _attachment_summary_for_title(user_msg) if user_msg else ''
    base = re.sub(r'\s+', ' ', str(user_text or '')).strip()
    if attachment_note:
        if base:
            return f'{base}\n{attachment_note}'[:500]
        return attachment_note[:500]
    return base[:500]


def _fallback_title_from_attachment_note(user_text: str) -> Optional[str]:
    """Derive a short topic label from attachment names when the LLM is unavailable."""
    text = str(user_text or '')
    match = re.search(r'\[User attached:\s*([^\]]+)\]', text)
    if match:
        raw_names = match.group(1).strip()
        primary = raw_names.split(',')[0].strip()
        if primary.startswith('+') and ' more' in primary:
            return 'Attached files review'
        stem = Path(primary).stem.strip()
        if stem:
            return stem[:60]
    if '[User attached' in text:
        return 'Image analysis'
    return None


def _title_echoes_assistant(next_title: str, assistant_text: str) -> bool:
    """True when the title copies the assistant answer instead of summarizing the topic."""
    title_norm = _normalize_title_compare(next_title)
    if not title_norm:
        return False
    assistant_clean = re.sub(r'\s+', ' ', _strip_thinking_markup(assistant_text or '')).strip()
    if not assistant_clean:
        return False
    first_clause = re.split(r'[.!?\n\u3002\uff01\uff1f]', assistant_clean)[0].strip()
    clause_norm = _normalize_title_compare(first_clause)
    if not clause_norm:
        return False
    if title_norm == clause_norm:
        return True
    if len(title_norm) >= 20 and clause_norm.startswith(title_norm):
        return True
    if len(title_norm) >= 15 and title_norm in clause_norm and len(title_norm) / len(clause_norm) > 0.7:
        return True
    return False


def _title_is_unhelpful_echo(
    next_title: str,
    user_text: str,
    messages,
    *,
    ...
    """Reject titles that merely repeat the provisional first-message slice."""
    norm = _normalize_title_compare(next_title)
    if not norm:
        return True
    plain_user = str(user_text or '').split('\n', 1)[0].strip()
    if _normalize_title_compare(plain_user) == norm and _is_short_generic_user_prompt(plain_user):
        return True
    provisional = title_from(messages, '')
    if provisional and _normalize_title_compare(provisional[:64]) == norm:
        return True
    if current_title and _normalize_title_compare(current_title) == norm:
        if _is_provisional_title(current_title, messages):
            return True
    if assistant_text and _title_echoes_assistant(next_title, assistant_text):
        return True
    return False
...
def _exchange_snippets_for_title(messages):
    """First-exchange snippets enriched with attachment/image context for titles."""
    user_text, asst_text = _first_exchange_snippets(messages)
    return _enrich_user_text_for_title(user_text, messages), asst_text
...
def _latest_exchange_snippets(messages):
    ...
    for m in reversed(messages or []):
        ...
        if user_text and asst_text:
            break
    user_text = _enrich_user_text_for_title(user_text, messages)
    return user_text[:500], asst_text[:500]
...
_ATTACH_PATH_TITLE_RE = re.compile(r'(?:\s|^)@[^\s]+')


def _provisional_title_candidates(derived: str) -> list[str]:
    """Comparable provisional titles from the first user message slice.

    chat/start writes the raw prompt immediately, but the stored first user
    message may later include ``@.uploads/...`` paths or workspace tags.
    """
    raw = re.sub(r'\s+', ' ', str(derived or '')).strip()
    if not raw:
        return []
    candidates = [raw]
    first_line = re.sub(r'\s+', ' ', str(derived).split('\n', 1)[0]).strip()
    if first_line and first_line not in candidates:
        candidates.append(first_line)
    stripped = _ATTACH_PATH_TITLE_RE.sub('', raw)
...
def _is_provisional_title(current_title: str, messages) -> bool:
    ...
    if not current:
        return False
    for candidate in _provisional_title_candidates(derived[:64]):
        if current == candidate:
            return True
    return False


def _is_misassigned_provisional_title(current_title: str, messages) -> bool:
    """True when the title matches a later user prompt, not the first exchange."""
    derived_first = title_from(messages, '') or ''
    if not derived_first:
        return False
    current = re.sub(r'\s+', ' ', str(current_title or '')).strip()
    if not current or _is_provisional_title(current, messages):
        return False
    first_slice = re.sub(r'\s+', ' ', derived_first[:64]).strip()
    ...
    for m in messages or []:
        ...
        if not candidate:
            continue
        prompt_slice = re.sub(r'\s+', ' ', candidate[:64]).strip()
        if current == prompt_slice and prompt_slice != first_slice:
            return True
    return False


def _register_title_event_sink(session_id: str, put_event) -> None:
    with SESSION_TITLE_SINKS_LOCK:
        SESSION_TITLE_EVENT_SINKS[session_id] = put_event


def _resolve_title_event_sink(session_id: str, fallback_put):
    with SESSION_TITLE_SINKS_LOCK:
        return SESSION_TITLE_EVENT_SINKS.get(session_id, fallback_put)


def _clear_title_event_sink(session_id: str, put_event) -> None:
    with SESSION_TITLE_SINKS_LOCK:
        if SESSION_TITLE_EVENT_SINKS.get(session_id) is put_event:
            SESSION_TITLE_EVENT_SINKS.pop(session_id, None)


def _should_schedule_background_title(session) -> tuple[bool, str, str]:
    """Return whether to spawn background title generation for *session*."""
    current = re.sub(r'\s+', ' ', str(getattr(session, 'title', '') or '')).strip()
    messages = getattr(session, 'messages', None) or []
    looks_default = current in ('Untitled', 'New Chat', '') or not current
    looks_provisional = _is_provisional_title(current, messages)
    invalid_existing = _looks_invalid_generated_title(current)
    misassigned = _is_misassigned_provisional_title(current, messages)
    needs_auto_title = looks_default or looks_provisional or invalid_existing or misassigned
    llm_generated = bool(getattr(session, 'llm_title_generated', False))
    should = needs_auto_title and (
        not llm_generated or looks_provisional or invalid_existing or misassigned
    )
    user_text = ''
    asst_text = ''
    if should:
        user_text, asst_text = _exchange_snippets_for_title(messages)
        if not user_text and not asst_text:
            should = False
    return should, user_text, asst_text
...
def _title_prompts(user_text: str, assistant_text: str) -> tuple[str, list[str]]:
    ...
    vision_hint = ''
    if '[User attached' in user_text or 'image' in user_text.lower():
        vision_hint = (
            "The user attached images or files. Read the exchange and name the topic "
            "being analyzed (object, model, document), not the user's short question.\n"
        )
    generic_hint = ''
    plain_user = user_text.split('\n', 1)[0].strip()
    if _is_short_generic_user_prompt(plain_user):
        generic_hint = (
            "The user's opening line is a short generic prompt (for example 'what do you see'). "
            "Do NOT repeat that prompt. Read the full exchange and write a topic label.\n"
        )
    synthesis_rules = (
        "Read the exchange above and write a short descriptive session title.\n"
        "Synthesize the topic in your own words as a concise label (3-8 words).\n"
        "Do NOT repeat the user's question verbatim.\n"
        "Do NOT copy or quote the assistant answer verbatim.\n"
        "Do NOT paste the assistant's opening sentence.\n"
        "Write the title in the same language as the user's message (e.g. Thai if they wrote in Thai).\n"
        "Return only the title text, as a noun-phrase topic label.\n"
        ...
        "Bad: repeating the user's short question, copying the assistant reply, "
        "The user is asking..., OK, all set.\n"
        "Good: Title Generation Test, Clarify Dialog Layout, GitHub Issue Triage, "
        "3D CAD Minipack Model Review"
    )
    prompts = [
        "Generate a short session title from this conversation start.\n" + vision_hint + generic_hint + synthesis_rules,
        (
            "Rewrite this conversation start as a concise noun-phrase title.\n"
            "Use the same language as the user's message.\n"
            "Name the topic discussed, not the task outcome.\n"
            + vision_hint
            + generic_hint
            + "Return title text only.\n"
            "Do not repeat the user question or copy the assistant answer.\n"
            "Do not use markdown, bullets, labels, or prefixes like Session Title:.\n"
...
def _supports_title_reasoning_extra_body(provider: str = '', model: str = '', base_url: str = '') -> bool:
    """Return True when auxiliary title calls may send ``extra_body.reasoning``.

    Strict OpenAI-compatible providers (Z.AI/GLM, etc.) reject unknown
    extra_body fields with HTTP 400 instead of ignoring them.
    """
    text = ' '.join([
        str(provider or '').lower(),
        str(model or '').lower(),
    ...
    if any(token in text for token in (
        'z.ai', 'bigmodel.cn', 'zhipu', 'open.bigmodel', '/glm', 'glm-', 'glm_',
    )):
        return False
    return 'openrouter' in text or 'nousresearch' in text


def _title_aux_extra_body(provider: str = '', model: str = '', base_url: str = '') -> dict:
    """Provider-safe extra_body for auxiliary title-generation calls."""
    extra: dict = {}
    if _is_minimax_route(provider, model, base_url):
        extra['reasoning_split'] = True
        return extra
    if _supports_title_reasoning_extra_body(provider, model, base_url):
...
def generate_title_raw_via_aux(
    ...
    reasoning_extra = _title_aux_extra_body(provider, model, base_url)
    try:
        ...
        for idx, prompt in enumerate(prompts):
            ...
            try:
                for budget_idx, max_tokens in enumerate(budgets):
                    ...
                    if reasoning_extra:
                        llm_kwargs['extra_body'] = reasoning_extra
                    resp = call_llm(**llm_kwargs)
...
def _normalize_title_route_key(provider: str, model: str, base_url: str) -> tuple[str, str, str]:
    """Comparable provider/model/base_url tuple for title LLM route de-duplication."""
    return (
        re.sub(r'\s+', ' ', str(provider or '')).strip().lower(),
        re.sub(r'\s+', ' ', str(model or '')).strip().lower(),
        re.sub(r'\s+', ' ', str(base_url or '')).strip().lower().rstrip('/'),
    )


def _agent_title_route_key(agent) -> tuple[str, str, str]:
    return _normalize_title_route_key(
        getattr(agent, 'provider', ''),
        getattr(agent, 'model', ''),
        getattr(agent, 'base_url', ''),
    )
...
def _configured_aux_title_route_key() -> tuple[str, str, str]:
    configured = _get_aux_title_config()
...
def _generate_session_title_llm(user_text: str, assistant_text: str, agent=None) -> tuple[Optional[str], str, str]:
    """Generate a session title via LLM, preferring the active chat model.

    Order when *agent* is available:
      1. Direct agent API (session provider/model/base_url)
      2. Auxiliary client with the same session model attrs (when distinct from configured aux)
      3. ``auxiliary.title_generation`` config (when not already attempted via aux)

    When *agent* is unavailable, only step 3 (and local fallback upstream) apply.
    """
    aux_routes_tried: set[tuple[str, str, str]] = set()
    last_status = 'llm_error'
    last_preview = ''

    if agent:
        session_route = _agent_title_route_key(agent)
        next_title, llm_status, raw_preview = _generate_llm_session_title_for_agent(
            agent, user_text, assistant_text,
        )
        if next_title:
            return next_title, llm_status, raw_preview
        last_status = llm_status or last_status
        last_preview = raw_preview or last_preview

        configured_route = _configured_aux_title_route_key()
        if session_route != configured_route:
            next_title, llm_status, raw_preview = _generate_llm_session_title_via_aux(
                user_text, assistant_text, agent=agent, use_agent_model=True,
            )
            aux_routes_tried.add(session_route)
            if next_title:
                return next_title, llm_status, raw_preview
            last_status = llm_status or last_status
            last_preview = raw_preview or last_preview
    ...
    configured_route = _configured_aux_title_route_key()
    if configured_route not in aux_routes_tried or not agent:
        next_title, llm_status, raw_preview = _generate_llm_session_title_via_aux(
...
def _generate_llm_session_title_via_aux(user_text: str, assistant_text: str, agent=None, *, use_agent_model: bool = False) -> tuple[Optional[str], str, str]:
    """Generate a title via auxiliary LLM route, then sanitize/validate result.

    When use_agent_model is False (default), the auxiliary client resolves
    provider/model/base_url from config.yaml auxiliary.title_generation.
    When True, the active agent's provider/model/base_url are passed through
    so the title call tracks the session's selected chat model.
    """
...
def _put_stream_close(put_event, session_id: str) -> None:
    """Close the live chat SSE channel after optional post-``stream_end`` events."""
    try:
        put_event('stream_close', {'session_id': session_id})
    except Exception:
        logger.debug("Failed to emit stream_close for session %s", session_id, exc_info=True)
...
def _fallback_title_from_exchange(user_text: str, assistant_text: str) -> Optional[str]:
    ...
    plain_user = user_text.split('\n', 1)[0].strip()
    if _is_short_generic_user_prompt(plain_user) or '[User attached' in user_text:
        from_attachment = _fallback_title_from_attachment_note(user_text)
        if from_attachment:
    ...
    if not tokens:
        ...
        if from_attachment:
            return from_attachment
        # Greetings and other non-generic non-Latin openers: use the assistant
        # opening sentence instead of a vague English placeholder. Skip this for
        # generic vision prompts like "เห็นอะไร" — those should use attachment
        # names or keyword heuristics, not a verbatim assistant answer copy.
        if assistant_text and not _is_short_generic_user_prompt(plain_user):
            asst_head = re.split(r'[.!?\n]', assistant_text)[0].strip()
            asst_head = re.sub(r'\s+', ' ', asst_head).strip()
            if len(asst_head) >= 4:
                return asst_head[:60]
        return 'Conversation topic'
...
def _run_background_title_update(session_id: str, user_text: str, assistant_text: str, placeholder_title: str, put_event, agent=None):
    """Generate and publish a better title after `stream_end` (fire-and-forget)."""
    try:
        try:
        ...
        still_auto = (
            ...
            or _is_misassigned_provisional_title(current, s.messages)
        ...
        with profiles_api.profile_env_for_background_worker(s, "background title", logger_override=logger):
            next_title, llm_status, raw_preview = _generate_session_title_llm(
                user_text, assistant_text, agent=agent,
            )
            if next_title and _title_is_unhelpful_echo(
                next_title,
                user_text,
                s.messages,
                ...
                logger.debug("Rejecting unhelpful echo title: %r", next_title)
                raw_preview = raw_preview or str(next_title)
                next_title = None
                llm_status = llm_status or 'llm_echo'
            source = llm_status
            if not next_title:
            ...
            with _get_session_agent_lock(session_id):
                with LOCK:
                    ...
                    still_auto = (
                        ...
                        or _is_misassigned_provisional_title(effective_title, s.messages)
        ...
        emit_put = _resolve_title_event_sink(session_id, put_event)
        if wrote_title:
            if source == 'fallback':
                _put_title_status(emit_put, session_id, source, fallback_reason, effective_title, raw_preview)
            else:
                _put_title_status(emit_put, session_id, source, llm_status, effective_title, raw_preview)
            emit_put('title', {'session_id': session_id, 'title': effective_title})
        else:
            _put_title_status(emit_put, session_id, 'skipped', source or 'unchanged', effective_title, raw_preview)
    finally:
        _put_stream_close(put_event, session_id)
        _clear_title_event_sink(session_id, put_event)
...
def _run_background_title_refresh(session_id: str, user_text: str, assistant_text: str, current_title: str, put_event, agent=None):
    ...
    try:
        ...
        if next_title and _title_is_unhelpful_echo(
            ...
            user_text,
            s.messages,
            current_title=effective,
            assistant_text=assistant_text,
        ):
            logger.debug("Rejecting unhelpful echo title on refresh: %r", next_title)
            raw_preview = raw_preview or str(next_title)
            next_title = None
            llm_status = llm_status or 'llm_echo'
        if not next_title:
...
def _maybe_schedule_title_refresh(session, put_event, agent) -> bool:
...
def _sanitize_messages_for_api(
    ...
    *,
    ...
    """
    strip_native_images = _resolve_image_input_mode(cfg, provider=provider, model=model) == "text"
    ...
    for msg in messages:
        ...
        if sanitized.get('role') in ('tool', 'user') and sanitized.get('content'):
            try:
                from app.domain.workspace import rewrite_agent_visible_message_content

                content = sanitized.get('content')
                if not isinstance(content, list):
                    sanitized['content'] = rewrite_agent_visible_message_content(
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text):
    ...
    if _messages_have_prefix(result_messages, previous_context):
        ...
        marker_candidates = [
            normalize_compaction_message_role(m)
            for m in result_messages[:current_user_idx if current_user_idx is not None else len(result_messages)]
    ...
    for msg in candidates:
        ...
        merged.append(copy.deepcopy(normalize_compaction_message_role(display_msg)))
        if key is not None:
            seen.add(key)
    return merged


def _finalize_context_messages(messages):
    return normalize_compaction_message_roles(_deduplicate_context_messages(messages))
...
def _tool_result_snippet(raw, limit: int = _TOOL_RESULT_SNIPPET_MAX) -> str:
    ...
    try:
        ...
        pass
    try:
        from app.domain.workspace import nested_workspaces_enabled, rewrite_agent_visible_message_content

        if nested_workspaces_enabled():
            text = rewrite_agent_visible_message_content(text)
    except Exception:
        pass
    return text[:limit]
...
def _last_resort_sync_from_core(session, stream_id, agent_lock):
    ...
    Must never raise.
    """
    from app.domain.models import _get_profile_home, _apply_core_sync_or_error_marker
    try:
...
def _run_agent_streaming(
    ...
    *,
    ...
    old_workspace_virtual = None
    old_account_workspace_root = None
    old_profile_env = {}
    ...
    try:
        ...
        from app.domain.session_search_scope import (
            install_webui_session_search_scoping,
            normalize_session_owner_id,
            stamp_session_owner,
            tag_state_db_session_user,
            wrap_session_db_for_access,
        )

        _stream_access = get_request_user_access()
        install_webui_session_search_scoping()
        if not ephemeral and stamp_session_owner(s, _stream_access):
            s.save()
        _stream_owner_id = normalize_session_owner_id(_stream_access)
        update_active_run(stream_id, phase="running", session_id=session_id)
        _session_profile = getattr(s, "profile", None)
        try:
            from app.domain.profiles import get_hermes_home_for_profile

            _session_profile_home = get_hermes_home_for_profile(_session_profile)
        except Exception:
            _session_profile_home = None
        s.workspace = _resolve_agent_session_workspace(
            workspace,
        ...
        try:
            from app.domain.profiles import (
                ...
                patch_vision_virtual_path_rewrite,
            ...
            patch_terminal_virtual_path_rewrite = None
            apply_agent_runtime_patches = None
            patch_vision_virtual_path_rewrite = None
            patch_execute_code_session_approval = None
            patch_account_workspace_containment = None
        ...
        _session_workspace_virtual = None
        try:
            from app.domain.workspace import (
            ...
            if is_virtual_workspace_path(workspace):
                _session_workspace_virtual = str(workspace).strip()
            elif nested_workspaces_enabled() and _profile_home:
                _session_workspace_virtual = disk_path_to_virtual(
        ...
        _account_workspace_root = None
        try:
            from app.domain.workspace import account_workspace_containment_root

            _root_path = account_workspace_containment_root(
                workspace_disk=str(s.workspace),
                terminal_cwd=str(s.workspace),
                active_workspace_virtual=_session_workspace_virtual,
                profile_home=Path(_profile_home) if _profile_home else None,
            )
            if _root_path is not None:
                _account_workspace_root = str(_root_path)
        except Exception:
            _account_workspace_root = None
        ...
        _thread_env = _build_agent_thread_env(
            ...
            session_id,
            _profile_home,
            _session_workspace_virtual,
            _account_workspace_root,
        )
        ...
        with _ENV_LOCK:
            ...
            old_workspace_virtual = os.environ.get('HERMES_WEBUI_ACTIVE_WORKSPACE_VIRTUAL')
            old_account_workspace_root = os.environ.get('HERMES_WEBUI_ACCOUNT_WORKSPACE_ROOT')
            _apply_process_agent_env_unlocked(_thread_env)
            if apply_agent_runtime_patches is not None:
                apply_agent_runtime_patches()
            if _profile_home:
                ...
                if patch_terminal_virtual_path_rewrite is not None:
                    patch_terminal_virtual_path_rewrite()
                if patch_vision_virtual_path_rewrite is not None:
                    patch_vision_virtual_path_rewrite()
                if patch_execute_code_session_approval is not None:
                    patch_execute_code_session_approval()
                if patch_account_workspace_containment is not None:
                    patch_account_workspace_containment()
        ...
        try:
            from app.domain.mcp_runtime import (
                ...
                refresh_cached_agent_tools,
            )

            if _profile_home:
                discover_profile_mcp_tools(_profile_home)
                refresh_cached_agent_tools()
        except Exception:
        ...
        try:
            ...
            def _approval_notify_cb(approval_data):
                ...
                if isinstance(cmd, str):
                    from app.domain.workspace import display_virtual_paths_in_shell_command
                    data['command'] = display_virtual_paths_in_shell_command(cmd)
                put('approval', data)
                try:
                    from app.domain.routes import _approval_sse_notify_gateway
                    _approval_sse_notify_gateway(session_id)
                except Exception:
                    logger.debug(
                        "Failed to notify approval SSE after gateway push",
                        exc_info=True,
                    )
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            ...
            try:
                from app.domain.clarify import submit_pending as _submit_clarify_pending, clear_pending as _clear_clarify_pending
            except ImportError:
                return (
        ...
        try:
            ...
            def _tool_args_snapshot(args):
                from app.domain.workspace import display_virtual_paths_in_tool_args
                displayed = display_virtual_paths_in_tool_args(
                    args if isinstance(args, dict) else {},
                )
                args_snap = {}
                for k, v in list(displayed.items())[:4]:
                    s2 = str(v)
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                if event_type in (None, 'tool.started'):
                    _pin_process_agent_env(_thread_env)
                    ...
                    try:
                        ...
                        if _has_pending(session_id):
                            ...
                            if p:
                                ...
                                if isinstance(cmd, str):
                                    p = dict(p)
                                    p['command'] = display_virtual_paths_in_shell_command(cmd)
                                put('approval', p)
                    except ImportError:
                        pass
                    return
            ...
            try:
                ...
                if _stream_owner_id:
                    tag_state_db_session_user(_session_db, session_id, _stream_owner_id)
                _session_db = wrap_session_db_for_access(_session_db, _stream_access)
            except Exception as _db_err:
                print(f"[webui] WARNING: SessionDB init failed — session_search will be unavailable: {_db_err}", flush=True)

            # Streaming runs off the HTTP thread, so module-level cfg may reflect
            # the process-default profile rather than the session's stamped
            # profile. Load the session profile config.yaml first and pass it
            # through model/provider resolution (#profile-config-binding).
            _profile_cfg: dict = {}
            if _profile_home:
                try:
                    from app.domain.config import _load_yaml_config_file

                    _profile_cfg = _load_yaml_config_file(Path(_profile_home) / "config.yaml")
                except Exception:
                    _profile_cfg = {}

            _runtime_cfg = _profile_cfg if _profile_cfg else None
            resolved_model, resolved_provider, resolved_base_url = resolve_model_provider(
                model_with_provider_context(model, provider_context),
                _runtime_cfg,
            )

            if resolved_provider and _profile_cfg:
                try:
                    from app.domain.config import _get_provider_base_url

                    _profile_base_url = _get_provider_base_url(
                        resolved_provider,
                        _profile_cfg,
                    )
                    if _profile_base_url:
                        resolved_base_url = _profile_base_url
                except Exception:
                    pass
            ...
            resolved_provider, resolved_api_key, resolved_base_url = _resolve_custom_provider_runtime_overrides(
                resolved_provider, resolved_api_key, resolved_base_url, _runtime_cfg,
            )

            # Use session profile config on the streaming worker — get_config() would
            # fall back to the process-global active profile outside request context.
            if _profile_cfg:
                _cfg = _profile_cfg
            else:
                from app.domain.config import get_config as _get_config
                _cfg = _get_config()
            ...
            from app.domain.config import _resolve_cli_toolsets
            ...
            try:
                from app.domain.models import Session, SESSION_DIR
            ...
            try:
                from app.domain.config import parse_reasoning_effort as _parse_reff
            ...
            if ephemeral:
                ...
                with SESSION_AGENT_CACHE_LOCK:
                    ...
                    if _cached and _cached[1] == _agent_sig:
                        ...
                        try:
                            from app.domain.session_lifecycle import register_agent
                ...
                if agent is not None:
                    ...
                    with SESSION_AGENT_CACHE_LOCK:
                        ...
                        SESSION_AGENT_CACHE.move_to_end(session_id)  # LRU: mark as recently used
                        from app.domain.config import SESSION_AGENT_CACHE_MAX
            ...
            _apply_webui_api_max_retries(agent, _cfg)
            ...
            attachments, msg_text = stage_chat_attachments_to_workspace(
                attachments or [],
                str(s.workspace),
                message=msg_text,
            )
            _native_vision_turn = _should_embed_native_images_for_turn(
                _cfg,
                provider=resolved_provider,
                model=resolved_model,
            )
            _preserve_upload_path_hints = _preserve_upload_path_hints_for_native_turn(
                resolved_provider,
            )
            _omit_image_path_hints = (
                _native_vision_turn and not _preserve_upload_path_hints
            )
            _msg_before_native_strip = msg_text
            if _omit_image_path_hints:
                msg_text = strip_image_paths_from_attached_files_marker(
                    msg_text,
                    attachments,
                )
            _agent_msg_text = msg_text
            _attach_hint = build_attachment_agent_context(
                attachments,
                active_workspace=str(s.workspace),
                omit_images=_omit_image_path_hints,
            )
            if _attach_hint:
                _agent_msg_text = f"{_attach_hint}\n\n{_agent_msg_text}".strip()
            if _process_notifications:
                _agent_msg_text = "\n\n".join(
                    [*_process_notifications, _agent_msg_text],
                ).strip()
            ...
            if _native_vision_turn and not isinstance(user_message, list):
                _fallback_hint = build_attachment_agent_context(
                    attachments,
                    active_workspace=str(s.workspace),
                    omit_images=False,
                )
                _fallback_body = _msg_before_native_strip
                if _fallback_hint:
                    _fallback_body = f"{_fallback_hint}\n\n{_fallback_body}".strip()
                user_message = workspace_ctx + _fallback_body
            ...
            if ephemeral:
                ...
                for _m in reversed(result.get('messages') or []):
                    if isinstance(_m, dict) and _m.get('role') == 'assistant':
                        ...
                        break
                put('done', {
                    'session': prepare_session_dict_for_api_display({
                        'session_id': session_id,
            ...
            with _agent_lock:
                ...
                s.context_messages = _finalize_context_messages(_next_context_messages)
                ...
                if not _assistant_added and not _token_sent:
                    ...
                    if _is_quota:
                        ...
                        if _heal_rt is not None:
                            ...
                            with STREAMS_LOCK:
                                AGENT_INSTANCES[stream_id] = agent
                            from app.domain.config import SESSION_AGENT_CACHE as _SAC, SESSION_AGENT_CACHE_LOCK as _SAC_L
                            with _SAC_L:
                ...
                _should_bg_title, _u0, _a0 = _should_schedule_background_title(s)
                ...
                # (msg_text is the full message including trailing @path attachment tokens)
                if attachments:
                    display_attachments = [
                        _display_attachment_record(a)
                        for a in attachments
                        if _display_attachment_record(a)
                    ]
                    for m in reversed(s.messages):
                        if m.get('role') == 'user':
                            content = str(m.get('content', ''))
                            # Match if content is part of the sent message or vice-versa
                            from app.domain.upload import strip_attachment_path_suffix
                            base_text = strip_attachment_path_suffix(msg_text)
                ...
                if not ephemeral:
                    ...
                    try:
                        from app.domain.session_lifecycle import mark_turn_completed
            ...
            try:
                from app.domain.config import load_settings as _load_settings
            ...
            try:
                from app.domain.goals import evaluate_goal_after_turn, has_active_goal
            ...
            if _should_bg_title and _u0 and _a0:
                _register_title_event_sink(s.session_id, put)
                threading.Thread(
                    target=_run_background_title_update,
                    args=(s.session_id, _u0, _a0, str(s.title or '').strip(), put, agent),
                    daemon=True,
                ).start()
            elif not _maybe_schedule_title_refresh(s, put, agent):
                put('stream_close', {'session_id': session_id})
        finally:
            ...
            with _ENV_LOCK:
                ...
                if old_workspace_virtual is None:
                    os.environ.pop('HERMES_WEBUI_ACTIVE_WORKSPACE_VIRTUAL', None)
                else:
                    os.environ['HERMES_WEBUI_ACTIVE_WORKSPACE_VIRTUAL'] = old_workspace_virtual
                if old_account_workspace_root is None:
                    os.environ.pop('HERMES_WEBUI_ACCOUNT_WORKSPACE_ROOT', None)
                else:
                    os.environ['HERMES_WEBUI_ACCOUNT_WORKSPACE_ROOT'] = old_account_workspace_root
        ...
        if _exc_is_quota:
            ...
            if not _self_healed:
                ...
                if _heal_rt is not None:
                    ...
                    from app.domain.config import SESSION_AGENT_CACHE as _SAC2, SESSION_AGENT_CACHE_LOCK as _SAC2_L
                    with _SAC2_L:
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    from app.domain import config as _cfg
...
def cancel_stream(stream_id: str) -> bool:
    ...
    # Use module-level aliases (imported from app.domain.config at startup).
    ...
    with streams_lock:
        ...
        try:
            from app.domain.clarify import clear_pending as _clear_clarify_pending
            ...
            try:
                ...
                if _stripped or _has_reasoning or _has_tools:
                    ...
                    if _has_reasoning:
                        ...
                        reasoning_text = _cancel_reasoning.strip()
                        if reasoning_is_distinct_from_content(
                            reasoning_text,
                            _stripped,
                        ):
                            _partial_msg['reasoning'] = reasoning_text
                    if _has_tools:
