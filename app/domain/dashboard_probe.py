...
def save_dashboard_config(payload: dict) -> dict:
    ...
    if not patch:
        raise ValueError("no dashboard fields to save")

    from app.domain import config as webui_config
    ...
    if "enabled" in patch:
        enabled = str(patch.get("enabled") or "auto").strip().lower()
        if enabled not in _DASHBOARD_ENABLED_VALUES:
    ...
    if "url" in patch:
        raw_url = str(patch.get("url") or "").strip()
    ...
    return get_dashboard_config(config_data)
