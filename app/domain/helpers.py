...
from app.domain.config import IMAGE_EXTS, MD_EXTS
...
def _assistant_reasoning_trace(msg: dict) -> str:
    """Prefer ``reasoning_content`` (clean per-turn) over ``reasoning`` on read."""
    return str(msg.get("reasoning_content") or msg.get("reasoning") or "").strip()


def _normalize_assistant_reasoning_for_api(msg: dict) -> dict:
    """Expose a single ``reasoning`` field for the WebUI; drop duplicate traces."""
    if msg.get("role") != "assistant":
        return msg
    out = dict(msg)
    raw = _assistant_reasoning_trace(msg)
    ...
    if not reasoning_is_distinct_from_content(raw, content):
        out["reasoning"] = None
    else:
        out["reasoning"] = raw
    return out
...
def redact_session_data(session_dict: dict) -> dict:
    ...
    """
    from app.domain.config import load_settings
    from app.domain.streaming import prepare_session_dict_for_api_display

    _enabled = bool(load_settings().get("api_redact_enabled", True))
    result = prepare_session_dict_for_api_display(dict(session_dict))
    if isinstance(result.get('title'), str):
        result['title'] = _redact_text(result['title'], _enabled=_enabled)
    if 'messages' in result:
        messages = _redact_value(result['messages'], _enabled=_enabled)
        if isinstance(messages, list):
            messages = [
                _normalize_assistant_reasoning_for_api(m)
