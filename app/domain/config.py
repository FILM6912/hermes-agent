...
# Bedrock discovery in list_available_providers() triggers botocore IMDS probes
# (169.254.169.254). On Docker/LAN hosts without IAM roles that adds ~4s per
# cold /api/models build unless disabled (see tests/conftest.py).
os.environ.setdefault("AWS_EC2_METADATA_DISABLED", "true")

# ── Basic layout ──────────────────────────────────────────────────────────────
HOME = Path.home()
# REPO_ROOT is the repository root (parent of app/)
...
def _resolve_hermes_path(raw: str) -> Path:
    """Resolve profile-relative paths (./webui, ./workspace) under HERMES_HOME."""
    token = raw.strip()
    if not token:
        return (_hermes_home_base() / "webui").resolve()
    candidate = Path(token).expanduser()
    if candidate.is_absolute():
        return candidate.resolve()
    if token.startswith("./"):
        return (_hermes_home_base() / token[2:]).resolve()
    return (_hermes_home_base() / token).resolve()


STATE_DIR = _resolve_hermes_path(
    os.getenv("HERMES_WEBUI_STATE_DIR", str(_DEFAULT_HERMES_HOME / "webui"))
)
...
def _python_is_runnable(python_path: str) -> bool:
    """Return True when ``python_path`` can execute a trivial import-free script."""
    import subprocess
...
def virtual_env_root_for_python(python_exe: str) -> str | None:
    """Return the venv root directory for a ``bin/python`` interpreter, if any."""
    try:
        binary = Path(python_exe).resolve()
    except OSError:
        binary = Path(python_exe)
...
def resolve_webui_virtual_env() -> str | None:
    """Resolve the active WebUI/agent virtualenv for terminal + execute_code alignment."""
    explicit = os.getenv("HERMES_WEBUI_VIRTUAL_ENV", "").strip()
    if explicit:
        return explicit

    for candidate in (sys.executable, os.getenv("HERMES_WEBUI_PYTHON", "").strip()):
        if not candidate:
            continue
        root = virtual_env_root_for_python(candidate)
        if root and _python_is_runnable(candidate):
            return root
    ...
    for rel in ("venv", ".venv"):
        ...
        for name in ("python.exe", "python3.exe", "python", "python3"):
            ...
            if candidate.is_file() and _python_is_runnable(str(candidate)):
                return str(root)
...
def _discover_python(agent_dir: Path) -> str:
    ...
    Priority:
      ...
      2. Active WebUI interpreter (``sys.executable``) when it is a runnable venv
      3. Agent venv at <agent_dir>/venv/bin/python
      4. Local venv inside this repo (``venv/`` or ``.venv/``)
      5. System python3
    """
    if os.getenv("HERMES_WEBUI_PYTHON"):
        explicit = os.getenv("HERMES_WEBUI_PYTHON", "").strip()
        if explicit and _python_is_runnable(explicit):
            return explicit

    if sys.executable and _python_is_runnable(sys.executable):
    ...
    for name in ("python3", "python"):
        ...
        if found and _python_is_runnable(found):
            return found
...
def get_config() -> dict:
    ...
    with _cfg_lock:
        ...
        profile_path_changed = _cfg_path is not None and _cfg_path != config_path
        if (
            not _cfg_cache
            or profile_path_changed
            or (cache_stale and not _cfg_has_in_memory_overrides())
        ):
            reload_config()
        return _cfg_cache
...
def _expand_workspace_candidate(candidate: str | Path) -> Path | None:
    """Expand a workspace candidate, honoring profile-relative ./ paths."""
    token = str(candidate).strip()
    if not token:
        return None
    base = os.getenv('HERMES_HOME', '').strip()
    base_path = Path(base).expanduser() if base else HOME
    if token.startswith('./') or token == 'workspace':
        ...
        try:
            ...
            return None
    try:
        from app.domain.workspace import profile_workspace_rel, _resolve_canonical_profile_workspace
        if token == profile_workspace_rel():
            return _resolve_canonical_profile_workspace(base_path)
    except Exception:
        logger.debug("Failed to expand configured profile workspace candidate")
    try:
...
def _workspace_candidates(raw: str | Path | None = None) -> list[Path]:
    ...
    def add(candidate: str | Path | None) -> None:
        path = _expand_workspace_candidate(candidate) if candidate not in (None, "") else None
        if path is None:
            return
...
def _discover_default_workspace() -> Path:
    ...
    Resolve the default workspace in order:
      1. HERMES_WEBUI_DEFAULT_WORKSPACE env var (supports ./workspace under HERMES_HOME)
...
def print_startup_config() -> None:
    ...
    try:
        from app.document_api.integration import (
            document_api_enabled,
            is_document_api_feature_requested,
        )
        from app.core.outbound_proxy import outbound_proxy_configured, outbound_proxy_settings
        from app.domain.routes import _allowed_public_origins

        if outbound_proxy_configured():
            proxy = outbound_proxy_settings()
            lines.append(
                f"  outbound proxy: http={proxy.get('http', '(unset)')} https={proxy.get('https', '(unset)')}"
            )
        if document_api_enabled():
            lines.append("  document API: enabled")
        elif is_document_api_feature_requested():
            lines.append("  document API: requested but not ready (check SUPABASE_URL / deps)")
        else:
            lines.append(
                "  document API: disabled (HERMES_WEBUI_DOCUMENT_API_ENABLED=false or no SUPABASE_URL)"
            )
        allowed = _allowed_public_origins()
        if allowed:
            lines.append(f"  allowed origins: {len(allowed)} configured")
    except Exception:
        pass
    lines.append("")
...
# ── Limits ───────────────────────────────────────────────────────────────────
# Text reads via /api/file/read (editor + agent tool calls).
MAX_FILE_BYTES = 200_000
# Binary inline preview via /api/file/raw (PDF, DOCX, XLSX, PPTX download, etc.).
PREVIEW_MAX_BYTES = _env_mb_bytes("HERMES_WEBUI_PREVIEW_MAX_MB", 50)
...
def _provider_ids_equivalent(left: object, right: object) -> bool:
    """True when two provider slugs refer to the same configured provider."""
    left_raw = str(left or "").strip().lower()
    right_raw = str(right or "").strip().lower()
    if not left_raw or not right_raw:
        return False
    if left_raw == right_raw:
        return True
    left_alias = _resolve_provider_alias(left_raw)
    right_alias = _resolve_provider_alias(right_raw)
    return left_alias == right_alias


def _providers_cfg_entry(provider_id: object, config_obj: dict | None = None) -> dict:
    """Return the ``providers.<id>`` dict for *provider_id*, alias-aware."""
    source = config_obj if isinstance(config_obj, dict) else cfg
    ...
    if not pid:
        return {}
    direct = providers.get(pid) or providers.get(pid.lower())
    if isinstance(direct, dict):
        return direct
    for key, value in providers.items():
        if isinstance(value, dict) and _provider_ids_equivalent(key, provider_id):
            return value
    return {}
...
def _get_provider_base_url(provider_id, config_obj: dict | None = None):
    ...
    Checks two locations, in order:
      ...
      2. ``cfg["model"]["base_url"]`` — falls back here when
         ``cfg["model"]["provider"]`` matches *provider_id* (alias-aware, so
         ``lm-studio`` in config still resolves for ``lmstudio`` lookups).
         This is the historical shape (the model block carries both the active
         provider AND the base URL for that provider in a single record).

    Returns the URL stripped of trailing ``/`` if configured, otherwise None.
    """
    prov_cfg = _providers_cfg_entry(provider_id, config_obj)
    ...
    if isinstance(model_cfg, dict):
        ...
        if _provider_ids_equivalent(model_provider, provider_id):
...
def resolve_model_provider(model_id: str, config_obj: dict | None = None) -> tuple:
    ...
    When *config_obj* is supplied (e.g. a session profile's config.yaml loaded
    on a streaming worker thread), it overrides the process-global ``cfg`` so
    concurrent tabs on different profiles route to the correct provider/base_url.
    """
    source = config_obj if isinstance(config_obj, dict) else cfg
    ...
    custom_providers = source.get('custom_providers', [])
    ...
    if model_id.startswith("@") and ":" in model_id:
        ...
        return bare_model, provider_hint, _get_provider_base_url(provider_hint, source)
    ...
    if "/" in model_id:
        ...
        if prefix and config_provider and prefix != config_provider:
            _custom_cfg = source.get("custom_providers", [])
...
def resolve_custom_provider_connection(
    ...
    # Prefer caller-supplied profile config (streaming thread) over process-global cache.
...
def get_reasoning_status(
    *,
    ...
    effort = str(effort_raw or "").strip().lower() or None
    ...
    with _cfg_lock:
        ...
        if task == "__reset__":
            ...
            if not isinstance(slot_cfg, dict):
                slot_cfg = {}
            if update_provider:
                slot_cfg["provider"] = (provider or "auto") or "auto"
            if update_model:
                slot_cfg["model"] = model or ""
            if update_provider and provider and (
                provider.startswith("custom:") or provider == "custom"
            ):
                try:
                    _, _, resolved_base_url = resolve_model_provider(model or "")
                    if resolved_base_url:
...
_AVAILABLE_MODELS_CACHE_TTL: float = 86400.0  # overridden below after _env_float is defined
...
# Keep /api/v1/models responsive when live provider endpoints are slow.
...
# Keep /api/v1/models responsive when live provider endpoints are slow.
_LIVE_MODELS_TIMEOUT_S = _env_float(
    "HERMES_WEBUI_MODELS_LIVE_TIMEOUT",
    4.0,
    min_value=0.5,
)
_CACHE_BUILD_WAIT_TIMEOUT_S = _env_float(
    "HERMES_WEBUI_MODELS_CACHE_WAIT_TIMEOUT",
    8.0,
    min_value=0.5,
)
_AVAILABLE_MODELS_CACHE_TTL = _env_float(
    "HERMES_WEBUI_MODELS_CACHE_TTL",
    86400.0,
    min_value=0.0,
)
...
# Disk-backed /api/models cache survives container restarts when HERMES_HOME is
# bind-mounted (Docker Compose). Disable for deployments that must always probe
# live provider endpoints on every cold build.
_MODELS_DISK_CACHE_ENABLED = _env_bool("HERMES_WEBUI_MODELS_DISK_CACHE", default=True)
...
def _current_webui_version() -> str | None:
    ...
    `from app.domain.updates import WEBUI_VERSION` at the top of this module without a
    ...
    try:
        ...
        import sys as _sys
        mod = _sys.modules.get('app.domain.updates')
        if mod is None:
            return None
...
_LEGACY_MODELS_CACHE_BASENAME = "models_cache.json"
# Tests monkeypatch this module attribute to pin a fixed cache file path.
_models_cache_path = STATE_DIR / _LEGACY_MODELS_CACHE_BASENAME


def _models_cache_profile_slug() -> str:
    """Filesystem-safe slug for the active Hermes profile."""
    try:
        ...
        name = "default"
    safe = re.sub(r"[^\w.-]", "_", str(name).strip().lower())[:128]
    return safe or "default"


def _get_models_cache_path() -> Path:
    """Return the on-disk /api/models cache file for the active profile.

    Production uses one file per profile under ``STATE_DIR`` so switching
    profiles can reload a warm snapshot without deleting other profiles'
    caches. Tests may monkeypatch ``_models_cache_path`` to a fixed path.
    """
    explicit = _models_cache_path
    if isinstance(explicit, Path):
        if explicit.parent != STATE_DIR or explicit.name != _LEGACY_MODELS_CACHE_BASENAME:
            return explicit
    return STATE_DIR / f"models_cache.{_models_cache_profile_slug()}.json"


def _get_auth_store_path() -> Path:
    """Return the auth.json path for the active Hermes profile."""
    try:
        from app.domain.profiles import get_active_hermes_home as _gah
...
def _load_models_cache_from_disk() -> dict | None:
    ...
    try:
        ...
        if not cache_path.exists():
            legacy = STATE_DIR / _LEGACY_MODELS_CACHE_BASENAME
            if legacy != cache_path and legacy.exists():
                cache_path = legacy
            else:
                return None
...
def _save_models_cache_to_disk(cache: dict) -> None:
    ...
    """
    if not _MODELS_DISK_CACHE_ENABLED:
        return
    try:
        if not _is_valid_models_cache(cache):
            return
...
def invalidate_models_cache(*, delete_disk: bool = True):
    ...
    When ``delete_disk`` is True (default), also deletes the on-disk cache
    for the active profile so a subsequent cold build does not reload a stale
    snapshot. Profile switches pass ``delete_disk=False`` so each profile
    keeps its own warm ``models_cache.<profile>.json`` file.
...
def _prefetch_available_models_cache() -> None:
    """Warm the in-memory models cache after a profile switch (best-effort)."""
    try:
        get_available_models()
    except Exception:
        logger.debug("models cache prefetch after profile switch failed", exc_info=True)


def invalidate_models_cache_after_profile_switch() -> None:
    """Drop in-memory models cache for a profile switch; keep per-profile disk files."""
    invalidate_models_cache(delete_disk=False)
    threading.Thread(
        target=_prefetch_available_models_cache,
        daemon=True,
        name="hermes-models-cache-prefetch",
    ).start()
...
def invalidate_credential_pool_cache(provider_id: str):
    ...
    try:
        # api.providers imports from app.domain.config; keep this lazy to avoid
        # import-cycle/module-initialization issues.
        from app.domain.providers import invalidate_account_usage_status_cache
...
def get_available_models() -> dict:
    ...
    def _build_available_models_uncached() -> dict:
            ...
            if isinstance(_pool, dict) and _pool:
                try:
                    ...
                    for _pid in list(_pool.keys()):
                        try:
                            _raw_entries = _pool.get(_pid)
                            if isinstance(_raw_entries, list) and len(_raw_entries) == 0:
                                continue
        ...
        if not _hermes_auth_used:
            try:
                from app.domain.profiles import get_active_hermes_home as _gah2
        ...
        def _read_custom_endpoint_models(
            ...
            *,
            ...
            try:
                ...
                from app.core.outbound_proxy import build_url_opener

                req = urllib.request.Request(endpoint_url, method="GET")
                req.add_header("User-Agent", "OpenAI/Python 1.0")
                for k, v in headers.items():
                    req.add_header(k, v)
                with build_url_opener().open(req, timeout=_LIVE_MODELS_TIMEOUT_S) as response:
            ...
            for _cp in _custom_providers_cfg:
                ...
                for _cp_model in _cp_model_ids:
                    ...
                    if _cp_model and _dedup_key not in _seen_custom_ids:
                        if _slug and _slug in _named_custom_errors:
                            continue
            ...
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    try:
                        ...
                        with _urlreq.urlopen(_req, timeout=_LIVE_MODELS_TIMEOUT_S) as _resp:
                    ...
                    if lm_ids:
                        ...
                        if lm_base_url:
                            ...
                            try:
                                ...
                                with _urlreq.urlopen(req, timeout=_LIVE_MODELS_TIMEOUT_S) as resp:
        ...
        if default_model:
            ...
            elif _active_endpoint_error and cfg_base_url:
                logger.debug(
                    "Skipping default model injection after active endpoint probe failure",
                )
            elif active_provider and active_provider in _named_custom_errors:
                logger.debug(
                    "Skipping default model injection for unreachable provider %s",
                    active_provider,
                )
            else:
        ...
        # Drop unreachable provider groups and any optgroup that ended up with
        # zero models — those are pure UI noise in the model picker.
        groups = [g for g in groups if g.get("models")]
    ...
    disk_groups = None
    if _available_models_cache is None and _MODELS_DISK_CACHE_ENABLED:
    ...
    with _available_models_cache_lock:
        ...
        if _cache_build_in_progress:
            _cache_build_cv.wait_for(
                ...
                timeout=_CACHE_BUILD_WAIT_TIMEOUT_S,
            )
...
def _evict_session_agent(session_id: str) -> None:
    ...
    should_close = True
    try:
        from app.domain.session_lifecycle import commit_session_memory, has_uncommitted_work, unregister_agent
...
def _load_settings_stored_dict() -> dict | None:
    """Load raw settings document from WebUI DB with JSON fallback."""
    try:
        from app.storage.repositories.settings import get_settings_repository

        repo = get_settings_repository()
        repo.maybe_migrate_legacy()
        stored = repo.load_document()
        if isinstance(stored, dict):
            return stored
    except Exception:
        logger.debug("WebUI DB settings load failed; falling back to JSON")
    try:
    ...
    try:
        ...
        repo = get_settings_repository()
        if repo.import_legacy_document(stored):
            return repo.load_document() or stored
    except Exception:
        logger.debug("Failed to import settings.json into webui_settings", exc_info=True)
    return stored
...
def load_settings() -> dict:
    ...
    stored = _load_settings_stored_dict()
    if stored:
        settings.update(
            {
                k: v
...
def save_settings(settings: dict) -> dict:
    ...
    # #1560: env-var password overrides settings.json — never mutate hash on disk.
    if os.getenv("HERMES_WEBUI_PASSWORD", "").strip():
        settings.pop("_set_password", None)
        settings.pop("_clear_password", None)
    ...
    if not _password_changed and SETTINGS_FILE.exists():
        try:
            on_disk = json.loads(SETTINGS_FILE.read_text(encoding="utf-8"))
            disk_hash = on_disk.get("password_hash") if isinstance(on_disk, dict) else None
            if disk_hash and not current.get("password_hash"):
                current["password_hash"] = disk_hash
        except Exception:
            pass
    ...
    try:
        ...
        get_settings_repository().save_document(persisted)
        ...
        try:
            ...
            get_webui_store().append_history(
                "settings",
                "document",
                "save",
                payload={"keys": list(persisted.keys())},
            )
        except Exception:
            logger.debug("Settings save history append failed", exc_info=True)
    except Exception:
        logger.debug("WebUI DB settings save failed; writing JSON only", exc_info=True)
        SETTINGS_FILE.parent.mkdir(parents=True, exist_ok=True)
        SETTINGS_FILE.write_text(
            json.dumps(persisted, ensure_ascii=False, indent=2),
            encoding="utf-8",
    ...
    if "show_cli_sessions" in settings:
        from app.domain.models import clear_cli_sessions_cache
...
try:
    from app.domain.profiles import init_profile_state
