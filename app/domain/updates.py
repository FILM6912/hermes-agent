...
from app.domain.config import REPO_ROOT, STREAMS, STREAMS_LOCK

# Lazy -- may be None if agent not found
try:
    from app.domain.config import _AGENT_DIR
except ImportError:
    _AGENT_DIR = None
...
def _detect_webui_version() -> str:
    ...
    Resolution order:
      ...
      2. ``app/domain/_version.py`` — a fallback written by the Docker / CI release
    ...
    # Docker / baked-image fallback: app/domain/_version.py written by CI at build time.
    ...
    version_file = REPO_ROOT / 'app' / 'domain' / '_version.py'
