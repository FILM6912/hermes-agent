...
API shim note (FastAPI migration):
  ``Session`` and legacy session CRUD remain defined in this module. Lazy
  re-exports at the bottom expose ``SessionRepository`` / ``SessionService`` from
  ``app.*`` for the FastAPI layer. Thin ``repository_*`` helpers delegate to the
  repository without changing existing ``get_session`` / ``all_sessions`` imports.
  Do not add new business logic here — extend ``app.services.sessions`` instead.
"""
import collections
import copy
import datetime
...
from app.domain.compression_anchor import normalize_compaction_message_roles
...
class Session:
    def __init__(self, session_id: str=None, title: str='Untitled',
        ...
        self.title = title
        try:
            from app.domain.workspace import is_virtual_workspace_path, nested_workspaces_enabled

            if nested_workspaces_enabled() and is_virtual_workspace_path(workspace):
    ...
    def save(self, touch_updated_at: bool = True, skip_index: bool = False) -> None:
        ...
        METADATA_FIELDS = [
            ...
            'pinned', 'archived', 'project_id', 'profile', 'owner_user_id',
        ...
        # Post-save durability side effects (#1558 .bak + index) live in
        # app.domain.session_persistence — see persist_session() docstring.
        from app.domain.session_persistence import (
            refresh_session_index_after_save,
            write_shrink_guard_backup,
        )

        write_shrink_guard_backup(self.path, incoming_messages=self.messages or [])
        ...
        try:
            ...
            raise
        if not skip_index:
            refresh_session_index_after_save(self)
    ...
    def load(cls, sid):
        ...
        raw_context = data.get('context_messages')
        normalized_messages = normalize_compaction_message_roles(raw_messages)
        normalized_context = (
            normalize_compaction_message_roles(raw_context)
            if isinstance(raw_context, list)
            else raw_context
        )
        roles_migrated = (
            normalized_messages is not raw_messages
            or (isinstance(raw_context, list) and normalized_context is not raw_context)
        )
        data['messages'] = normalized_messages
        if isinstance(raw_context, list):
            data['context_messages'] = normalized_context
        ...
        if roles_migrated:
            try:
                ...
                logger.debug("Failed to persist compaction role migration for %s", sid, exc_info=True)
...
def _get_profile_home(profile) -> Path:
    ...
    Prefers the profile-specific helper from app.domain.profiles; falls back to the
...
def _journal_is_still_arriving(session, stream_id: str | None) -> bool:
    ...
    if not stream_id:
        return False
    try:
        from app.domain.run_journal import _run_path, latest_run_summary
...
def ensure_cron_project() -> str:
    ...
    """
    from app.domain.profiles import get_active_profile_name, _is_root_profile
...
def _cli_sessions_cache_ttl_seconds() -> float:
    raw = os.environ.get("HERMES_WEBUI_CLI_SESSIONS_CACHE_TTL")
...
_FASTAPI_LAYER_EXPORTS = {
    "SessionRepository": ("app.repositories.sessions", "SessionRepository"),
    "SessionService": ("app.services.sessions", "SessionService"),
}
...
def session_repository() -> "SessionRepository":
    """Return a SessionRepository instance (FastAPI migration shim)."""
    return _fastapi_layer_import("app.repositories.sessions", "SessionRepository")()


def session_service() -> "SessionService":
    """Return a SessionService instance (FastAPI migration shim)."""
    return _fastapi_layer_import("app.services.sessions", "SessionService")()


def repository_load_session(session_id: str) -> Session | None:
    """Delegate session load to SessionRepository (FastAPI migration shim)."""
    return session_repository().load(session_id)


def repository_load_session_metadata(session_id: str) -> Session | None:
    """Delegate metadata-only load to SessionRepository (FastAPI migration shim)."""
    return session_repository().load_metadata_only(session_id)


def repository_save_session(
    session: Session,
    *,
    touch_updated_at: bool = True,
    skip_index: bool = False,
) -> None:
    """Delegate session save to SessionRepository (FastAPI migration shim)."""
...
def repository_list_sessions(diag=None) -> list[dict]:
    """Delegate session listing to SessionRepository (FastAPI migration shim)."""
    return session_repository().list_sessions(diag=diag)
