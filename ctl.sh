...
_normalize_repo_dotenv_paths() {
  # Docker .env uses paths relative to HERMES_HOME (e.g. ./webui).
  if [[ "${HERMES_WEBUI_STATE_DIR:-}" == ./* ]]; then
    export HERMES_WEBUI_STATE_DIR="${HERMES_HOME}/${HERMES_WEBUI_STATE_DIR#./}"
  fi
  if [[ "${HERMES_WEBUI_DEFAULT_WORKSPACE:-}" == ./* ]]; then
    export HERMES_WEBUI_DEFAULT_WORKSPACE="${HERMES_HOME}/${HERMES_WEBUI_DEFAULT_WORKSPACE#./}"
  fi
}
...
_load_repo_dotenv_preserving_env() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    case "${key}" in UID|GID|EUID|PPID|BASHPID|SHLVL) continue ;; esac
    if [[ -n "${!key+x}" ]]; then
      value="${!key}"
      preserved+=("${key}=${value}")
    fi
  ...
  # Bash treats UID/GID as read-only; Docker .env files often set them.
  local _readonly_assign='^(UID|GID|EUID|PPID|BASHPID|SHLVL)='
  while IFS= read -r line || [[ -n "${line}" ]]; do
    line="${line#${line%%[![:space:]]*}}"
    [[ -z "${line}" || "${line}" == \#* ]] && continue
    [[ "${line}" =~ ${_readonly_assign} ]] && continue
    if [[ "${line}" == export\ * ]]; then
      line="${line#export }"
    fi
    [[ "${line}" == *=* ]] || continue
